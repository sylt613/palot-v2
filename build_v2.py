...
# Output dir pinned next to this script; OUT_PDF honors settings.py unless
# settings did not specify one (legacy default "palaot_v2.pdf") — in that
# case, fall back to variant-suffixed filename so A/B runs don't clobber.
_FIXING_BUILD_DIR = os.path.dirname(os.path.abspath(__file__))
S.OUT_DIR = os.path.join(_FIXING_BUILD_DIR, "output")

# Mamar-box variant: env var > settings.MAMAR_BOX_VARIANT > 'default'.
_MAMAR_BOX_VARIANT = (
    os.environ.get('MAMAR_BOX_VARIANT')
    or getattr(S, 'MAMAR_BOX_VARIANT', 'default')
    or 'default'
).strip().lower()

# Only auto-rename when settings.py did not set an explicit Tehila/custom
# filename — detected by the legacy default basename "palaot_v2.pdf" or a
# missing OUT_PDF attribute.
_out_pdf_settings = getattr(S, 'OUT_PDF', None)
_auto_rename = (
    not _out_pdf_settings
    or os.path.basename(_out_pdf_settings) in ('palaot_v2.pdf', 'palaot.pdf', 'palaot_tehila_full.pdf')
)
if _auto_rename:
    if _MAMAR_BOX_VARIANT != 'default':
        S.OUT_PDF = os.path.join(S.OUT_DIR, f"palaot_{_MAMAR_BOX_VARIANT}frame.pdf")
    else:
...
if (S.DROPCAP_FONT_NAME, S.DROPCAP_FONT_PATH) != (S.BODY_FONT_NAME, S.BODY_FONT_PATH):
...
_PNG_FRAME_CORNER     = os.path.join(_ORNAMENT_DIR, 'frame_corner_halo.png')
_STRIP_FRAME_CACHE    = {}  # keyed by (w_px, h_px, corner_px)
...
def _build_frame_image(target_w_px, target_h_px, intensity=1.0):
    ...
    try:
        ...
        F_TOP, F_BOT, F_LEFT, F_RIGHT = 173, 850, 36, 1499
        frame = src.crop((F_LEFT, F_TOP, F_RIGHT + 1, F_BOT + 1))
        FW, FH = frame.size  # 1464×678

        # ── Corner crop sizes in source pixels ──
        # Large enough to include full scroll ornament + rule overlap.
        CX      = 200    # horizontal extent of corner crop from each side
        CY_TOP  = 160    # vertical extent of top corners
        CY_BOT  = 160    # vertical extent of bottom corners
...
def _build_strip_frame(w_px, h_px, intensity=1.0):
    """Build a frame image at (w_px x h_px) using:
      - divider_post_anaf.png tiled as top/bottom/left/right border strips
      - frame_corner_halo.png (RGBA with edge fade) pasted at each corner,
        rotated 90/180/270 for the other three corners.
    Returns an RGB PIL Image (white background).
    """
    cache_key = (w_px, h_px, round(intensity, 3))
    cached = _STRIP_FRAME_CACHE.get(cache_key)
    if cached is not None:
        return cached

    if not os.path.exists(_PNG_DIVIDER_POST_ANAF) or not os.path.exists(_PNG_FRAME_CORNER):
        return None

    divider_src = Image.open(_PNG_DIVIDER_POST_ANAF).convert("RGBA")
    corner_src  = Image.open(_PNG_FRAME_CORNER).convert("RGBA")   # 1024x1024 RGBA with halo

    # Corner size: whichever is smaller of the two frame edges, capped at 35% of each
    corner_px = min(w_px, h_px, int(w_px * 0.35), int(h_px * 0.35))
    corner_px = max(corner_px, 10)

    # Scale corner to corner_px x corner_px
    cs = corner_px
    c_tl = corner_src.resize((cs, cs), Image.LANCZOS)
    c_tr = c_tl.rotate(-90, expand=True)
    c_br = c_tl.rotate(180, expand=True)
    c_bl = c_tl.rotate(90, expand=True)

    # ── Horizontal strips (top + bottom) ────────────────────────────────────
    div_w, div_h = divider_src.size   # 900x210
    # Scale divider so its height = cs
    h_scale = cs / div_h
    div_h_scaled_w = max(1, int(div_w * h_scale))
    div_h_unit = divider_src.resize((div_h_scaled_w, cs), Image.LANCZOS)

    inner_w = w_px - 2 * cs
    if inner_w > 0:
        tiles_h = (inner_w // div_h_scaled_w) + 2
        horiz_canvas = Image.new("RGBA", (tiles_h * div_h_scaled_w, cs), (255,255,255,0))
        for i in range(tiles_h):
            horiz_canvas.paste(div_h_unit, (i * div_h_scaled_w, 0), div_h_unit)
        horiz_strip = horiz_canvas.crop((0, 0, inner_w, cs))
    else:
        horiz_strip = None

    # ── Vertical strips (left + right) ──────────────────────────────────────
    # Rotate divider 90° → vertical unit
    div_v_src = divider_src.rotate(90, expand=True)   # was 900x210 → now 210x900
    dv_w, dv_h = div_v_src.size
    v_scale = cs / dv_w
    div_v_scaled_h = max(1, int(dv_h * v_scale))
    div_v_unit = div_v_src.resize((cs, div_v_scaled_h), Image.LANCZOS)

    inner_h = h_px - 2 * cs
    if inner_h > 0:
        tiles_v = (inner_h // div_v_scaled_h) + 2
        vert_canvas = Image.new("RGBA", (cs, tiles_v * div_v_scaled_h), (255,255,255,0))
        for i in range(tiles_v):
            vert_canvas.paste(div_v_unit, (0, i * div_v_scaled_h), div_v_unit)
        vert_strip = vert_canvas.crop((0, 0, cs, inner_h))
    else:
        vert_strip = None

    # ── Composite onto white canvas ─────────────────────────────────────────
    frame = Image.new("RGBA", (w_px, h_px), (255, 255, 255, 255))

    if horiz_strip:
        frame.paste(horiz_strip, (cs, 0), horiz_strip)
        frame.paste(horiz_strip.transpose(Image.FLIP_TOP_BOTTOM), (cs, h_px - cs), horiz_strip)
    if vert_strip:
        frame.paste(vert_strip, (0, cs), vert_strip)
        frame.paste(vert_strip.transpose(Image.FLIP_LEFT_RIGHT), (w_px - cs, cs), vert_strip)

    # Paste corners LAST, WITHOUT alpha mask — corners are fully opaque (ink + white),
    # so they completely cover and kill any strip line ends that poke into the corner zone.
    frame.paste(c_tl.convert("RGB"), (0, 0))
    frame.paste(c_tr.convert("RGB"), (w_px - cs, 0))
    frame.paste(c_bl.convert("RGB"), (0, h_px - cs))
    frame.paste(c_br.convert("RGB"), (w_px - cs, h_px - cs))
    ...
    # Flatten to RGB (white bg) — reportlab mask='auto' handles transparency via
    # the white background pixel masking done by draw_frame_box
    result = Image.new("RGB", (w_px, h_px), (255, 255, 255))
    result.paste(frame, mask=frame.split()[3])

    _STRIP_FRAME_CACHE[cache_key] = result
    return result


_ASSEMBLED_FRAME_CACHE = {}  # keyed by (w_px, h_px) -> ImageReader

def draw_frame_box(c, x_left, y_bot, width_pt, height_pt, intensity=1.0, png_path=None):
    """Draw the ornamental scroll frame at any size using the SVG-based tiled assembler.
    x_left, y_bot are PDF coordinates (bottom-left of frame). Returns True on success."""
    import io as _io
    FRAME_DPI = 200  # render resolution for the assembled frame
    px_w = max(80, int(width_pt  * FRAME_DPI / 72))
    px_h = max(40, int(height_pt * FRAME_DPI / 72))
    cache_key = (px_w, px_h, round(intensity, 3))
    ...
    if reader is None:
        try:
            ...
            if intensity < 1.0:
                ...
                r = r.point(lut); g = g.point(lut); b = b.point(lut)
                frame_img = Image.merge('RGB', (r, g, b))
            buf = _io.BytesIO()
            frame_img.save(buf, format='PNG')
            buf.seek(0)
            reader = ImageReader(buf)
            _ASSEMBLED_FRAME_CACHE[cache_key] = reader
        except Exception as _e:
            # Fallback: try static png_path
...
def _apply_white_gray_rings(canvas, near_dil=7, far_blur=12,
                             white_val=252, gray_val=120):
    """Wrapper around apply_halo_glow for backward compatibility."""
    return apply_halo_glow(canvas, near_dil=near_dil, far_blur=far_blur,
...
# ── Curl ornament line renderer (for TOC/mamar box) ──────────────────────────
# Uses the curly SVG file: /data/curly lines/good curls svg.svg
# viewBox 0 0 2048 2048, width=height=1024.
# Root group transform: (0.5, 0, 0, -0.5, 0, 1024) — SVG RL coords are 1024×1024.
#
# RL band ranges (y increases upward in RL):
#   Line 3 (top ornament, SVG y 1029–1261): RL y_bot≈393.5, RL y_top≈509.5  h≈116
#   Line 2 (bottom ornament, SVG y 564–821): RL y_bot≈613.5, RL y_top≈742   h≈128.5
_CURL_SVG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                               '..', '..', '..', 'curly lines', 'good curls svg.svg')
_CURL_DRAWING_CACHE = {}
_CURL_BAND_PIL_CACHE = {}     # (line_id, band_w_px) → PIL 'L' image of the band
_MAMAR_BOX_CACHE = {}         # (w_pt, h_pt, dpi, line_w_pt) → ImageReader, halo_pad_pt

_MAMAR_BOX_GPT15_PATH = os.path.join(S.OUT_DIR, 'mamar_box_gpt15.png')
_MAMAR_BOX_GPT15_BBOX = (297, 62, 1240, 901)  # crop bounds from 1536x1024 source

# Alternate frame asset (user-supplied, already cropped tight to content).
_MAMAR_BOX_ALT_PATH = os.path.join(S.OUT_DIR, 'mamar_box_alt_asset.png')
_MAMAR_BOX_ALT_BBOX = (0, 0, 1194, 993)

# New box variant (clean gray rounded-rect with centered top/bottom flourishes).
# Source: user-supplied gpt-image-1.5 render. Ink bbox on 1536x1024 canvas.
_MAMAR_BOX_NEWBOX_PATH = os.path.join(S.OUT_DIR, 'mamar_box_newbox.png')
_MAMAR_BOX_NEWBOX_BBOX = (161, 155, 1374, 840)

# Built-in variant registry: each entry → (default_path, default_bbox, default_top_h_src).
# top_h_src=None means auto-detect from straight-band sy0.
_MAMAR_BOX_VARIANTS = {
    'default': (_MAMAR_BOX_GPT15_PATH,  _MAMAR_BOX_GPT15_BBOX,  275),
    'alt':     (_MAMAR_BOX_ALT_PATH,    _MAMAR_BOX_ALT_BBOX,    408),
    'newbox':  (_MAMAR_BOX_NEWBOX_PATH, _MAMAR_BOX_NEWBOX_BBOX, None),
}
# Per-variant nominal ornament height (pt). May be overridden by settings.MAMAR_BOX_ORN_H_PT.
_MAMAR_BOX_ORN_H_PT = {'default': 13.0, 'alt': 32.0, 'newbox': 42.0}

def _mb_variant_source(variant):
    """Return (path, bbox, top_h_src_or_None) for the given variant.
    Settings overrides (MAMAR_BOX_IMAGE_PATH / MAMAR_BOX_BBOX) take precedence."""
    v = (variant or 'default').lower()
    path, bbox, top_h_src = _MAMAR_BOX_VARIANTS.get(v, _MAMAR_BOX_VARIANTS['default'])
    s_path = getattr(S, 'MAMAR_BOX_IMAGE_PATH', None)
    s_bbox = getattr(S, 'MAMAR_BOX_BBOX', None)
    if s_path:
        path = s_path
    if s_bbox:
        bbox = tuple(s_bbox)
    return path, bbox, top_h_src

def _mamar_orn_h_pt(variant=None):
    """Nominal ornament height (pt) per variant. Settings override wins."""
    s_val = getattr(S, 'MAMAR_BOX_ORN_H_PT', None)
    if s_val is not None:
        return float(s_val)
    v = (variant or _MAMAR_BOX_VARIANT or 'default').lower()
    return _MAMAR_BOX_ORN_H_PT.get(v, 13.0)

# Source geometry constants (measured from cropped frame 943×839).
_MB_TOP_H_SRC    = 275   # rows occupied by top ornament
_MB_BOT_Y_SRC    = 567   # row where bottom ornament begins
_MB_CENTER_X0    = 215   # left edge of center ornament
_MB_CENTER_X1    = 729   # right edge of center ornament
_MB_CORNER_W_SRC = 130   # (legacy – unused in new 9-slice code)
_MB_SIDE_W_SRC   = 60    # (legacy – unused)
_MB_TILE_COL     = 172   # 1-px source col for horizontal injection (inside the
                         # straight top/bottom run between corner and center
                         # ornament; mirrored at fw-_MB_TILE_COL for the right).
_MB_SIDE_ROW     = 421   # 1-px source row for vertical injection — exact
                         # vertical center of the frame's straight side run.


def _mamar_box_min_height_px(target_w_px, orn_h_px=83):
    """Minimum total image height (px) to fit all ornament zones at given ornament height."""
    s = orn_h_px / float(_MB_TOP_H_SRC)
    fh_src = _MB_BOT_Y_SRC + (_MB_TOP_H_SRC * (_MB_TOP_H_SRC / _MB_TOP_H_SRC))  # ≈ src frame h
    # fh from image: top_h + mid_h + bot_h
    mid_h_src = _MB_BOT_Y_SRC - _MB_TOP_H_SRC
    bot_h_src_approx = _MB_TOP_H_SRC  # roughly symmetric
    return max(1, round((orn_h_px + mid_h_src * s + bot_h_src_approx * s)))


def _mb_tile_horiz(slice_img, target_w):
    """Tile an RGBA slice horizontally to fill target_w."""
    out = Image.new('RGBA', (target_w, slice_img.height), (0, 0, 0, 0))
    x = 0
    while x < target_w:
        out.alpha_composite(slice_img, (x, 0))
        x += slice_img.width
    return out.crop((0, 0, target_w, slice_img.height))


def _mb_resize_horiz(slice_img, target_w):
    """Resize an RGBA slice horizontally to target_w without changing height."""
    if slice_img.width == target_w:
        return slice_img
    return slice_img.resize((target_w, slice_img.height), Image.LANCZOS)


def _mb_tile_1px_horiz(slice_img, target_w):
    """Tile a 1-px-wide RGBA slice horizontally to fill target_w.
    A single column is perfectly seamless — no texture repetition artifacts."""
    if slice_img.width != 1:
        # Take the horizontal centre pixel column as the tile
        cx = slice_img.width // 2
        slice_img = slice_img.crop((cx, 0, cx + 1, slice_img.height))
    out = Image.new('RGBA', (target_w, slice_img.height), (0, 0, 0, 0))
    x = 0
    while x < target_w:
        out.alpha_composite(slice_img, (x, 0))
...
def _mb_detect_straight_band(stroke_mask_arr, bbox):
    """Find the longest run of 'straight' rows in the frame.

    A row is straight iff its leftmost and rightmost ink pixel sit within 2px
    of the global left/right extremes — i.e. the side walls are fully vertical
    with no corner curvature.  Returns (y0, y1, sample_y) in *image* coords.
    """
    import numpy as _np
    l, t, r, b = bbox
    left_min = None
    ...
    rows = []
    for y in range(t, b):
        cols = _np.where(stroke_mask_arr[y, l:r])[0]
        if len(cols) == 0:
            rows.append(None)
            continue
        rl, rr = int(cols.min()) + l, int(cols.max()) + l
        rows.append((rl, rr))
        left_min = rl if left_min is None else min(left_min, rl)
        right_max = rr if right_max is None else max(right_max, rr)
    ...
    for row in rows:
        ...
        rl, rr = row
        good.append(abs(rl - left_min) <= 2 and abs(rr - right_max) <= 2)
    ...
    for i, ok in enumerate(good):
        if ok and cur is None:
            cur = i
        if (not ok or i == len(good) - 1) and cur is not None:
            end = i if (ok and i == len(good) - 1) else i - 1
            if best_s is None or end - cur > best_e - best_s:
    ...
    if best_s is None:
        ...
        run_len = best_e - best_s
        pad = max(24, min(run_len // 3, 80))
        y0 = t + best_s + pad
        y1 = t + best_e - pad
    sample_y = (y0 + y1) // 2
    return y0, y1, sample_y


def _mb_detect_cap_split(frame_rgba, top_frac=0.28, ink_thresh=40, pad_px=8,
                         left_fixed_frac=0.14,
                         right_fixed_frac=0.86):
    """Auto-detect the horizontal extent of the center ornament in the top cap.

    Returns (x1_frac, x2_frac, x3_frac, x4_frac) as fractions of frame width:
      [0 .. x1]   left corner (preserved)
      [x1 .. x2]  left rail   (1-px tiled)
      [x2 .. x3]  center ornament (preserved)
      [x3 .. x4]  right rail  (1-px tiled)
      [x4 .. 1]   right corner (preserved)

    left_fixed_frac / right_fixed_frac are hard floors/ceilings for x1/x4 so
    that undetected corners (whose curls may not appear in the top cap band)
    are still given a generous preserved slab.
    """
    import numpy as _np
    arr = _np.array(frame_rgba.convert('RGBA'))
    alpha = arr[..., 3]
    gray  = arr[..., :3].mean(axis=2)
    ink   = (gray < 245) & (alpha > 16)
    h, w  = ink.shape
    cap_h = max(1, int(h * top_frac))
    top_cap = ink[:cap_h, :]
    col_ink = top_cap.sum(axis=0)
    strong  = col_ink > ink_thresh
    ...
    for i, on in enumerate(strong):
        if on and start is None:
            start = i
        if start is not None and ((not on) or i == len(strong) - 1):
            end = i if (on and i == len(strong) - 1) else i - 1
            runs.append((start, end))
            start = None

    if not runs:
        return (left_fixed_frac, left_fixed_frac + 0.04,
                right_fixed_frac - 0.04, right_fixed_frac)

    mid = w // 2
    center_l, center_r = max(
        runs,
        key=lambda rr: ((rr[1] - rr[0]), -abs(((rr[0] + rr[1]) // 2) - mid)),
    )
    center_l = max(0, center_l - pad_px)
    center_r = min(w, center_r + 1 + pad_px)

    # x1 = corner/rail split point on the left side.
    # Use the midpoint of [edge .. center_l] but never exceed left_fixed_frac.
    x1 = min(int(w * left_fixed_frac), max(1, center_l - 4))
    # x4 = corner/rail split point on the right side.
    x4 = max(int(w * right_fixed_frac), min(w - 1, center_r + 4))

    return (x1 / w, center_l / w, center_r / w, x4 / w)


def _mb_expand_cap_width(cap, target_w, x1, x2, x3, x4, lsx, rsx):
    """Extend a top/bottom cap strip to target_w using 5-region 1-px tiling.

        [0  .. x1]  left corner curl  — preserved as-is
        [x1 .. x2]  left rail        — 1-px column at lsx, tiled
        [x2 .. x3]  center ornament  — preserved as-is
        [x3 .. x4]  right rail       — 1-px column at rsx, tiled
        [x4 .. W ]  right corner     — preserved as-is

    lsx / rsx are x-coords of the 1-px sample columns (must sit inside the
    smooth rail zones, away from corner curves and ornament shoulders).
    """
    if cap.width >= target_w:
        # Source already wider than target: keep pixel scale (preserves
        # ornament size + line thickness) and center-crop the rail zones.
        # The left/right corner curls and the center ornament are preserved
        # by cropping only the straight rail areas between them.
        fixed_l = x1                     # left corner width
        fixed_c = max(1, x3 - x2)        # center ornament width
        fixed_r = cap.width - x4         # right corner width
        fixed   = fixed_l + fixed_c + fixed_r
        if fixed >= target_w:
            # Corners + center alone overflow → final safety: LANCZOS shrink.
            return cap.resize((target_w, cap.height), Image.LANCZOS)
        rail_total = target_w - fixed
        # Split remaining width evenly between the two rail zones
        rail_l = rail_total // 2
        rail_r = rail_total - rail_l
        left   = cap.crop((0, 0, fixed_l, cap.height))
        center = cap.crop((x2, 0, x3, cap.height))
        right  = cap.crop((x4, 0, cap.width, cap.height))
        # Crop rail zones to the shorter lengths we need
        rail_l_src_end = min(x2, x1 + rail_l)
        rail_r_src_beg = max(x3, x4 - rail_r)
        rail_left  = cap.crop((x1, 0, rail_l_src_end, cap.height))
        rail_right = cap.crop((rail_r_src_beg, 0, x4, cap.height))
        out = Image.new('RGBA', (target_w, cap.height), (0, 0, 0, 0))
        xc = 0
        out.alpha_composite(left,       (xc, 0)); xc += left.width
        out.alpha_composite(rail_left,  (xc, 0)); xc += rail_left.width
        out.alpha_composite(center,     (xc, 0)); xc += center.width
        out.alpha_composite(rail_right, (xc, 0))
        out.alpha_composite(right, (target_w - right.width, 0))
        return out

    left   = cap.crop((0,  0, x1,         cap.height))
    center = cap.crop((x2, 0, x3,         cap.height))
    right  = cap.crop((x4, 0, cap.width,  cap.height))

    lsx_c = max(x1, min(x2 - 1, lsx))
    rsx_c = max(x3, min(x4 - 1, rsx))
    left_tile  = cap.crop((lsx_c, 0, lsx_c + 1, cap.height))
    right_tile = cap.crop((rsx_c, 0, rsx_c + 1, cap.height))

    fixed_w = left.width + center.width + right.width
    grow    = max(0, target_w - fixed_w)
    src_l   = max(1, x2 - x1)
    src_r   = max(1, x4 - x3)
    total   = src_l + src_r
    left_w  = int(round(grow * src_l / total))
    right_w = grow - left_w

    left_fill  = _mb_tile_1px_horiz(left_tile,  max(1, left_w))
    right_fill = _mb_tile_1px_horiz(right_tile, max(1, right_w))

    out = Image.new('RGBA', (target_w, cap.height), (0, 0, 0, 0))
    x = 0
    out.alpha_composite(left,       (x, 0)); x += left.width
    out.alpha_composite(left_fill,  (x, 0)); x += left_fill.width
    out.alpha_composite(center,     (x, 0)); x += center.width
    out.alpha_composite(right_fill, (x, 0)); x += right_fill.width
    out.alpha_composite(right, (target_w - right.width, 0))
    return out


def _mb_expand_middle_band(band_img, target_w, left_fixed, right_fixed):
    """Extend the straight 1-px-tall middle row to target_w.

    Preserves the full side-wall slab (left_fixed / right_fixed pixels each
    side, containing both vertical double-lines).  The pure-white interior gap
    between the walls is filled by tiling a single centre pixel — guaranteed
    seamless.
    """
    if band_img.width >= target_w:
        # Source wider than target: keep pixel scale (preserves wall line
        # thickness) and trim from the interior gap, not the side walls.
        fixed = left_fixed + right_fixed
        if fixed >= target_w:
            return band_img.resize((target_w, band_img.height), Image.LANCZOS)
        mid_w = target_w - fixed
        left  = band_img.crop((0, 0, left_fixed, band_img.height))
        right = band_img.crop((band_img.width - right_fixed, 0, band_img.width, band_img.height))
        sample_x = max(left_fixed, min(band_img.width - right_fixed - 1, band_img.width // 2))
        mid_tile = band_img.crop((sample_x, 0, sample_x + 1, band_img.height))
        mid_fill = _mb_tile_1px_horiz(mid_tile, mid_w)
        out = Image.new('RGBA', (target_w, band_img.height), (0, 0, 0, 0))
        out.alpha_composite(left, (0, 0))
        out.alpha_composite(mid_fill, (left.width, 0))
        out.alpha_composite(right, (target_w - right.width, 0))
        return out
    left  = band_img.crop((0, 0, left_fixed, band_img.height))
    right = band_img.crop((band_img.width - right_fixed, 0, band_img.width, band_img.height))
    # Sample from the exact centre of the interior gap
    sample_x = max(left_fixed, min(band_img.width - right_fixed - 1, band_img.width // 2))
    mid_tile  = band_img.crop((sample_x, 0, sample_x + 1, band_img.height))
    mid_w     = max(1, target_w - left.width - right.width)
    mid_fill  = _mb_tile_1px_horiz(mid_tile, mid_w)
    out = Image.new('RGBA', (target_w, band_img.height), (0, 0, 0, 0))
    out.alpha_composite(left,     (0, 0))
    out.alpha_composite(mid_fill, (left.width, 0))
...
# ── Mamar-box source geometry (auto-detected at first use, cached here) ───────
_MB_GEOM_CACHE        = {}     # variant → (cap_split_fracs, straight_band)
# Internal source-render scale.  BOOST > 1 renders the source frame at a
# larger pixel scale (thicker lines, larger ornaments); the outer drawn
# rectangle stays the same size (rails get trimmed/tiled less).
_MB_ROOT_SCALE_MAX    = 3.00
_MB_ROOT_SCALE_BOOST  = 1.00


def _mb_get_geometry(frame_rgba, stroke_mask_arr, bbox, variant='default'):
    """Return cached (cap_split_fracs, straight_band) for the source frame."""
    key = (variant or 'default').lower()
    cached = _MB_GEOM_CACHE.get(key)
    if cached is None:
        # Settings-driven overrides (set for a tuned variant or hand-tweak):
        #   MAMAR_BOX_CAP_SPLIT_FRACS: (x1,x2,x3,x4) in [0..1] to skip detection
        #   MAMAR_BOX_CAP_INK_THRESH: per-column ink floor for auto-detect
        s_fracs = getattr(S, 'MAMAR_BOX_CAP_SPLIT_FRACS', None)
        s_thr   = getattr(S, 'MAMAR_BOX_CAP_INK_THRESH', None)
        if s_fracs and len(s_fracs) == 4:
            cap_fr = tuple(float(v) for v in s_fracs)
        else:
            cap_fr = _mb_detect_cap_split(
                frame_rgba,
                ink_thresh=(int(s_thr) if s_thr is not None else 40),
            )
        cached = (cap_fr,
                  _mb_detect_straight_band(stroke_mask_arr, bbox))
        _MB_GEOM_CACHE[key] = cached
    return cached


def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83, variant=None, halo_pad_px=0):
    """Assemble the GPT-generated mamar/TOC box using the shar-blatt
    reference algorithm: aspect-locked scale + 5-region 1-px-tiling.

    variant: 'default' (gpt15 frame) or 'alt' (user-supplied frame).
    """
    variant = (variant or _MAMAR_BOX_VARIANT or 'default').lower()
    src_path, src_bbox, top_h_src_hint = _mb_variant_source(variant)
    if not os.path.exists(src_path):
        return None
    ...
    l, t, r, b = src_bbox
    frame = src.crop((l, t, r, b))
    fw, fh = frame.size

    # Build stroke mask for geometry detection (dark non-white pixels)
    arr   = _np.array(frame)
    gray  = arr[..., :3].mean(axis=2)
    alpha = arr[..., 3]
    stroke_mask = (gray < 245) & (alpha > 16)
    bbox_local  = (0, 0, fw, fh)

    split_fracs, (sy0, sy1, ssamp) = _mb_get_geometry(frame, stroke_mask, bbox_local, variant=variant)

    # Top-cap source height: pinned for default variant (matches approved
    # visual), auto-detected from straight-band for others.
    top_h_src = top_h_src_hint if top_h_src_hint is not None else max(40, int(sy0))

    # Single aspect-locked source scale, then tiling only.
    # Scale is calibrated so the source top-cap renders at ~orn_h_px,
    # making ornament size independent of box aspect ratio.
    fit_scale = min(target_w_px / fw, target_h_px / fh)
    root_scale = min(_MB_ROOT_SCALE_MAX,
                     (orn_h_px / max(1.0, (top_h_src * fit_scale))) * _MB_ROOT_SCALE_BOOST)
    scale = max(0.02, fit_scale * root_scale)

    sw = max(4, round(fw * scale))
    sh = max(4, round(fh * scale))
    scaled = frame.resize((sw, sh), Image.LANCZOS)

    # Straight band in scaled coords
    local_y0   = max(1, min(sh - 3, round(sy0   * scale)))
    local_y1   = max(local_y0 + 1, min(sh - 2, round(sy1   * scale)))
    local_samp = max(local_y0, min(local_y1, round(ssamp * scale)))

    top_cap     = scaled.crop((0, 0,          sw, local_y0))
    mid_row     = scaled.crop((0, local_samp, sw, local_samp + 1))
    bottom_cap  = scaled.crop((0, local_y1,   sw, sh))

    # Cap split coords in scaled px
    x1 = max(1,      min(sw - 4, round(split_fracs[0] * sw)))
    x2 = max(x1 + 1, min(sw - 3, round(split_fracs[1] * sw)))
    x3 = max(x2 + 1, min(sw - 2, round(split_fracs[2] * sw)))
    x4 = max(x3 + 1, min(sw - 1, round(split_fracs[3] * sw)))

    # 1-px sample positions: midpoint of each rail zone
    lsx = (x1 + x2) // 2
    rsx = (x3 + x4) // 2

    top_cap    = _mb_expand_cap_width(top_cap,    target_w_px, x1, x2, x3, x4, lsx, rsx)
    bottom_cap = _mb_expand_cap_width(bottom_cap, target_w_px, x1, x2, x3, x4, lsx, rsx)

    # Side wall width = x1 (left) and sw-x4 (right) — the full corner slab
    # containing both vertical double-lines.  Clamp to a floor so the two-line
    # wall does not collapse when the overall frame is scaled down by ornament
    # height constraints.
    side_wall_floor = max(8, round(target_w_px * 0.015))
    side_left_w  = max(side_wall_floor, x1)
    side_right_w = max(side_wall_floor, sw - x4)
    side_left_w  = min(side_left_w, sw // 3)
    side_right_w = min(side_right_w, sw // 3)
    mid_row = _mb_expand_middle_band(mid_row, target_w_px, side_left_w, side_right_w)

    top_h = top_cap.height
    bot_h = bottom_cap.height

    if top_h + bot_h >= target_h_px:
        # Caps are already taller than the box.  Rather than squash the whole
        # frame (which kills the line-thickness/ornament-size boost), trim
        # from the straight-band edges of each cap (top-cap's bottom row and
        # bottom-cap's top row are straight rails and safe to crop).
        min_split = max(4, target_h_px // 2)
        new_top_h = min(top_h, min_split)
        new_bot_h = min(bot_h, target_h_px - new_top_h)
        if new_top_h + new_bot_h < target_h_px:
            # Fill any leftover middle with tiled mid-row even though it's small
            mid_h = target_h_px - new_top_h - new_bot_h
        else:
            mid_h = 0
        top_cap    = top_cap.crop((0, 0, target_w_px, new_top_h))
        bottom_cap = bottom_cap.crop((0, bot_h - new_bot_h, target_w_px, bot_h))
        out_rgba = Image.new('RGBA', (target_w_px, target_h_px), (0, 0, 0, 0))
        out_rgba.alpha_composite(top_cap, (0, 0))
        for row_y in range(new_top_h, new_top_h + mid_h):
            out_rgba.alpha_composite(mid_row, (0, row_y))
        out_rgba.alpha_composite(bottom_cap, (0, target_h_px - new_bot_h))
        out_rgb = Image.new('RGB', (target_w_px, target_h_px), (255, 255, 255))
        out_rgb.paste(out_rgba, (0, 0), out_rgba)
    else:
        out_rgba = Image.new('RGBA', (target_w_px, target_h_px), (0, 0, 0, 0))
        out_rgba.alpha_composite(top_cap, (0, 0))
        # Tile the mid row vertically (1px high, so composite once per row)
        mid_h = max(1, target_h_px - top_h - bot_h)
        for row_y in range(top_h, top_h + mid_h):
            out_rgba.alpha_composite(mid_row, (0, row_y))
        out_rgba.alpha_composite(bottom_cap, (0, target_h_px - bot_h))
        out_rgb = Image.new('RGB', (target_w_px, target_h_px), (255, 255, 255))
        out_rgb.paste(out_rgba, (0, 0), out_rgba)

    # Flood-fill interior to pure white so PDF transparency mask drops it.
    try:
        from PIL import ImageDraw
        cx = target_w_px // 2
        cy = target_h_px // 2
        ImageDraw.floodfill(out_rgb, (cx, cy), (255, 255, 255), thresh=40)
    except Exception:
        pass

    # Optional halo post-processing (mirrors draw_post_anaf_divider_band).
    if getattr(S, 'MAMAR_BOX_HALO', False):
        try:
            # Embed the tiled box on a larger white canvas so the halo has
            # room to fade without getting clipped.
            pad = max(0, int(halo_pad_px))
            if pad > 0:
                bigger = Image.new('RGB',
                                   (target_w_px + 2*pad, target_h_px + 2*pad),
                                   (255, 255, 255))
                bigger.paste(out_rgb, (pad, pad))
                out_rgb = bigger
            out_rgb = _apply_mamar_box_halo(out_rgb)
        except Exception as _e:
            # Fail-soft: halo is cosmetic; bail quietly if anything goes wrong.
            pass

    return out_rgb


def _apply_mamar_box_halo(box_rgb):
    """Apply a post-anaf-style white frame with a soft gray outer ring.

    The gray is ring-only: about the same thickness as the white frame growth,
    with only its outside edge feathered slightly. No dark source ink is pasted
    back unless explicitly enabled.
    """
    import numpy as _np
    from PIL import ImageFilter

    near_dil      = int(getattr(S, 'MAMAR_BOX_HALO_NEAR_DIL', 22))
    ring_dil      = int(getattr(S, 'MAMAR_BOX_HALO_RING_DIL', near_dil * 2))
    far_blur      = float(getattr(S, 'MAMAR_BOX_HALO_FAR_BLUR', 18))
    white_val     = int(getattr(S, 'MAMAR_BOX_HALO_WHITE_VAL', 255))
    gray_val      = int(getattr(S, 'MAMAR_BOX_HALO_GRAY_VAL', 150))
    fade_strength = float(getattr(S, 'MAMAR_BOX_HALO_FADE_STRENGTH', 0.60))
    preserve_ink  = bool(getattr(S, 'MAMAR_BOX_HALO_PRESERVE_INK', False))

    def _odd(n):
        n = max(3, int(round(n)))
        return n if n % 2 == 1 else n + 1
    white_k = _odd(near_dil * 2 + 1)
    gray_k  = _odd((near_dil * 2) + (ring_dil * 2) + 1)

    # Expand canvas so the halo fades out into room, not into the crop edge.
    # (The caller normally provides padding already; add a small safety margin
    # in case it didn't.)
    ext_pad = int(getattr(S, 'MAMAR_BOX_HALO_EXTRA_PAD', 0))
    if ext_pad > 0:
        src_arr = _np.array(box_rgb.convert('RGB'))
        H0, W0 = src_arr.shape[:2]
        arr = _np.full((H0 + 2*ext_pad, W0 + 2*ext_pad, 3), 255, dtype=_np.uint8)
        arr[ext_pad:ext_pad+H0, ext_pad:ext_pad+W0] = src_arr
    else:
        arr = _np.array(box_rgb.convert('RGB'))

    lum = arr.mean(axis=2)

    # Optional source-ink darkening kept for parity (harmless when we don't
    # re-paste the ink later).
    dk_thr = getattr(S, 'MAMAR_BOX_INK_DARKEN_THRESHOLD', None)
    dk_tgt = getattr(S, 'MAMAR_BOX_INK_DARKEN_TARGET', None)
    if dk_thr is not None and dk_tgt is not None:
        thr = float(dk_thr); tgt = float(dk_tgt)
        dark_mask = lum < thr
        if dark_mask.any():
            scale = tgt / max(1.0, thr)
            new_lum = _np.clip(lum * scale, 0, 255)
            ratio = _np.where(lum > 0, new_lum / _np.maximum(lum, 1.0), 1.0)
            ratio3 = ratio[..., None].astype(_np.float32)
            arr = _np.where(dark_mask[..., None],
                            _np.clip(arr.astype(_np.float32) * ratio3, 0, 255).astype(_np.uint8),
                            arr)
            lum = arr.mean(axis=2)

    ink_bool = lum < 245
    ink = ink_bool.astype(_np.uint8) * 255
    ink_img = Image.fromarray(ink, 'L')

    white_mask = ink_img.filter(ImageFilter.MaxFilter(white_k))
    gray_mask  = ink_img.filter(ImageFilter.MaxFilter(gray_k))
    gray_ring_arr = _np.array(gray_mask, dtype=_np.int16) - _np.array(white_mask, dtype=_np.int16)
    gray_ring = Image.fromarray(_np.clip(gray_ring_arr, 0, 255).astype(_np.uint8), 'L')
    if far_blur > 0:
        gray_ring = gray_ring.filter(ImageFilter.GaussianBlur(far_blur))
...
_CURL_LINE_BANDS = {
    # line_id → (rl_y_bot, rl_y_top)  in the 1024×1024 RL drawing space
    2: (613.5, 742.0),   # bottom ornament
    3: (393.5, 509.5),   # top ornament
}


def _load_curl_drawing_raw():
    """Load SVG as a fresh reportlab Drawing (unmodified colors/widths)."""
    if not os.path.exists(_CURL_SVG_PATH):
        return None
    return svg2rlg(_CURL_SVG_PATH)


def _curl_gray_shapes(node, gray=0.0, stroke_scale=1.0):
    """Recursively recolor dark fills/strokes to a given gray; scale strokeWidths."""
    from reportlab.lib.colors import Color
    gray_c = Color(gray, gray, gray)
    _dark_thresh = 0.85
    def _is_dark(col):
        if col is None:
            return False
        try:
            return (col.red + col.green + col.blue) / 3.0 < _dark_thresh
        except Exception:
            return False
    def _recurse(n):
        for attr in ('fillColor', 'strokeColor'):
            v = getattr(n, attr, None)
            if v is not None and _is_dark(v):
                setattr(n, attr, gray_c)
        sw = getattr(n, 'strokeWidth', None)
        if sw is not None and sw > 0:
            n.strokeWidth = sw * stroke_scale
        children = getattr(n, 'contents', None)
        if children:
...
def _rasterize_drawing_to_pil(drawing, dpi):
    """Render a reportlab Drawing to a PIL RGB image at given DPI via pymupdf.

    drawing.width / drawing.height are in points (1/72 inch).
    Returns PIL Image at (drawing.width * dpi/72) × (drawing.height * dpi/72) px.
    """
    import io, pymupdf
    from reportlab.pdfgen import canvas as _rl_canvas
    buf = io.BytesIO()
    pc = _rl_canvas.Canvas(buf, pagesize=(drawing.width, drawing.height))
    renderPDF.draw(drawing, pc, 0, 0)
    pc.showPage(); pc.save()
...
def _get_curl_band_pil(line_id, band_w_px, stroke_scale=1.0, ink_dilate_px=0):
    """Rasterize one curl ornament band to a PIL 'L' image.

    Output width = band_w_px, height = band_w_px * (band_rl_h / 1024) (aspect-locked).
    Black ink on white background.

    ink_dilate_px: thicken the rasterized ink by this many pixels (per side) via
    morphological dilation (PIL MinFilter). Necessary because the curl SVG is
    composed of filled paths (no strokes), so strokeWidth scaling is a no-op.
    """
    cache_key = (line_id, band_w_px, round(stroke_scale, 3), ink_dilate_px)
    cached = _CURL_BAND_PIL_CACHE.get(cache_key)
    if cached is not None:
        return cached
    band = _CURL_LINE_BANDS.get(line_id)
    if band is None:
        return None
    rl_y_bot, rl_y_top = band
    band_rl_h = rl_y_top - rl_y_bot
    band_h_px = max(1, round(band_w_px * band_rl_h / 1024.0))

    # Build a fresh drawing scaled so 1024 RL units → band_w_px points (we render at 72dpi
    # then upscale via dpi). Easier: render the full SVG at the right dpi to give us a
    # square of band_w_px, then crop the band rows.
    drawing = _load_curl_drawing_raw()
    if drawing is None:
        return None
    if stroke_scale != 1.0:
        _curl_gray_shapes(drawing, gray=0.0, stroke_scale=stroke_scale)
    # drawing.width / .height are in points (1024 each). We want band_w_px output width.
    dpi = band_w_px / drawing.width * 72.0
    full = _rasterize_drawing_to_pil(drawing, dpi=dpi)  # band_w_px × band_w_px square
    # Crop the band: RL y_bot..y_top maps to PIL rows from bottom.
    full_h = full.height
    px_per_rl = full_h / 1024.0
    pil_y_bot = full_h - int(round(rl_y_bot * px_per_rl))   # bottom edge → larger PIL y
    pil_y_top = full_h - int(round(rl_y_top * px_per_rl))   # top edge → smaller PIL y
    crop = full.crop((0, pil_y_top, full.width, pil_y_bot))
    if crop.height != band_h_px:
        crop = crop.resize((band_w_px, band_h_px), Image.LANCZOS)
    crop = crop.convert('L')
    if ink_dilate_px > 0:
        # MinFilter with kernel k dilates dark content by (k-1)/2 pixels per side.
        k = 2 * ink_dilate_px + 1
        crop = crop.filter(ImageFilter.MinFilter(k))
    _CURL_BAND_PIL_CACHE[cache_key] = crop
    return crop


_BAND_FLAT_ROW_CACHE = {}


def _find_band_flat_row(band_pil, ink_thresh=128):
    """Return the PIL row index (0 = top) with the longest contiguous run of
    ink pixels. This row is the "flat baseline" of a curl ornament that should
    align flush with the box line."""
    import numpy as np
    key = (id(band_pil), band_pil.size)
    cached = _BAND_FLAT_ROW_CACHE.get(key)
    if cached is not None:
        return cached
    arr = np.array(band_pil.convert('L'))
    ink = arr < ink_thresh
    best_row, best_len = 0, 0
    H, W = ink.shape
    for y in range(H):
        row = ink[y]
        if not row.any():
            continue
        # vectorized longest run of True
        ...
        if mx > best_len:
            best_len = mx
            best_row = y
    _BAND_FLAT_ROW_CACHE[key] = best_row
    return best_row


def _find_band_base_row_bottom_up(band_pil, ink_thresh=128, min_width_frac=0.4):
    """Return the BOTTOM-MOST PIL row with a horizontal ink run longer than
    min_width_frac * band_width. This is the 'bottom line' of the ornament
    that the user wants flush with the box edge."""
    import numpy as np
    arr = np.array(band_pil.convert('L'))
    ink = arr < ink_thresh
    H, W = ink.shape
    min_run = max(int(W * min_width_frac), 8)
...
# ── Generic halo (white ring + soft gray glow) ───────────────────────────────
def apply_halo_glow(canvas_pil, near_dil=8, far_blur=20,
                    white_val=252, gray_val=100, halo_boost=1.0,
                    ink_threshold=None, halo_style='feathered',
                    gray_band_px=None):
    """Apply white ring + gray halo around any dark shape.

    Pixel priority: ink (untouched) > white ring (crisp) > gray halo.

    halo_style:
      'feathered'  — 4 concentric rings fading from gray_val → white (default,
                     used for post-anaf divider and other "glow" contexts).
      'outline'    — single thick saturated gray band after the white ring,
                     with just a tiny smoothing blur. Looks like a clean 2nd
                     outline. Use gray_band_px for its thickness (per side).
    """
    import numpy as np
    ...
    arr_l = np.array(canvas_pil.convert('L'))
    ink   = arr_l < ink_threshold

    near_k    = near_dil if near_dil % 2 == 1 else near_dil + 1
    ink_pil   = Image.fromarray((ink * 255).astype(np.uint8), 'L')
    near_zone = np.array(ink_pil.filter(ImageFilter.MaxFilter(near_k))) > 128

    if halo_style == 'outline':
        # Single thick gray band — like a second outline around the white ring.
        if gray_band_px is None:
            gray_band_px = max(4, near_dil // 2)
        outer_k = 2 * gray_band_px + 1
        outer_kernel = near_k + outer_k  # total dilation = white + gray band
        if outer_kernel % 2 == 0:
            outer_kernel += 1
        outer_zone = np.array(ink_pil.filter(ImageFilter.MaxFilter(outer_kernel))) > 128
        gray_layer = np.full_like(arr_l, 255, dtype=np.uint8)
        gray_layer[outer_zone & ~near_zone & ~ink] = gray_val
        # Tiny smoothing so the gray band edge isn't jagged
        gray_pil = Image.fromarray(gray_layer, 'L').filter(ImageFilter.GaussianBlur(radius=1.2))
        gv_arr   = np.array(gray_pil)
    else:
        # ── Multi-ring feathered halo (simulates _draw_title_with_glow) ──────
        band_w = max(6, near_dil + 2)
        rings = [
            (near_dil + band_w,         gray_val),                        # solid dark gray line
            (near_dil + band_w + 10,    min(250, gray_val + 45)),         # medium
            (near_dil + band_w + 22,    min(252, gray_val + 85)),         # light
            (near_dil + band_w + max(34, far_blur), min(253, gray_val + 120)),  # feathered
        ]
        gray_layer = np.full_like(arr_l, 255, dtype=np.uint8)
        prev_zone  = near_zone | ink
        for k, gv in rings:
            k_odd = k if k % 2 == 1 else k + 1
            zone = np.array(ink_pil.filter(ImageFilter.MaxFilter(k_odd))) > 128
            new_zone = zone & ~prev_zone
            gray_layer[new_zone] = gv
            prev_zone = zone
        smooth_r = max(1.0, far_blur * 0.18)
        gray_pil = Image.fromarray(gray_layer, 'L').filter(ImageFilter.GaussianBlur(radius=smooth_r))
        gv_arr   = np.array(gray_pil)

    if halo_boost != 1.0 and halo_boost > 0:
        norm = gv_arr.astype(np.float32) / 255.0
        norm = np.power(norm, halo_boost)
        gv_arr = (norm * 255.0).clip(0, 255).astype(np.uint8)

    out = np.array(canvas_pil)
    cur_val   = out[..., 0]
    white_mask = near_zone & ~ink
    gray_mask  = (~near_zone) & (~ink) & (gv_arr < cur_val) & (gv_arr < 250)

    out[white_mask] = (white_val, white_val, white_val)
    out[gray_mask]  = np.stack([gv_arr, gv_arr, gv_arr], axis=-1)[gray_mask]
    return Image.fromarray(out, 'RGB')


# ── Mamar box renderer (rounded-rect + 2 curl ornaments + halo) ──────────────
def render_mamar_box_image(width_pt, height_pt, dpi=600,
                            line_w_pt=2.4, orn_h_pt=20.0,
                            halo_pad_pt=0.0,
                            near_dil=None, far_blur=None,
                            white_val=252, gray_val=95,
                            variant=None):
    """Build the mamar/TOC box from the GPT-generated frame image using tiled
    straight runs with fixed-aspect center ornaments.

    orn_h_pt: height of the top (and bottom) ornament in points. Controls how
              large the decorative scrollwork is; everything else scales from this.
    variant:  'default' or 'alt' (see _mb_variant_source). Falls back to the
              module-level _MAMAR_BOX_VARIANT env toggle.

    Returns (ImageReader, halo_pad_pt). The image occupies exactly width_pt × height_pt.
    """
    variant = (variant or _MAMAR_BOX_VARIANT or 'default').lower()

    # Compute halo padding (in pt) based on settings so the gray ring isn't
    # clipped at the image edge. The builder embeds the tiled box on a
    # canvas that is pad_px larger on every side.
    if getattr(S, 'MAMAR_BOX_HALO', False):
        _near = int(getattr(S, 'MAMAR_BOX_HALO_NEAR_DIL', 22))
        _ring = int(getattr(S, 'MAMAR_BOX_HALO_RING_DIL', _near * 2))
        _far  = int(getattr(S, 'MAMAR_BOX_HALO_FAR_BLUR', 18))
        halo_pad_pt = max(halo_pad_pt, (_near + _ring + (3 * _far) + 8) * 72.0 / dpi)
    halo_pad_px = int(round(halo_pad_pt * dpi / 72.0))

    cache_key = (round(width_pt, 2), round(height_pt, 2), dpi,
                 round(orn_h_pt, 2), near_dil, far_blur, white_val, gray_val,
                 variant, halo_pad_px)
    cached = _MAMAR_BOX_CACHE.get(cache_key)
    if cached is not None:
        return cached
    w_px      = max(1, round(width_pt  * dpi / 72.0))
    h_px      = max(1, round(height_pt * dpi / 72.0))
    orn_h_px  = max(20, round(orn_h_pt * dpi / 72.0))
    assembled = _build_tiled_mamar_box_image(w_px, h_px, orn_h_px=orn_h_px,
                                             variant=variant,
                                             halo_pad_px=halo_pad_px)
    if assembled is None:
        return None, halo_pad_pt

    reader = ImageReader(assembled)
    _MAMAR_BOX_CACHE[cache_key] = (reader, halo_pad_pt)
    return reader, halo_pad_pt


def draw_mamar_box(c, x_left, y_bot, width_pt, height_pt, **kwargs):
    """Draw the ornament box on the PDF canvas at (x_left, y_bot) with size
    (width_pt × height_pt). Interior white pixels are masked transparent so
    text drawn afterwards shows through.

    If halo is enabled, the rendered image includes a padding ring around
    the logical box so the halo can fade out naturally.  We expand the
    draw rect by pad_pt on every side so the box content still occupies
    exactly width_pt × height_pt at the user-requested position.
    """
    kwargs.setdefault('variant', _MAMAR_BOX_VARIANT)
    reader, pad_pt = render_mamar_box_image(width_pt, height_pt, **kwargs)
    c.saveState()
    c.drawImage(reader,
                x_left - pad_pt, y_bot - pad_pt,
                width=width_pt + 2*pad_pt, height=height_pt + 2*pad_pt,
                mask=[254, 255, 254, 255, 254, 255])
    c.restoreState()


# ── Legacy helper kept as a no-op shim (other code may still call it) ────────
def _draw_curl_orn_line(c, line_id, x_center, y_center_page, target_w, target_h):
    """Deprecated — use render_mamar_box_image / draw_mamar_box."""
    return False


def _get_curl_drawing():
    return _load_curl_drawing_raw()


def _get_curl_drawing_pass(_pass_id):
    return _load_curl_drawing_raw()
...
def _draw_anaf_flanker(c, x_left, x_right, y_center, side='right'):
    """Small flanker = the owner's bottom-divider end curl WITH its own line
    continuation (the line that comes out of the curl), already faded at the
    far end in the source PNG. NOT a separately-drawn rule.

    The PNG (`anaf_flanker_curl.png`) is curl-on-left + line-running-right that
    fades out. It is drawn at its NATURAL aspect ratio (never stretched) at a
    small height, placed at the OUTER end of the slot with the curl outward and
    the faded line pointing toward the label. Much smaller than the label.
    """
    render_h = float(getattr(S, 'ANAF_FLANKER_H', 8.0))   # small; settings-driven
    slot = x_right - x_left
    if slot < 4 or not os.path.exists(_PNG_ANAF_FLANKER_CURL):
        return
    cache_key = ('img', round(render_h, 2), side)
    info = _ANAF_FLANKER_CACHE.get(cache_key)
    if info is None:
        try:
            img = Image.open(_PNG_ANAF_FLANKER_CURL).convert('RGBA')
            bbox = img.getbbox()
            if bbox:
                img = img.crop(bbox)
            # Owner: the CURL sits NEXT TO THE TEXT (inner), the faded line runs
            # OUTWARD. Right flanker (right of label): inner = LEFT → curl-left
            # = PNG as-is. Left flanker: inner = RIGHT → mirror so curl-right.
            if side == 'left':
                img = img.transpose(Image.FLIP_LEFT_RIGHT)
            flat = Image.new('RGB', img.size, (255, 255, 255))
            flat.paste(img, mask=img.split()[3])
            info = (ImageReader(flat), img.width / float(img.height))
            _ANAF_FLANKER_CACHE[cache_key] = info
        except Exception:
            return
    reader, aspect = info
    draw_w = render_h * aspect           # natural aspect — never stretched
    if draw_w > slot:                    # never exceed the reserved slot
        draw_w = slot
    # Anchor so the curl sits at the INNER (label-facing) end of the slot.
    # Right flanker: curl-left → anchor at x_left. Left flanker: curl-right →
    # anchor so the curl reaches x_right.
    x = x_left if side == 'right' else (x_right - draw_w)
    c.saveState()
    c.drawImage(reader, x, y_center - render_h / 2,
                width=draw_w, height=render_h, mask=[254, 255, 254, 255, 254, 255])
    c.restoreState()
...
# Heading spacing
HEADING_AFTER_GAP = LH * 1.5
HEADING_BEFORE_GAP = LH * 0.5
H1_TOP_GAP = LH * 1.0            # modest top gap above anaf heading
...
DC_FONT = S.H2_FONT_NAME          # first word uses the same medium Tehila as subheadings
...
def _draw_header_band_leader(c, y, x_left, x_right, curl_side='right'):
    """Header leader using the curly-line ornament, tiled via one-pixel method.
    y is the text baseline; bottom of rule flush with baseline (page-num side).
    curl_side: 'right' = curl at right end, 'left' = curl at left end."""
    if x_right <= x_left + 6:
        return
    render_h = 9.0
    ppi = 300
    ...
    band_h_px   = max(4, round(render_h * ppi / 72))
    cache_key   = (needed_w_px, band_h_px, curl_side)
    reader = _HEADER_BAND_LEADER_CACHE.get(cache_key)
    if reader is None:
        if not os.path.exists(_PNG_HEADER_LEADER_RIGHT):
            return
        try:
            img = Image.open(_PNG_HEADER_LEADER_RIGHT).convert("RGBA")
            bbox = img.getbbox()
            ...
            if tile is None:
                return
            tiled = _build_tiled_leader(tile, flourish, needed_w_px, band_h_px)
            if curl_side == 'left':
...
def _draw_title_with_glow(c, x, y, text, font, size,
    ...
    """Draw title with smooth 16-point ring halo: gray outer, white inner, text on top.

    Uses the same technique as the v1 palot production build for a smooth,
    non-faceted halo at any font size.
    """
    ring_dirs = (
    ...
    gray_core  = glow_color[0]
    gray_mid   = gray_core + (1.0 - gray_core) * 0.35
    gray_outer = gray_core + (1.0 - gray_core) * 0.65

    c.saveState()
    c.setFont(font, size)
    _draw_ring(gray_r3, gray_outer)
    _draw_ring(gray_r2, gray_mid)
    _draw_ring(gray_r1, gray_core)
    _draw_ring(gray_r0, gray_core)
    _draw_ring(white_r, 1.0)
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    """
    header_y = C_TOP - 2
    rule_y = header_y + 3.5  # band center midway between baseline and cap-height
    ...
    if band_end > band_start + 8:
        _draw_header_band_leader(c, rule_y, x_left=band_start, x_right=band_end, curl_side='left')
...
def draw_running_header_even(c, page_num, anaf_label, anaf_name):
    ...
    # Leader: curly ornament, band center midway between baseline and cap-height.
    rule_y = header_y + 3.5
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Frame sizing — ornament is ~20pt (≈1-2 lines); box fits the text
    tw           = Wid(title_text, HEADING_BOLD_FONT, TOC_TITLE_FS)
    frame_pad_x  = 22.0
    # Alt frame ornament renders at ~28pt with BOOST=0.55 (32pt gives breathing room)
    orn_h_pt     = _mamar_orn_h_pt()
    frame_pad_y  = 4.0   # inner padding top & bottom inside box rectangle
    title_frame_w = tw + frame_pad_x * 2
    # Height = ornaments top+bot + box rect (text + inner padding)
    title_frame_h = 2 * orn_h_pt + TOC_TITLE_FS + frame_pad_y * 2
    ...
    # Draw box first (interior masked transparent), then text on top
    draw_mamar_box(c, title_frame_left, title_frame_bot, title_frame_w, title_frame_h,
                   orn_h_pt=orn_h_pt)

    # ── Center text within the box RECTANGLE (between top and bottom ornaments) ──
    BODY_CENTRE_ABOVE_BL = 0.2475
    # The box rect center is at: title_frame_bot + orn_h_pt + (box_rect_h)/2
    box_rect_h = TOC_TITLE_FS + frame_pad_y * 2
    box_rect_cy = title_frame_bot + orn_h_pt + box_rect_h / 2
    title_y  = box_rect_cy - BODY_CENTRE_ABOVE_BL * TOC_TITLE_FS
    ...
    if MAMAR_NAME:
        ...
        draw_generated_subhead_rule(c, mamar_cx, top_rule_y, mamar_rule_w,
                                    mirror=False, flip_vertical=False, intensity=0.72)
        ...
        draw_generated_subhead_rule(c, mamar_cx, bot_rule_y, mamar_rule_w,
                                    mirror=True, flip_vertical=True, intensity=0.72)
    ...
    name_right_x = label_right_x - label_col_w - 2
    ...
    # Pre-compute fixed dot_end based on the widest FIRST LINE of any name
    # (multi-line names must not use full string width — that would kill all dots)
    def _first_line_w(name_str_v):
        words = name_str_v.split(' ')
        space_w = Wid(' ', HEADING_BOLD_FONT, TOC_ENTRY_FS)
        line_w = 0.0
        for word in words:
            ww = Wid(word, HEADING_BOLD_FONT, TOC_ENTRY_FS)
            add_w = ww if line_w == 0 else line_w + space_w + ww
            if add_w <= name_avail_w or line_w == 0:
                line_w = add_w
            else:
                break
        return line_w
    _all_first_nw = [_first_line_w(vis(livorna_fix_quotes(strip_nikud(nm))))
                     for _, nm, _ in anaf_entries]
    # Common dot start = just right of the widest page number (same x for all rows)
    _all_pw = [sum(Wid(ch, HEADING_FONT, TOC_ENTRY_FS) for ch in heb_page(pg))
               for _, _, pg in anaf_entries]
    _max_pw = max(_all_pw) if _all_pw else 0
    common_dot_start = pgnum_x + _max_pw + dot_w * 2
    ...
    for label_str, name_str, pg in anaf_entries:
        ...
        if nw <= name_avail_w:
            ...
            dot_line_w = nw     # dots run beside this (only) line
        else:
            ...
            dot_line_w = Wid(lines[-1], HEADING_BOLD_FONT, TOC_ENTRY_FS)  # last line width for dots
        ...
        # Dot leaders: fixed start (just right of widest page number) → just left of this row's dot line
        dot_start = common_dot_start
        dot_end   = name_right_x - dot_line_w - dot_w * 2
        dot_x = dot_start
...
def draw_precomputed_lines(c, line_descs, x_right, y_top, is_last_centered=True,
    ...
    for i, ld in enumerate(line_descs):
        ...
        if ld['type'] == 'subhead':
            ...
            text_y = y  # no rule padding — draw text directly at current y
            if tw <= col_w:
...
def _anaf_label_fits(label_vis, fs):
    """Returns True if the label + minimum flankers fit within C_W."""
    lw = Wid(label_vis, HEADING_BOLD_FONT, fs)
    # Need room for: label + 2*gap + 2*min_flanker (18pt each)
    min_total = lw + 2 * S.H1_ORNAMENT_GAP + 2 * 18.0
    return min_total <= C_W

def _anaf_label_fit_size(label_vis):
    """Shrink label font size until it fits within content width with flankers.
    Returns adjusted font size (never larger than ANAF_FS, never smaller than 70% of it)."""
    fs = ANAF_FS
    min_fs = ANAF_FS * 0.70
    while fs > min_fs and not _anaf_label_fits(label_vis, fs):
        fs -= 0.5
    return fs

def _anaf_name_lines(name_clean, fs):
    """Wrap anaf name to fit within C_W. Returns list of word lists."""
    words = name_clean.split()
    # Use balanced wrap so multi-line name looks symmetrical
    return wrap_words_balanced(words, C_W, HEADING_BOLD_FONT, fs)

def _anaf_name_fit_size(name_clean):
    """Adjust name font size so it fits on 1-2 lines within C_W.
    Prefer 1 line; allow shrink to 80% of ANAF_NAME_FS before wrapping to 2 lines."""
    fs = ANAF_NAME_FS
    min_fs_single = ANAF_NAME_FS * 0.80
    # Try to fit on single line by shrinking down to 80%
    while fs >= min_fs_single:
        if Wid(vis(name_clean), HEADING_BOLD_FONT, fs) <= C_W:
            return fs, [name_clean.split()]
        fs -= 0.5
    # Shrink failed — use balanced wrap at full size
    fs = ANAF_NAME_FS
    lines = _anaf_name_lines(name_clean, fs)
    return fs, lines

def draw_anaf_heading(c, label, name, y):
    """Draw ענף heading: Line 1 = flanker + label + flanker, line 2+ = name.
    Label is auto-shrunk if needed to leave room for flankers within C_W.
    Name is shrunk or wrapped to stay within C_W (never overflows margins)."""
    ...
    # Auto-shrink label if it would overflow content width with min flankers
    label_fs = _anaf_label_fit_size(label_vis)
    lw1 = Wid(label_vis, HEADING_BOLD_FONT, label_fs)

    # Each flanker spans from content edge to gap before label
    flanker_w = (C_W * 0.325 - lw1) / 2 - gap
    flanker_w = max(flanker_w, 18.0)

    label_right = x_center + lw1 / 2
    label_left  = x_center - lw1 / 2

    right_x_left  = label_right + gap
    right_x_right = min(right_x_left + flanker_w, C_RIGHT)
    left_x_right  = label_left - gap
    left_x_left   = max(left_x_right - flanker_w, C_LEFT)

    flanker_y = y + label_fs * 0.28

    _draw_anaf_flanker(c, right_x_left, right_x_right, flanker_y, side='right')
    _draw_anaf_flanker(c, left_x_left,  left_x_right,  flanker_y, side='left')

    c.setFont(HEADING_BOLD_FONT, label_fs)
    c.setFillColorRGB(0, 0, 0)
    ...
    if name:
        ...
        name_fs, name_line_groups = _anaf_name_fit_size(name_clean)
        name_y = y - label_fs - INTER
        c.setFont(HEADING_BOLD_FONT, name_fs)
        c.setFillColorRGB(0, 0, 0)
        line_step = name_fs * 1.18
        for i, words in enumerate(name_line_groups):
            line_text = ' '.join(words)
            line_vis = vis(line_text)
            tw = Wid(line_vis, HEADING_BOLD_FONT, name_fs)
            cy = name_y - i * line_step
            c.drawString(x_center - tw / 2, cy, line_vis)
        total_h += INTER + name_fs + (len(name_line_groups) - 1) * line_step
    return total_h

def anaf_heading_height(has_name=True, label='', name=''):
    """Compute anaf heading height accounting for wrap/shrink."""
    INTER = ANAF_FS * 0.8
    if not has_name:
        # Label may shrink but height is still ANAF_FS-ish; use ANAF_FS as upper bound
        return ANAF_FS
    # Account for possible name wrap
    if name:
        name_clean = livorna_fix_quotes(strip_nikud(name))
        name_fs, name_line_groups = _anaf_name_fit_size(name_clean)
        n_lines = len(name_line_groups)
        line_step = name_fs * 1.18
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # Alt frame ornament renders at ~28pt with BOOST=0.55 (32pt gives breathing room)
        orn_h_pt    = _mamar_orn_h_pt()
        frame_pad_y = 6.0   # inner padding top & bottom inside box rectangle

        frame_bw = max_lw + frame_pad_x * 2
        # Total height = top ornament + box rect (text + padding) + bottom ornament
        box_rect_h = total_h + frame_pad_y * 2
        frame_bh   = 2 * orn_h_pt + box_rect_h
        ...
        # ── Draw box first (interior is flood-filled white → masked transparent) ──
        draw_mamar_box(c, frame_left, frame_bot, frame_bw, frame_bh, orn_h_pt=orn_h_pt)

        # ── Text centered in the box RECTANGLE (between the two ornament bands) ──
        BODY_CENTRE_ABOVE_BL = 0.2475  # fraction of fs
        box_rect_cy  = frame_bot + orn_h_pt + box_rect_h / 2
        first_baseline = box_rect_cy - BODY_CENTRE_ABOVE_BL * fs + (n_lines - 1) * fs * 1.18 / 2
...
def heading_height(text, kind='section', has_name=True, label='', name=''):
    if kind == 'anaf':
        return H1_TOP_GAP + anaf_heading_height(has_name=has_name, label=label, name=name) + HEADING_AFTER_GAP
        ...
        # Must exactly match draw_section_header sefer_title path
        frame_pad_x = 30.0
        orn_h_pt    = _mamar_orn_h_pt()
        frame_pad_y = 6.0
        max_lw = max(Wid(vis(' '.join(lw)), font, fs) for lw in lines)
        frame_bw = max_lw + frame_pad_x * 2
        frame_bh = 2 * orn_h_pt + text_h + frame_pad_y * 2
        return frame_bh + HEADING_AFTER_GAP + LH * 0.5
    elif kind == 'anaf_sub':
        fs = SEC_FS
        font = HEADING_BOLD_FONT
        text_clean = strip_nikud(text)
...
def _inter_frag_gap(para_run, cd, ci, es):
    ...
    if _is_subhead_ending_frag(para_run, cur_kind, cur_pi, cur_k):
        ...
        #
        # RULE (PERMANENT): SUB_AFTER must always stay visibly smaller than
        # SUB_BEFORE — the subheading must always appear coupled to the
        # paragraph below, regardless of whether subheading is 1 or N lines.
        # Cap SUB_AFTER at 80% of SUB_BEFORE_FRAG_GAP.
        sub_after_ceiling = min(MAX_SUB_AFTER_GAP, SUB_BEFORE_FRAG_GAP * 0.8)
        if _force_sub_after_stretchable:
            return max(MIN_SUB_AFTER_GAP, min(sub_after_ceiling, SUB_AFTER_GAP + es))
        return min(sub_after_ceiling, SUB_AFTER_FRAG_GAP)
    ...
    def _render_page(self, elements, el_idx, line_off, fn_off, carry_fn_lds):
        ...
        def flush_acc(force_height=None, allow_trailing_subhead=False, max_height=None):
            ...
            if force_height is not None:
                ...
                _allow_subhead_top_pad = any(b.get('kind') == 'sefer_title' for b in plan)
        ...
        while cur_el_idx < len(elements):
            ...
            if el['kind'] == 'anaf':
                ...
                flush_acc()
                page_empty = False
                sec_h = heading_height(el['label'], 'anaf', has_name=bool(el.get('name', '')), label=el['label'], name=el.get('name', ''))
            ...
            # ── Skip ornament if no room ──
            # If the paragraph fits without the ornament but not with it,
            # accept the paragraph anyway — the render pass will skip the
            # ornament when there is insufficient vertical room.
            _orn_h_here = _orn_h_after(cur_el_idx + 1)
            _need_no_orn = used_h + trial_h + sep
            _fits_with_orn = _total_need <= _avail_now + 0.01
            _fits_without_orn = _need_no_orn <= _avail_now + 0.01
            if not _fits_with_orn and _fits_without_orn and _orn_h_here > 0:
                _trace.log('ornament_skip',
                           f'el[{cur_el_idx}] para fits without ornament '
                           f'(need={_need_no_orn:.1f} orn={_orn_h_here:.1f} '
                           f'avail={_avail_now:.1f}) -> skip ornament',
                           el_idx=cur_el_idx, orn_h=round(_orn_h_here, 2))
                _total_need = _need_no_orn  # pretend ornament isn't there
            ...
            for nn in range(nL - MIN_TAIL_LINES, MIN_HEAD_LINES - 1, -1):
                ...
                _trace.log('split_try',
                           ...
                           fits=_split_fits)
                if _split_fits:
                    _raw_diff = abs(_tt_h1 - _tt_h2)
                    _gap_load = abs(te1) + abs(te2)
                    _score = (float(_used_height_constraint), _raw_diff, _gap_load, -nn)
                    _trace.log('split_candidate',
                               f'nn={nn} score={_score} current_best={best_split_score}',
                               nn=nn, raw_diff=round(_raw_diff, 3), gap_load=round(_gap_load, 3),
                               constrained=_used_height_constraint)
                    if best_split_score is None or _score < best_split_score:
                        best_split_score = _score
                        best_n = nn
            ...
            if best_n > 0 and not _sub_retry_used:
                ...
                if len(_np_preview) >= 2:
                    ...
                    # Preview has < 2 paras — standard multi-para balance
                    # scoring won't work.  But we can still detect a bad
                    # split when the tail is a single paragraph that would
                    # span two sparse pages (e.g., 7 lines → 3+3 on next
                    # page + 1 orphan on the page after).
                    # If reducing best_n by 1 makes the tail even (better
                    # two-column balance, fewer pages), prefer that.
                    if len(_np_preview) == 1:
                        _tail_nL = _np_preview[0]['nlines']
                        # Compute column balance for current tail
                        _cur_c1, _cur_c2, _cur_e1, _cur_e2, _, _ = _col_layout(_np_preview)
                        _cur_tail_res = _col_layout_residual(
                            _np_preview, _cur_c1, _cur_c2, _cur_e1, _cur_e2)
                        # Try best_n - 1 (send one more line to next page)
                        if best_n - 1 >= MIN_PART_LINES:
                            _alt_preview = _build_next_page_preview(best_n - 1)
                            if len(_alt_preview) == 1:
                                _alt_nL = _alt_preview[0]['nlines']
                                _alt_c1, _alt_c2, _alt_e1, _alt_e2, _, _ = _col_layout(_alt_preview)
                                _alt_tail_res = _col_layout_residual(
                                    _alt_preview, _alt_c1, _alt_c2, _alt_e1, _alt_e2)
                                # Also verify current page still fits with best_n - 1
                                _la_vnl2 = sum(ld.get('nlines', 1)
                                               for ld in remaining_lines[:best_n - 1])
                                _la_para2 = {'lines': remaining_lines[:best_n - 1],
                                             'nlines': _la_vnl2}
                                if el.get('is_subhead'):
                                    _la_para2['is_subhead'] = True
                                _la_acc2 = acc_paras + [_la_para2]
                                _la_cc1_2, _la_cc2_2, _la_ce1_2, _la_ce2_2, _, _ = \
                                    _col_layout(_la_acc2)
                                _la_cur_h2 = _col_height(
                                    _la_acc2, _la_cc1_2, _la_cc2_2,
                                    _la_ce1_2, _la_ce2_2)
                                _la_fits = (_la_cur_h2 + used_h + sep
                                            <= body_avail(len(acc_fn_lds)) + 0.01)
                                if _la_fits and _alt_tail_res < _cur_tail_res - 0.5:
                                    _trace.log('lookahead_single_para_reduce',
                                               f'best_n {best_n}->{best_n-1} '
                                               f'tail_nL {_tail_nL}->{_alt_nL} '
                                               f'tail_res {_cur_tail_res:.2f}->{_alt_tail_res:.2f}',
                                               old_n=best_n, new_n=best_n - 1,
                                               old_tail_res=round(_cur_tail_res, 3),
                                               new_tail_res=round(_alt_tail_res, 3))
                                    best_n -= 1
                                else:
                                    _trace.log('lookahead_single_para_no_improve',
                                               f'best_n-1 no improvement: fits={_la_fits} '
                                               f'tail_res {_cur_tail_res:.2f}->{_alt_tail_res:.2f}')
                        else:
                            _trace.log('lookahead_skip',
                                       f'preview has {len(_np_preview)} paras (< 2), '
                                       f'best_n-1 < MIN_PART, skipping')
                    else:
                        _trace.log('lookahead_skip',
        ...
        if (not is_final_flush
            ...
            if _bl_res > BLEED_THRESHOLD:
                ...
                for _bleed in range(1, _max_bleed + 1):
                    ...
                    _orig_el   = elements[_last_origin['el_idx']]
                    _orig_total = len(_orig_el.get('lines', []))
                    _next_pg_lines = _orig_total - (_last_origin['ln_off'] + _keep)
                    if _next_pg_lines < MIN_PART_LINES:
                        # Would create a <2-line orphan on the next page.
                        # Only allow if the next element is a body para that
                        # will provide enough content on that page.
                        _nxt_ei = _last_origin['el_idx'] + 1
                        _nxt_is_body = (_nxt_ei < len(elements)
                                        and elements[_nxt_ei]['kind'] == 'para'
                                        and not elements[_nxt_ei].get('is_subhead'))
                        if not _nxt_is_body:
                            _trace.log('bleed_orphan_skip',
                                       f'bleed={_bleed} would leave {_next_pg_lines} '
                                       f'orphan lines on next page, skipping',
                                       bleed=_bleed, orphan_lines=_next_pg_lines)
                            continue
        ...
        for blk in plan:
            if blk['kind'] in ('sefer_title', 'anaf', 'anaf_sub', 'section'):
                simulated_y -= blk.get('gap_above', 0.0)
        ...
        if slack > 0.5 and not skip_stretch:
            ...
            for blk in plan:
                if blk['kind'] in ('sefer_title', 'anaf', 'anaf_sub', 'section'):
                    simulated_y2 -= blk.get('gap_above', 0.0)
            ...
            if remaining_slack > 0.5:
                for blk in plan:
                    ...
                    for _round in range(3):
                        ...
                        _orig_lines = para_run[best_pi]['lines']
                        _orig_nlines = para_run[best_pi]['nlines']
                        ...
                        # Guard: reject reflow if it would overflow the page
                        if delta_h > remaining_slack + 0.5:
                            # Undo reflow — restore original lines
                            para_run[best_pi]['lines'] = _orig_lines
                            para_run[best_pi]['nlines'] = _orig_nlines
                            break
        ...
        for bi, block in enumerate(plan):
            ...
            if (next_el_is_anaf or final_block_of_book) and S.SECTION_END_ORNAMENT:
                # Only draw ornament if there is enough vertical room.
                # The visual band needs HSEP*1.4 (gap above) + ~39pt (band height).
                # The LH*2.2 gap below is allowed to intrude into the bottom margin.
                _orn_draw_need = HSEP * 1.4 + 39.0
                _orn_has_room  = (render_y - _orn_draw_need) >= C_BOT - _ORN_MARGIN_INTRUDE
                if _orn_has_room:
                    div_w = C_W * 0.72
                    render_y -= HSEP * 1.4
