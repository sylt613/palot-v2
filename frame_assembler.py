...
def _assemble_frame_svg(target_w_px, target_h_px):
    ...
    # ── Paste SVG's natural center motif over the center seam ──
    # The SVG has a diamond/star motif at its center (x=1023). Rendering it at
    # the same scale as the tiles and pasting it over the center hides any seam.
    _CENTER_SVG_W = 200  # SVG units to capture around center
    center_out_w = max(1, int(_CENTER_SVG_W * sc))
    center_motif = _render_svg_region(
        _SVG_CENTER_X - _CENTER_SVG_W // 2, _SVG_CONTENT_Y0,
        _CENTER_SVG_W, _SVG_CORNER_H, center_out_w, ch)
    cx = mid_w // 2 - center_out_w // 2
    top_strip.paste(center_motif, (cx, 0))
    ...
    # ── Paste SVG's natural center motif over the vertical center seam ──
    _CENTER_SVG_H = 200  # SVG units to capture around vertical center
    vcenter_out_h = max(1, int(_CENTER_SVG_H * sc))
    vcenter_motif = _render_svg_region(
        _SVG_CONTENT_X0, _SVG_CENTER_Y - _CENTER_SVG_H // 2,
        _SVG_CORNER_W, _CENTER_SVG_H, cw, vcenter_out_h)
    cy = mid_h // 2 - vcenter_out_h // 2
    lft_strip.paste(vcenter_motif, (0, cy))
