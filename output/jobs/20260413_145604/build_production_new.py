...
def draw_precomputed_lines(c, line_descs, x_right, y_top, is_last_centered=True,
    ...
    for i, ld in enumerate(line_descs):
        ...
        if ld['type'] == 'subhead':
            ...
            # Subheadings span the full content width, not just one column
            draw_w = C_W
            if tw <= draw_w:
                draw_x = x_right - (draw_w + tw) / 2
                c.setFont(S.H2_FONT_NAME, SUB_FS)
                c.setFillColorRGB(0, 0, 0)
                c.drawString(draw_x, y, text_vis)
            else:
                sub_lines = wrap_words(text_clean.split(), draw_w, S.H2_FONT_NAME, SUB_FS)
                c.setFont(S.H2_FONT_NAME, SUB_FS)
                c.setFillColorRGB(0, 0, 0)
                line_y = y
