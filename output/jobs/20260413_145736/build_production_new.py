...
def livorna_fix_quotes(text):
    """Normalise Hebrew quote characters for rendering.
    - Single quote / apostrophe → ׳ geresh (U+05F3) — raised slightly
    - Double quote → plain ASCII " — rendered at baseline like a normal double-quote
    - U+05F4 gershayim already in source → plain ASCII "
    Used for any text rendered in Frank Real / EFT fonts."""
    result = []
    for ch in text:
        if ch == '\u05F4':            # ״ gershayim in source → plain "
            result.append('"')
        elif ch == '"':               # ASCII double-quote → plain "
            result.append('"')
        elif ch == "'":               # ASCII apostrophe → ׳ geresh (will be raised)
            result.append('\u05F3')
        else:
            result.append(ch)
    return ''.join(result)

_GERESH_CHARS = '\u05F3'   # ׳ geresh only — gershayim now rendered as plain "
GERESH_RISE   = 2.5        # points to raise geresh above baseline

def draw_string_raised(c, x, y, text, font, size):
    """drawString that raises ׳ and ״ by GERESH_RISE points above the baseline.
    Falls back to a plain drawString when no geresh chars are present."""
    if not any(ch in _GERESH_CHARS for ch in text):
        c.drawString(x, y, text)
        return
    # Split into segments: normal segments and single geresh chars
    cx = x
    seg = []
    for ch in text:
        if ch in _GERESH_CHARS:
            if seg:
                seg_str = ''.join(seg)
                c.drawString(cx, y, seg_str)
                cx += c.stringWidth(seg_str, font, size)
                seg = []
            c.drawString(cx, y + GERESH_RISE, ch)
            cx += c.stringWidth(ch, font, size)
        else:
            seg.append(ch)
    if seg:
        c.drawString(cx, y, ''.join(seg))
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    # Leader: strong near text (right/inside), fades toward page number (left/outside)
...
def draw_precomputed_lines(c, line_descs, x_right, y_top, is_last_centered=True,
    ...
    for i, ld in enumerate(line_descs):
        ...
        if ld['type'] == 'subhead':
            ...
            # Subheadings are centered across the full content width (C_LEFT..C_RIGHT),
            # regardless of which column x_right refers to.
            cx_center = C_LEFT + C_W / 2
            if tw <= C_W:
                c.setFont(S.H2_FONT_NAME, SUB_FS)
                c.setFillColorRGB(0, 0, 0)
                c.drawString(cx_center - tw / 2, y, text_vis)
            else:
                sub_lines = wrap_words(text_clean.split(), C_W, S.H2_FONT_NAME, SUB_FS)
                c.setFont(S.H2_FONT_NAME, SUB_FS)
                c.setFillColorRGB(0, 0, 0)
                line_y = y
        ...
        for item in items_to_render:
            if item['type'] == 'sefer_title':
                ...
                if sub_tw <= C_W:
                    ...
                    sub_wrapped = wrap_words(text_clean.split(), C_W, S.H2_FONT_NAME, SUB_FS)
