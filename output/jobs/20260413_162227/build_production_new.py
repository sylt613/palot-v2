...
def livorna_fix_quotes(text):
    ...
    - U+05F4 gershayim → two geresh ׳׳ (U+05F3 U+05F3) — both raised uniformly
    - ASCII double-quote " → two geresh ׳׳ (same treatment)
    - Single quote / apostrophe → ׳ geresh (U+05F3) — raised slightly
    All quote marks rendered raised identically so geresh and gershayim look uniform."""
    result = []
    for ch in text:
        if ch == '\u05F4':            # ״ gershayim → two raised geresh
            result.append('\u05F3\u05F3')
        elif ch == '"':               # ASCII double-quote → two raised geresh
            result.append('\u05F3\u05F3')
        elif ch == "'":               # ASCII apostrophe → single raised geresh
            result.append('\u05F3')
        else:
            result.append(ch)
    return ''.join(result)

_GERESH_CHARS = '\u05F3'   # ׳ geresh — both single and double quotes use this
GERESH_RISE   = 2.5        # points to raise geresh above baseline

def draw_string_raised(c, x, y, text, font, size):
    """drawString that raises ׳ (geresh, U+05F3) by GERESH_RISE points above the baseline.
    Also applies livorna_fix_quotes so ״ gershayim in source always become two raised ׳.
