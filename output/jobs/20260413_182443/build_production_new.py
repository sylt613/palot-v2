...
def _get_nano_crop_reader(kind):
    ...
    try:
        ...
        if kind == 'header_leader':
            # Leader lives across the upper-middle band of the guided sheet.
            crop = img.crop((88, 356, 1328, 468))
        elif kind == 'anaf_flanker':
            crop = img.crop((204, 356, 420, 676))
        elif kind == 'toc_mark':
            crop = img.crop((930, 410, 1138, 620))
        else:
            return None
        bbox = crop.getbbox()
        if bbox:
...
def _draw_simple_printers_mark(c, x_center, y_center, size, gray=0.42):
    if draw_nano_crop(c, 'toc_mark', x_center, y_center, width=size * 2.25, preserve_aspect=True):
        return
    s = size / 2.0
    c.saveState()
...
def _draw_anaf_flanker(c, x_center, y_center, width, gray=0.38):
    if draw_nano_crop(c, 'anaf_flanker', x_center, y_center, width=width * 1.35, preserve_aspect=True):
        return
    h = width * 0.92
    top_y = y_center + h / 2
    bot_y = y_center - h / 2
...
def _draw_header_leader(c, y, x_left, x_right, fade_side='right', fade_zone=38):
    ...
    _segmented_line(y, 0.9,  0.75, 0.97,  y_off=0.0)
    _segmented_line(y, 0.35, 0.90, 0.99,  y_off=-0.5)
    _segmented_line(y, 0.18, 0.82, 0.98,  y_off=-0.9)
...
def _draw_header_leader_from_nano(c, y, x_left, x_right, mirror=False):
    if x_right <= x_left + 10:
        return False
    return draw_nano_crop(c, 'header_leader', (x_left + x_right) / 2, y + 0.8,
                          width=(x_right - x_left), preserve_aspect=False, mirror=mirror)
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    # Leader: reuse the guided nano-banana ornament, mirrored so the curl stays at the outer edge.
    rule_y = header_y - 1.35
    if not _draw_header_leader_from_nano(c, rule_y,
                                         x_left=C_LEFT + pg_w + 4,
                                         x_right=leader_stop_x,
                                         mirror=True):
        _draw_header_leader_curl(c, rule_y,
                                 x_left=C_LEFT + pg_w + 4,
                                 x_right=leader_stop_x)
...
def draw_running_header_even(c, page_num, anaf_label, anaf_name):
    ...
    # Leader: same guided nano-banana ornament, unmirrored on verso pages.
    rule_y = header_y - 1.2
    if not _draw_header_leader_from_nano(c, rule_y,
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    if MAMAR_NAME:
        ...
        if not draw_generated_mamar_frame(c, C_LEFT + C_W / 2, by + bh / 2, bw, bh):
            c.setFillColorRGB(0.975, 0.975, 0.975)
...
class PageLayout:
    ...
    def build(self):
        ...
        writer = PdfWriter()
        # Page 1: shaar blatt cropped to 5.5"x8.5"
