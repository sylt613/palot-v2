...
def _get_nano_crop_reader(kind):
    ...
    try:
        ...
        if kind == 'header_terminal':
            # Small terminal flourish taken from the approved nano-banana top ornament.
            crop = img.crop((1158, 16, 1338, 188))
...
def _draw_header_terminal_flourish(c, y, x_edge, side='right'):
    ...
    if side == 'left':
        ...
        x = x_edge - width
    # Anchor the bottom of the flourish slightly below the rule so the curl rises above it.
    c.drawImage(reader, x, y - height * 0.82, width=width, height=height, mask='auto')
    c.restoreState()
    return True
