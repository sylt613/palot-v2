...
def _get_nano_crop_reader(kind):
    ...
    try:
        ...
        if kind == 'header_leader':
            # Horizontal slice of the approved top ornament, used directly as the
            # running-header leader and mirrored for the opposite page side.
            crop = img.crop((110, 104, 1338, 242))
...
def _draw_header_leader_with_terminal(c, y, x_left, x_right, terminal_side='right'):
    if x_right <= x_left + 10:
        return
    if draw_nano_crop(c, 'header_leader', (x_left + x_right) / 2, y + 0.65,
                      width=(x_right - x_left), height=10.2,
