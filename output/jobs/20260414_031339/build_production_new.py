...
def _draw_header_leader_with_terminal(c, y, x_left, x_right, terminal_side='right'):
    ...
    if draw_png_ornament(c, _PNG_HEADER_LEADER_RIGHT, (x_left + x_right) / 2, y + 0.15,
                         width=span_w, height=6.4, preserve_aspect=False,
                         mirror=(terminal_side == 'left')):
        return
    term_w = 14.0
    reader = _get_nano_crop_reader('header_terminal')
    if reader is not None:
    ...
    if terminal_side == 'right':
        body_left = x_left
        body_right = max(x_left, x_right - term_w + body_overlap)
    else:
        body_left = min(x_right, x_left + term_w - body_overlap)
        body_right = x_right
    if body_right > body_left + 8:
        draw_nano_crop(c, 'header_leader', (body_left + body_right) / 2, y + 0.15,
                       width=(body_right - body_left), height=6.4,
