...
def _draw_header_leader_with_terminal(c, y, x_left, x_right, terminal_side='right'):
    ...
    if os.path.exists(_PNG_HEADER_LEADER_RIGHT):
        try:
            ...
            native_w = img.width * scale
            crop_w = min(native_w, x_right - x_left)
            crop_px_w = max(1, round(crop_w / scale))
            if terminal_side == 'right':
                crop = img.crop((img.width - crop_px_w, 0, img.width, img.height))
            else:
                crop = img.crop((0, 0, crop_px_w, img.height))
            reader = ImageReader(crop)
            c.saveState()
            if terminal_side == 'left':
                c.translate(x_left, 0)
                c.scale(-1, 1)
                draw_x = 0
            else:
                draw_x = x_right - crop_w
            c.drawImage(reader, draw_x, y - render_h * 0.52, width=crop_w, height=render_h, mask='auto')
            c.restoreState()
            return
        except Exception:
            pass
...
def draw_anaf_heading(c, label, name, y):
    ...
    # Compact engraved flankers
    _draw_anaf_flanker(c, sx - ow / 2, y + 5.2, orn_w * 0.72, gray=0.40)
    _draw_anaf_flanker(c, sx - ow - gap - lw1 - gap - ow / 2, y + 5.2, orn_w * 0.72, gray=0.40)
