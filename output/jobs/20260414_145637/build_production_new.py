...
def _build_tiled_leader(img, needed_w_px):
    """Build a full-width leader image by tiling the plain rule body and keeping the terminal flourish.

    img: RGBA PIL image of the leader (already auto-cropped), flourish on the RIGHT end.
    needed_w_px: desired final width in pixels.
    Returns a PIL RGBA image of exactly (needed_w_px, img.height).
    """
    h = img.height
    # Find where the flourish starts: scan columns from the right looking for
    # where the column alpha mass exceeds a "plain rule" threshold.
    # The plain rule section has very little vertical spread; the flourish has more.
    rule_thickness_threshold = h * 0.25  # if >25% of height has content, it's flourish territory
    flourish_start_x = img.width  # fallback: no flourish detected
    for x in range(img.width - 1, -1, -1):
        col = img.crop((x, 0, x + 1, h))
        col_data = list(col.getdata())
        opaque_count = sum(1 for _, _, _, a in col_data if a > 30)
        if opaque_count <= max(2, h * 0.08):
            # This column looks like just plain rule or empty — flourish ends here
            flourish_start_x = x + 1
            break
    # Safety: if the flourish is detected as >50% of the image, limit it
    if flourish_start_x < img.width * 0.4:
        flourish_start_x = int(img.width * 0.6)

    flourish = img.crop((flourish_start_x, 0, img.width, h))
    flourish_w = flourish.width

    # Extract a tileable rule segment from the middle of the plain section
    # Use a ~60px-wide strip from the middle of the rule body
    rule_end = max(1, flourish_start_x)
    tile_w = min(60, max(10, rule_end // 3))
    tile_start = max(0, rule_end // 2 - tile_w // 2)
    tile = img.crop((tile_start, 0, tile_start + tile_w, h))

    body_w = max(0, needed_w_px - flourish_w)

    canvas = Image.new("RGBA", (needed_w_px, h), (255, 255, 255, 0))
    # Tile the plain rule body
    for bx in range(0, body_w, tile_w):
        paste_w = min(tile_w, body_w - bx)
        if paste_w < tile_w:
            canvas.alpha_composite(tile.crop((0, 0, paste_w, h)), (bx, 0))
        else:
...
def _draw_header_leader_with_terminal(c, y, x_left, x_right, terminal_side='right'):
    ...
    if os.path.exists(_PNG_HEADER_LEADER_RIGHT):
        try:
            ...
            if bbox:
                img = img.crop(bbox)
            # Render height: the new asset is ~120px tall = 0.4" at 300dpi.
            # At 72dpi PDF points that's ~28.8pt. Use 14pt for a visible but not overpowering rule.
            render_h = 14.0
            span = x_right - x_left
            scale = render_h / img.height
            needed_w_px = max(1, round(span / scale))

            # Build tiled image: tile rule body + keep flourish
            tiled = _build_tiled_leader(img, needed_w_px)
            if terminal_side == 'left':
                tiled = tiled.transpose(Image.FLIP_LEFT_RIGHT)

            reader = ImageReader(tiled)
            c.saveState()
            c.drawImage(reader, x_left, y - render_h * 0.52, width=span, height=render_h, mask='auto')
            c.restoreState()
            return
