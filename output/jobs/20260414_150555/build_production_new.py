...
def _extract_flourish_terminal(img):
    ...
    """
    h = img.height
    # Scan from right to find where the flourish's thick vertical content begins
    # The flourish has many opaque pixels spread vertically; the plain rule has few.
    flourish_left = 0
    for x in range(img.width - 1, -1, -1):
        col_data = list(img.crop((x, 0, x + 1, h)).getdata())
        opaque = sum(1 for _, _, _, a in col_data if a > 30)
        if opaque <= max(1, h * 0.05):
            # Very sparse column — flourish content is to the right of here
            flourish_left = x + 1
            break
    if flourish_left >= img.width - 2:
        return None
    flourish = img.crop((flourish_left, 0, img.width, h))
    return flourish
...
def _build_hybrid_leader(flourish_img, needed_w_px, canvas_h):
    ...
    # Main rule (thickest, darkest)
    mid_y = canvas_h // 2
    gray_main = (70, 70, 70, 220)    # dark gray, ~0.27 rgb
    gray_thin = (120, 120, 120, 180)  # medium gray
    gray_hair = (150, 150, 150, 140)  # lighter gray
    
    # Main rule: 3px thick
    for dy in range(-1, 2):
        draw.line([(0, mid_y + dy), (body_w + 4, mid_y + dy)], fill=gray_main)
    # Upper thin accent line: 1px, offset -4px
    draw.line([(0, mid_y - 4), (body_w + 4, mid_y - 4)], fill=gray_thin)
    # Lower hairline: 1px, offset +3px
    draw.line([(0, mid_y + 3), (body_w + 4, mid_y + 3)], fill=gray_hair)
