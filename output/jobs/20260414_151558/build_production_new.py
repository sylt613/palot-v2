...
def _get_png_reader(png_path):
    reader = _PNG_READER_CACHE.get(png_path)
    if reader is None:
        ...
        try:
            ...
            return None
        _PNG_READER_CACHE[png_path] = reader
    return reader

def draw_png_ornament(c, png_path, x_center, y_center, width=None, height=None,
                     preserve_aspect=True, mirror=False):
    reader = _get_png_reader(png_path)
    if reader is None:
        return False
...
def _draw_anaf_flanker(c, x_center, y_center, width, gray=0.38):
    if draw_png_ornament(c, _PNG_ANAF_FLANKER, x_center, y_center, width=width, preserve_aspect=True):
        return
    h = width * 0.92
    top_y = y_center + h / 2
    bot_y = y_center - h / 2
...
def _extract_flourish_terminal(img):
    ...
    Returns: RGBA PIL image of just the flourish, or None.
    """
    h = img.height
    w = img.width
    alpha_band = list(img.split()[3].getdata())  # just the alpha channel
    
    # The plain rule uses ~14% of height; the flourish uses 20%+.
    # Use 18% as the split threshold.
    threshold = max(3, int(h * 0.18))
    
    # Scan from right to find the leftmost column that's still "just rule"
    flourish_left = 0
    for x in range(w - 1, -1, -1):
        ...
        if opaque <= threshold:
            flourish_left = x + 1
            break
    # If we couldn't find a clean split, take the rightmost 40%
    if flourish_left <= 2:
        flourish_left = int(w * 0.6)
    if flourish_left >= w - 2:
        return None
    return img.crop((flourish_left, 0, w, h))


def _build_hybrid_leader(flourish_img, needed_w_px, canvas_h):
    """Build a full-width leader: PIL-drawn rule body + AI flourish at the right end.
    
    flourish_img: RGBA PIL image of just the terminal flourish.
    ...
    fl_w = flourish_img.width if flourish_img else 0
    body_w = max(0, needed_w_px - fl_w)
    
    # Draw the rule body: triple parallel lines matching traditional sefer style
    # Scale line thickness relative to canvas height
    mid_y = canvas_h // 2
    dark = (55, 55, 55, 240)     # near-black main rule
    medium = (90, 90, 90, 200)   # medium gray accent
    light = (130, 130, 130, 160) # lighter gray hairline
    
    # Main rule: ~5px thick (visible at print size)
    main_half = max(2, canvas_h // 10)
    for dy in range(-main_half, main_half + 1):
        draw.line([(0, mid_y + dy), (body_w + 4, mid_y + dy)], fill=dark)
    # Upper accent line: 2px, offset above main rule
    accent_off = main_half + max(2, canvas_h // 12)
    for dy in range(2):
        draw.line([(0, mid_y - accent_off - dy), (body_w + 4, mid_y - accent_off - dy)], fill=medium)
    # Lower hairline: 1-2px, offset below main rule
    hair_off = main_half + max(2, canvas_h // 14)
    for dy in range(2):
        draw.line([(0, mid_y + hair_off + dy), (body_w + 4, mid_y + hair_off + dy)], fill=light)
    
    # Paste the AI flourish at the right end
    if flourish_img and fl_w > 0:
        # Resize flourish to match canvas height if needed
        if flourish_img.height != canvas_h:
            fl_scale = canvas_h / flourish_img.height
            new_fl_w = max(1, round(flourish_img.width * fl_scale))
            flourish_img = flourish_img.resize((new_fl_w, canvas_h), Image.LANCZOS)
            fl_w = new_fl_w
            body_w = max(0, needed_w_px - fl_w)
        canvas.alpha_composite(flourish_img, (body_w, 0))
    ...
    if os.path.exists(_PNG_HEADER_LEADER_RIGHT):
        try:
            ...
            if cache_key not in _LEADER_CACHE:
                ...
                if bbox:
                    img = img.crop(bbox)
                flourish = _extract_flourish_terminal(img)
                tiled = _build_hybrid_leader(flourish, needed_w_px, canvas_h_px)
                if terminal_side == 'left':
                    tiled = tiled.transpose(Image.FLIP_LEFT_RIGHT)
                _LEADER_CACHE[cache_key] = tiled

            reader = ImageReader(_LEADER_CACHE[cache_key])
            c.saveState()
            c.drawImage(reader, x_left, y - render_h * 0.45, width=span, height=render_h, mask='auto')
            c.restoreState()
            return
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    if MAMAR_NAME:
        ...
        mw = max(Wid(mamar_label, HEADING_BOLD_FONT, mamar_label_fs),
                 Wid(mamar_name, HEADING_BOLD_FONT, TOC_MAMAR_FS))
        mx = C_LEFT + (C_W - mw) / 2
        pad_x = 28
        pad_y = 18
        bx = mx - pad_x
        ...
        bw = mw + pad_x * 2
        bh = mamar_label_fs + TOC_MAMAR_FS + mamar_gap + pad_y * 2
        if not draw_generated_toc_mamar_frame(c, C_LEFT + C_W / 2, by + bh / 2, bw, bh):
        ...
        block_h = (label_asc + label_desc) + mamar_gap + (name_asc + name_desc)
        cy = by + (bh - block_h) / 2
        name_y = cy + name_desc
        ...
        c.drawString(C_LEFT + (C_W - Wid(mamar_name, HEADING_BOLD_FONT, TOC_MAMAR_FS)) / 2, name_y, mamar_name)
    ...
    for label_str, name_str, pg in anaf_entries:
        ...
        _draw_simple_printers_mark(c, label_right_x - lw - 14, y + 4.8, 7.2, gray=0.42)
...
def draw_anaf_heading(c, label, name, y):
    ...
    # Compact engraved flankers — smaller for dignified proportion
    flanker_render_w = orn_w * 0.78
    _draw_anaf_flanker(c, sx - ow / 2, y + 4.0, flanker_render_w, gray=0.40)
    _draw_anaf_flanker(c, sx - ow - gap - lw1 - gap - ow / 2, y + 4.0, flanker_render_w, gray=0.40)
