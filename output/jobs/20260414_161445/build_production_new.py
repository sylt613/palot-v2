...
def _draw_anaf_flanker(c, x_center, y_center, width, gray=0.38, mirror=False):
    if draw_png_ornament(c, _PNG_ANAF_FLANKER, x_center, y_center, width=width, preserve_aspect=True, mirror=mirror):
        return
    h = width * 0.92
    top_y = y_center + h / 2
    bot_y = y_center - h / 2
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Fixed x position for TOC accent ornaments — all aligned vertically
    toc_accent_x = label_right_x + 12
    ...
    for label_str, name_str, pg in anaf_entries:
        ...
        draw_png_ornament(c, _PNG_TOC_ENTRY_ACCENT, toc_accent_x, y + 5.0,
                          width=toc_accent_w, preserve_aspect=True)
...
def draw_anaf_heading(c, label, name, y):
    ...
    _draw_anaf_flanker(c, sx - ow / 2, y + 4.0, flanker_render_w, gray=0.40, mirror=False)
    _draw_anaf_flanker(c, sx - ow - gap - lw1 - gap - ow / 2, y + 4.0, flanker_render_w, gray=0.40, mirror=True)
