...
def draw_png_ornament(c, png_path, x_center, y_center, width=None, height=None,
                     preserve_aspect=True, mirror=False, darken=None):
...
def _build_9slice_frame(png_path, target_w_px, target_h_px):
    """Build a frame image at exact target pixel size using 9-slice scaling.
    
    Corners are preserved at native resolution, edges are tiled, center is transparent.
    Returns an RGBA PIL Image of size (target_w_px, target_h_px).
    """
    cache_key = (png_path, target_w_px, target_h_px)
    cached = _FRAME_9SLICE_CACHE.get(cache_key)
    ...
    # Inset for 9-slice: the corner regions to keep intact.
    # Based on analysis: corner flourishes extend ~130px from each edge (horizontal)
    # and ~60px from top/bottom.  Use generous slices to capture all detail.
    ix = min(150, sw // 3)   # horizontal inset (pixels)
    iy = min(80, sh // 3)    # vertical inset (pixels)

    # Extract 9 regions from source
    tl = src.crop((0, 0, ix, iy))
    tr = src.crop((sw - ix, 0, sw, iy))
    bl = src.crop((0, sh - iy, ix, sh))
    br = src.crop((sw - ix, sh - iy, sw, sh))
    top_edge = src.crop((ix, 0, sw - ix, iy))
    bot_edge = src.crop((ix, sh - iy, sw - ix, sh))
    left_edge = src.crop((0, iy, ix, sh - iy))
    right_edge = src.crop((sw - ix, iy, sw, sh - iy))

    # Scale corners if target is smaller than source
    # We want corner pixel sizes to stay proportional to the output
    scale = min(target_w_px / sw, target_h_px / sh, 1.0)
    if scale < 1.0:
        cix = max(1, int(ix * scale))
        ciy = max(1, int(iy * scale))
        tl = tl.resize((cix, ciy), Image.LANCZOS)
        tr = tr.resize((cix, ciy), Image.LANCZOS)
        bl = bl.resize((cix, ciy), Image.LANCZOS)
        br = br.resize((cix, ciy), Image.LANCZOS)
        top_edge = top_edge.resize((top_edge.width, ciy), Image.LANCZOS)
        bot_edge = bot_edge.resize((bot_edge.width, ciy), Image.LANCZOS)
        left_edge = left_edge.resize((cix, left_edge.height), Image.LANCZOS)
        right_edge = right_edge.resize((cix, right_edge.height), Image.LANCZOS)
    else:
        cix = ix
        ciy = iy

    # Build output canvas
    out = Image.new('RGBA', (target_w_px, target_h_px), (255, 255, 255, 0))

    # Paste corners
    out.paste(tl, (0, 0), tl)
    out.paste(tr, (target_w_px - cix, 0), tr)
    out.paste(bl, (0, target_h_px - ciy), bl)
    out.paste(br, (target_w_px - cix, target_h_px - ciy), br)

    # Tile top edge
    mid_w = target_w_px - 2 * cix
    if mid_w > 0 and top_edge.width > 0:
        # Scale edge height to match corner height
        te = top_edge.resize((top_edge.width, ciy), Image.LANCZOS) if top_edge.height != ciy else top_edge
        for x_off in range(0, mid_w, te.width):
            w = min(te.width, mid_w - x_off)
            strip = te.crop((0, 0, w, te.height))
            out.paste(strip, (cix + x_off, 0), strip)

    # Tile bottom edge
    if mid_w > 0 and bot_edge.width > 0:
        be = bot_edge.resize((bot_edge.width, ciy), Image.LANCZOS) if bot_edge.height != ciy else bot_edge
        for x_off in range(0, mid_w, be.width):
            w = min(be.width, mid_w - x_off)
            strip = be.crop((0, 0, w, be.height))
            out.paste(strip, (cix + x_off, target_h_px - ciy), strip)

    # Tile left edge
    mid_h = target_h_px - 2 * ciy
    if mid_h > 0 and left_edge.height > 0:
        le = left_edge.resize((cix, left_edge.height), Image.LANCZOS) if left_edge.width != cix else left_edge
        for y_off in range(0, mid_h, le.height):
            h = min(le.height, mid_h - y_off)
            strip = le.crop((0, 0, le.width, h))
            out.paste(strip, (0, ciy + y_off), strip)

    # Tile right edge
    if mid_h > 0 and right_edge.height > 0:
        re = right_edge.resize((cix, right_edge.height), Image.LANCZOS) if right_edge.width != cix else right_edge
        for y_off in range(0, mid_h, re.height):
            h = min(re.height, mid_h - y_off)
            strip = re.crop((0, 0, re.width, h))
            out.paste(strip, (target_w_px - cix, ciy + y_off), strip)

    _FRAME_9SLICE_CACHE[cache_key] = out
    return out


def _draw_9slice_frame(c, png_path, x_center, y_center, width_pt, height_pt):
    """Draw a 9-slice frame onto the reportlab canvas at the given center/size (in points)."""
    if not os.path.exists(png_path):
        return False
    # Convert pt to px at 300 DPI for high-quality rendering
    DPI = 300
    px_per_pt = DPI / 72.0
    target_w_px = max(1, int(width_pt * px_per_pt))
    target_h_px = max(1, int(height_pt * px_per_pt))

    frame_img = _build_9slice_frame(png_path, target_w_px, target_h_px)
    ...
    x = x_center - width_pt / 2
    y = y_center - height_pt / 2
    c.drawImage(reader, x, y, width=width_pt, height=height_pt, mask='auto')
    return True


def draw_generated_mamar_frame(c, x_center, y_center, width, height):
    if _draw_9slice_frame(c, _PNG_MAMAR_FRAME, x_center, y_center, width, height):
        return True
    # Fallback: simple stretch (should not normally be reached)
    return draw_png_ornament(c, _PNG_MAMAR_FRAME, x_center, y_center, width=width, height=height,
                             preserve_aspect=False)

def draw_generated_toc_mamar_frame(c, x_center, y_center, width, height):
    png_path = _PNG_TOC_MAMAR_FRAME if os.path.exists(_PNG_TOC_MAMAR_FRAME) else _PNG_MAMAR_FRAME
    if _draw_9slice_frame(c, png_path, x_center, y_center, width, height):
        return True
    # Fallback: simple stretch
    return draw_png_ornament(c, png_path, x_center, y_center, width=width, height=height,
                             preserve_aspect=False)
...
def _draw_anaf_flanker(c, x_center, y_center, width, gray=0.38, mirror=False, darken=None):
    if draw_png_ornament(c, _PNG_ANAF_FLANKER, x_center, y_center, width=width, preserve_aspect=True, mirror=mirror, darken=darken):
        return
    h = width * 0.92
    top_y = y_center + h / 2
    bot_y = y_center - h / 2
    ...
    outer = c.beginPath()
    outer.moveTo(x_center, bot_y)
    outer.curveTo(x_center + width * 0.12, y_center - h * 0.10,
                  x_center + width * 0.22, y_center + h * 0.15,
                  x_center, top_y)
    outer.curveTo(x_center - width * 0.22, y_center + h * 0.15,
                  x_center - width * 0.12, y_center - h * 0.10,
                  x_center, bot_y)
    c.drawPath(outer, fill=0, stroke=1)

    c.line(x_center, bot_y + h * 0.10, x_center, top_y - h * 0.12)

    left = c.beginPath()
    left.moveTo(x_center - width * 0.02, y_center - h * 0.05)
    left.curveTo(x_center - width * 0.20, y_center - h * 0.04,
                 x_center - width * 0.23, y_center - h * 0.18,
                 x_center - width * 0.06, y_center - h * 0.19)
    c.drawPath(left, fill=0, stroke=1)

    right = c.beginPath()
    right.moveTo(x_center + width * 0.02, y_center - h * 0.05)
    right.curveTo(x_center + width * 0.20, y_center - h * 0.04,
                  x_center + width * 0.23, y_center - h * 0.18,
                  x_center + width * 0.06, y_center - h * 0.19)
    c.drawPath(right, fill=0, stroke=1)

    c.circle(x_center, y_center - h * 0.02, width * 0.045, fill=1, stroke=0)
    c.restoreState()
...
SUB_AFTER_GAP = LH * 0.18       # small gap after subhead — subheads sit close to following par
SUB_LH = SUB_FS * S.H2_LINE_HEIGHT_MULT
SUB_BEFORE_FRAG_GAP = S.H2_SPACE_BEFORE_PT
SUB_AFTER_FRAG_GAP = SUB_AFTER_GAP
MIN_SUB_AFTER_GAP = LH * 0.10   # never crush below ~1.6pt
MAX_SUB_AFTER_GAP = LH * 1.5    # allow up to ~26pt stretch after sub-ending frags (matches MAX_E_PER_GAP)
...
HEADER_MAMAR_FS = S.ODD_HEADER_MAAR_SIZE  # mamar title slightly bigger
...
def _extract_flourish_terminal(img):
    ...
    # Extract tile from the safe plain-rule zone (middle of the left half)
    tile_center = min(w // 4, flourish_start // 2)
    tile_w = 20
    tile_x = max(0, tile_center - tile_w // 2)
    tile = img.crop((tile_x, 0, tile_x + tile_w, h))
    
    # Extract flourish (everything from flourish_start to the right edge)
    flourish = img.crop((flourish_start, 0, w, h))
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    if mamar_vis:
        ...
        if x - mw < C_LEFT:
            ...
            while words and Wid(' '.join(words) + '...', HEADING_BOLD_FONT, HEADER_MAMAR_FS) > x - C_LEFT - 2:
                words.pop()
        ...
        # Ornament between mamar and ענף (kept minimal; leader does the main decorative work)
        if label_vis:
            x -= (orn_gap + 2)
            c.setFillColorRGB(0.15, 0.15, 0.15)
    ...
    # Leader: straight rule plus small terminal flourish at the outer edge.
    rule_y = header_y - 1.35
    _draw_header_leader_with_terminal(c, rule_y,
...
def draw_running_header_even(c, page_num, anaf_label, anaf_name):
    ...
    _draw_title_with_glow(c, title_x, header_y, title_vis,
                           ...
                           text_color=(0.25, 0.25, 0.25),
                           glow_color=(0.84, 0.84, 0.84),
                           glow_gap=1.2, glow_width=0.5)
...
TOC_MAMAR_FS   = 13.8   # "מאמר" label + mamar name — boxed, centered
...
def draw_contact_page(c):
    ...
    # Bottom marble strip, flush with page bottom
    ...
    bw = C_W * 0.82
    bh = PAGE_H * (0.60 if strip_path else 0.34)
    bx = x_center - bw / 2
    by = (-PAGE_H * 0.04) if strip_path else (PAGE_H * 0.26)
    radius = 12
    ...
    c.setLineWidth(0.28)
    c.roundRect(bx + 3.2, by + 3.2, bw - 6.4, bh - 6.4, radius - 1.6, fill=0, stroke=1)

    if not strip_path:
        top_divider_y = by + bh + 26
        if not draw_generated_divider(c, x_center, top_divider_y, 56, kind='toc'):
            draw_gray_top_orn(c, x_center, top_divider_y, 56, gray=0.70)

    lines = [
        ("לכל עניני הספר:", HEADING_BOLD_FONT, 15.3, 0.10),
        ("יואל באלזאם", HEADING_BOLD_FONT, 14.6, 0.10),
        ("(908) 783-8307", HEADING_FONT, 13.5, 0.12),
        ("2151 58th St", HEADING_FONT, 13.0, 0.12),
        ("Brooklyn, NY 11204", HEADING_FONT, 13.0, 0.12),
        (None, None, 28.0, None),
        ("עימוד ועיצוב:", HEADING_BOLD_FONT, 13.5, 0.10),
        ("(845) 826 - 1850", HEADING_FONT, 12.0, 0.12),
    ]

    face = pdfmetrics.getFont(HEADING_BOLD_FONT).face
    asc = (face.ascent or 800) / 1000.0
    start_y = by + bh * (0.89 if strip_path else 0.80)
    y = start_y
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # ── Title: תוכן הענינים in ornamental frame ────────────────────────────
    title_text = vis("תוכן הענינים")
    title_y = C_TOP - 16
    tw = Wid(title_text, HEADING_BOLD_FONT, TOC_TITLE_FS)
    title_pad_x = 36
    title_pad_y = 16
    title_bw = tw + title_pad_x * 2
    title_bh = TOC_TITLE_FS + title_pad_y * 2 + 4
    title_cx = C_LEFT + C_W / 2
    title_cy = title_y + TOC_TITLE_FS * 0.35  # center box around text
    # Draw ornamental frame around the title
    if not draw_generated_toc_mamar_frame(c, title_cx, title_cy, title_bw, title_bh):
        # Fallback: simple rounded rect
        c.setStrokeColorRGB(0.50, 0.50, 0.50)
        c.setLineWidth(0.8)
        c.roundRect(title_cx - title_bw / 2, title_cy - title_bh / 2, title_bw, title_bh, 8, fill=0, stroke=1)
    ...
    if not draw_generated_divider(c, C_LEFT + C_W / 2, title_orn_y, 66, kind='toc'):
        draw_gray_top_orn(c, C_LEFT + C_W / 2, title_orn_y, 66, gray=0.65)

    # ── Mamar label + name (no frame, just centered text) ────────────────
    ...
    if MAMAR_NAME:
        ...
        c.drawString(C_LEFT + (C_W - Wid(mamar_label, HEADING_BOLD_FONT, mamar_label_fs)) / 2,
                     mamar_y + TOC_MAMAR_FS + mamar_gap, mamar_label)
        # Name
    ...
    # Fixed x position for TOC accent ornaments — centered between name and label columns
    toc_accent_x = name_right_x + (label_right_x - label_col_w - name_right_x) / 2
    toc_accent_w = 12  # rendered width of the small scroll accent
    ...
    for label_str, name_str, pg in anaf_entries:
        ...
        # Scroll accent — fixed x so all accents sit on top of each other
        draw_png_ornament(c, _PNG_TOC_ENTRY_ACCENT, toc_accent_x, y + 5.0,
                          width=toc_accent_w, preserve_aspect=True, darken=0.45)
    ...
    if not draw_generated_divider(c, C_LEFT + C_W / 2, orn_y, 66, kind='toc'):
    ...
    for i, ld in enumerate(line_descs):
        ...
        if ld['type'] == 'subhead':
            ...
            if tw <= col_w:
                ...
                c.setFillColorRGB(0, 0, 0)
                c.drawString(cx_center - tw / 2, y, text_vis)
            else:
...
def draw_anaf_heading(c, label, name, y):
    ...
    # Horizontal scroll flankers — right one is original, left is mirrored
    flanker_render_w = orn_w
    _draw_anaf_flanker(c, sx - ow / 2, y + 4.0, flanker_render_w, gray=0.40, mirror=False, darken=0.45)
    _draw_anaf_flanker(c, sx - ow - gap - lw1 - gap - ow / 2, y + 4.0, flanker_render_w, gray=0.40, mirror=True, darken=0.45)
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        if tw <= C_W:
            ...
            frame_pad_x = 40
            frame_pad_y = 18
            frame_bw = tw + frame_pad_x * 2
            frame_bh = fs + frame_pad_y * 2 + 6
            frame_cy = y + fs * 0.35
            if not draw_generated_mamar_frame(c, x_center, frame_cy, frame_bw, frame_bh):
                # Fallback: simple rounded rect
                c.saveState()
                c.setStrokeColorRGB(0.50, 0.50, 0.50)
                c.setLineWidth(0.8)
                c.roundRect(x_center - frame_bw / 2, frame_cy - frame_bh / 2,
                            frame_bw, frame_bh, 8, fill=0, stroke=1)
                c.restoreState()
            c.setFont(font, fs)
            c.drawString(x_center - tw / 2, y, text_vis)
            return fs
        else:
            # Wrap within C_W so title never bleeds into margins
            words = text_clean.split()
            lines = wrap_words(words, C_W, font, fs)
            total_h = len(lines) * fs * 1.3
            # Draw ornamental frame around the wrapped title
            max_lw = max(Wid(vis(' '.join(lw)), font, fs) for lw in lines)
            frame_pad_x = 40
            frame_pad_y = 18
            frame_bw = max_lw + frame_pad_x * 2
            frame_bh = total_h + frame_pad_y * 2
            frame_cy = y - total_h / 2 + fs * 0.35
            if not draw_generated_mamar_frame(c, x_center, frame_cy, frame_bw, frame_bh):
                c.saveState()
                c.setStrokeColorRGB(0.50, 0.50, 0.50)
                c.setLineWidth(0.8)
...
def heading_height(text, kind='section', has_name=True):
    if kind == 'anaf':
        ...
        n_lines = 1 if tw <= C_W else len(wrap_words(text_clean.split(), C_W, font, fs))
        text_h = n_lines * fs if n_lines == 1 else n_lines * fs * 1.3
        return text_h + HEADING_AFTER_GAP + LH * 2
    elif kind == 'anaf_sub':
        fs = SEC_FS
        font = HEADING_BOLD_FONT
        text_clean = strip_nikud(text)
        ...
        for bi, block in enumerate(plan):
            ...
            if block['kind'] in ('sefer_title', 'section', 'anaf', 'anaf_sub'):
                ...
                if block['kind'] == 'sefer_title' and S.SECTION_END_ORNAMENT:
                    render_y -= HSEP * 1.3
                    if not draw_generated_divider(c, x_center, render_y + 4, 92, kind='chapter'):
...
""")
    ...
    try:
        subprocess.run(
            ['pdftoppm', '-r', '200', '-png', out_pdf, stem],
            check=True)
