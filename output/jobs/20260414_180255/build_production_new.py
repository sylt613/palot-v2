...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    if mamar_vis:
        ...
        if label_vis:
            ...
            draw_png_ornament(c, _PNG_TOC_ENTRY_ACCENT, x, header_y + 4.0,
...
def draw_running_header_even(c, page_num, anaf_label, anaf_name):
    ...
    title_orn_w = 24
    title_orn_gap = 6
    title_orn_baseline_y = header_y + 1.8
    ...
    title_orn_x = title_x + tw + title_orn_gap + title_orn_w / 2
    draw_png_ornament(c, _PNG_TOC_ENTRY_ACCENT, title_orn_x, title_orn_baseline_y,
                      width=title_orn_w, preserve_aspect=True, darken=0.45)
    content_end_x = title_orn_x + title_orn_w / 2
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    title_bh = TOC_TITLE_FS + title_pad_y * 2 + 8
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        frame_cy = y - (total_h - fs) / 2 + fs * 0.35
