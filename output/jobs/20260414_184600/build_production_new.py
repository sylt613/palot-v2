...
SUB_RULE_INSET = 2.0
SUB_RULE_TOP_PAD = 1.8
SUB_TEXT_TOP_PAD = 6.8
SUB_TEXT_BOTTOM_PAD = 4.8
...
def _get_tiled_subhead_rule_reader(needed_w_px, canvas_h_px, terminal_side='right', fade_side='left', flip_vertical=False):
    ...
    try:
        ...
        fade_zone_px = min(max(18, int(needed_w_px * 0.16)), max(18, needed_w_px // 3))
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    if mamar_vis:
        ...
        # Small ornament between mamar and ענף
        if label_vis:
            x -= (mamar_anaf_orn_w / 2 + mamar_anaf_gap)
            draw_png_ornament(c, _PNG_TOC_ENTRY_ACCENT, x, header_y + 2.7,
                              width=mamar_anaf_orn_w, preserve_aspect=True, darken=0.45)
            x -= (mamar_anaf_orn_w / 2 + orn_gap)
            c.setFillColorRGB(0.15, 0.15, 0.15)
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    title_bh = TOC_TITLE_FS + title_pad_y * 2 + 10
    title_cx = C_LEFT + C_W / 2
    title_cy = title_y + TOC_TITLE_FS * 0.18  # visually center Hebrew text inside box
    # Draw ornamental frame around the title
    if not draw_generated_mamar_frame(c, title_cx, title_cy, title_bw, title_bh):
    ...
    # ── Y-anchor below title (divider removed per user request) ─────────
    ...
    # Fixed x position for TOC accent ornaments — right of center in gap, closer to label
    toc_accent_x = label_right_x - label_col_w - 8
    toc_accent_w = 22  # rendered width of the scroll accent (much bigger)
    ...
    for label_str, name_str, pg in anaf_entries:
        ...
        draw_png_ornament(c, _PNG_TOC_ENTRY_ACCENT, toc_accent_x, y + 3.6,
...
def draw_anaf_heading(c, label, name, y):
    ...
    orn_w = 36  # bigger single flanker per side
    ow = orn_w
    ...
    # One scroll flanker on each side — right is original, left is mirrored
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        frame_bh = total_h + frame_pad_y * 2 + 10
        frame_drop = 12.0
        frame_cy = y - (total_h - fs) / 2 + fs * 0.35 - frame_drop
...
def heading_height(text, kind='section', has_name=True):
    if kind == 'anaf':
        ...
        n_lines = len(lines)
        text_h = fs if n_lines == 1 else n_lines * fs * 1.3
        return text_h + HEADING_AFTER_GAP + LH * 0.5 + 12.0
    elif kind == 'anaf_sub':
        fs = SEC_FS
        font = HEADING_BOLD_FONT
        text_clean = strip_nikud(text)
