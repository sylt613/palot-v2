...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        rule_img  = _build_symmetric_tiled_rule(needed_w_px, rule_h_px)
...
def draw_title_rule_pair(c, x_center, y_top_rule, y_bot_rule, width):
    ...
    y_bot_rule: y-center of the bottom rule (bottom rule is top rule flipped vertically).
    Uses the same subhead_rule_right source — symmetric (curl on both ends)."""
    render_h = 14.0
    ppi = 300
    needed_w_px = max(1, round(width * ppi / 72))
    ...
    if cached is None:
        sym = _build_symmetric_tiled_rule(needed_w_px, rule_h_px)
        if sym is None:
            return False
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    title_rule_pad = 12.0   # gap between rule center and text edge
    title_rule_w   = C_W * 0.78
    title_cx = C_LEFT + C_W / 2
    # Top rule sits title_rule_pad above the text baseline
    title_top_rule_y = title_y + TOC_TITLE_FS * 0.85 + title_rule_pad
    # Bottom rule sits title_rule_pad below the descender
    title_bot_rule_y = title_y - title_rule_pad
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        frame_drop = 18.0
        rule_pad   = 10.0  # gap between rule center and text edge

        cy = y - frame_drop
        # Top rule: above the first text line cap-height
        top_rule_y = cy + fs * 0.88 + rule_pad
        # Render text lines first to find bottom baseline
        text_cy = cy
        for line_words in lines:
            ...
            text_cy -= fs * 1.18
        last_baseline = text_cy + fs * 1.18  # undo last decrement
...
def heading_height(text, kind='section', has_name=True):
    if kind == 'anaf':
        ...
        return text_h + HEADING_AFTER_GAP + LH * 0.5 + 18.0
    elif kind == 'anaf_sub':
        fs = SEC_FS
        font = HEADING_BOLD_FONT
        text_clean = strip_nikud(text)
