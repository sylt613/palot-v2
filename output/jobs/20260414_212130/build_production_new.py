...
SUB_AFTER_GAP = LH * 0.75       # gap after subhead bottom rule before next paragraph
...
MIN_SUB_AFTER_GAP = LH * 0.50   # never crush below ~8pt after subhead bottom rule
...
def _draw_header_band_leader(c, y, x_left, x_right):
    """Header leader using the post-anaf inner-band fragment, faded both ends.
    Replaces the old line+curl leader in both odd and even running headers."""
    if x_right <= x_left + 6:
        return
    render_h = 9.0
    ppi = 300
    ...
    c.saveState()
    c.drawImage(reader, x_left, y - render_h * 0.45,
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    if mamar_vis:
        ...
        # Inner-band fragment ornament between mamar and ענף — ~1 char wide
        if label_vis:
            frag_w_pt = 10.0
            frag_h_pt = 8.0
            frag_reader = _get_inner_band_fragment_reader(frag_w_pt, frag_h_pt)
            x -= (frag_w_pt / 2 + mamar_anaf_gap)
            if frag_reader:
                c.saveState()
                c.drawImage(frag_reader, x - frag_w_pt / 2,
                            header_y - frag_h_pt * 0.3,
                            width=frag_w_pt, height=frag_h_pt,
                            mask=[254, 255, 254, 255, 254, 255])
                c.restoreState()
            x -= (frag_w_pt / 2 + orn_gap)
            c.setFillColorRGB(0.15, 0.15, 0.15)
