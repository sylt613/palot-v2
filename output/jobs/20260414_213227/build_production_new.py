...
SUB_AFTER_GAP = LH * 0.90       # gap after subhead bottom rule before next paragraph
SUB_LH = SUB_FS * S.H2_LINE_HEIGHT_MULT
SUB_RULE_INSET = 2.0
SUB_RULE_TOP_PAD = 7.0
SUB_TEXT_TOP_PAD = 17.0
SUB_TEXT_BOTTOM_PAD = 13.0
SUB_BEFORE_FRAG_GAP = S.H2_SPACE_BEFORE_PT
SUB_AFTER_FRAG_GAP = SUB_AFTER_GAP
MIN_SUB_AFTER_GAP = LH * 0.55   # never crush below ~9pt after subhead bottom rule
MAX_SUB_AFTER_GAP = LH * 2.2    # allow more stretch after sub-ending frags
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    The inner-band leader runs as one continuous strip behind all the text."""
    header_y = C_TOP - 2
    ...
    # Draw the full-width band FIRST (behind text), from page-number gap to right margin
    rule_y = header_y - 1.35
    _draw_header_band_leader(c, rule_y,
                             x_left=C_LEFT + pg_w + 4,
                             x_right=C_RIGHT - 4)

    # Mamar name in bold — rightmost, drawn ON TOP of the band
    x = C_RIGHT
    if mamar_vis:
        ...
        x -= 6   # small gap between mamar and band

    # Anaf label — drawn ON TOP of the band, to the left of mamar
    if label_vis:
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Pre-build inner band fragment reader for TOC entry separators — narrow, ~1 char wide
    toc_frag_w_pt = 10.0
    toc_frag_h_pt = 8.0
    _toc_frag_reader = _get_inner_band_fragment_reader(toc_frag_w_pt, toc_frag_h_pt)
    # Center the fragment in the gap between name column and label column
    toc_accent_cx = (name_right_x + (label_right_x - label_col_w)) / 2
    ...
    for label_str, name_str, pg in anaf_entries:
        ...
        # Inner-band fragment accent — centered in the gap between label and name columns
        if _toc_frag_reader:
            c.saveState()
            c.drawImage(_toc_frag_reader,
                        toc_accent_cx - toc_frag_w_pt / 2,
                        y - toc_frag_h_pt * 0.15,
                        width=toc_frag_w_pt, height=toc_frag_h_pt,
                        mask=[254, 255, 254, 255, 254, 255])
            c.restoreState()
