...
_TITLE_RULE_CACHE = {}  # keyed by (needed_w_px, rule_h_px)

def _build_title_rule_symmetric(needed_w_px, canvas_h_px):
    """Build a symmetric curl+line+curl title rule using header_leader_right.png as source.
    This gives a visually distinct but related feel compared to the subhead rule.
    Falls back to subhead_rule_right if header_leader not available."""
    src_path = _PNG_HEADER_LEADER_RIGHT if os.path.exists(_PNG_HEADER_LEADER_RIGHT) else _PNG_SUBHEAD_RULE_RIGHT
    ...
    try:
        ...
        if tile is None:
            return None
        half_w = needed_w_px // 2
        # Right half: curl on right
        right_half = _build_tiled_leader(tile, flourish, half_w, canvas_h_px)
        # Left half: mirror so curl faces left
        left_half = right_half.transpose(Image.FLIP_LEFT_RIGHT)
        canvas = Image.new("RGBA", (needed_w_px, canvas_h_px), (255, 255, 255, 0))
        canvas.alpha_composite(left_half, (0, 0))
        canvas.alpha_composite(right_half, (needed_w_px - half_w, 0))
...
def draw_title_rule_pair(c, x_center, y_top_rule, y_bot_rule, width):
    """Draw matching symmetric curl+line+curl rules above and below a title block.
    Uses header_leader_right.png for a richer body, distinct from subhead rules.
    y_top_rule: y-center of the top rule.
    y_bot_rule: y-center of the bottom rule (flipped vertically)."""
    render_h = 14.0
    ppi = 300
    needed_w_px = max(1, round(width * ppi / 72))
    rule_h_px   = max(4, round(render_h * ppi / 72))
    cache_key   = (needed_w_px, rule_h_px)
    cached = _TITLE_RULE_CACHE.get(cache_key)
    if cached is None:
        sym = _build_title_rule_symmetric(needed_w_px, rule_h_px)
        if sym is None:
            return False
        flat     = Image.new("RGB", sym.size, (255, 255, 255))
        flat.paste(sym, mask=sym.split()[3])
        flat_flip = flat.transpose(Image.FLIP_TOP_BOTTOM)
        cached = (ImageReader(flat), ImageReader(flat_flip))
        _TITLE_RULE_CACHE[cache_key] = cached
    top_reader, bot_reader = cached
    x_left = x_center - width / 2
    c.saveState()
    c.drawImage(top_reader, x_left, y_top_rule - render_h / 2,
                width=width, height=render_h, mask=[254, 255, 254, 255, 254, 255])
    c.drawImage(bot_reader, x_left, y_bot_rule - render_h / 2,
                width=width, height=render_h, mask=[254, 255, 254, 255, 254, 255])
    c.restoreState()
    return True

def draw_generated_subhead_rule(c, x_center, y_center, width, mirror=False, flip_vertical=False):
    render_h = 14.0
    ppi = 300
    needed_w_px = max(1, round(width * ppi / 72))
...
def _get_tiled_subhead_rule_reader(needed_w_px, canvas_h_px, terminal_side='right', fade_side='left', flip_vertical=False):
    cache_key = (needed_w_px, canvas_h_px, terminal_side, fade_side, flip_vertical)
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    """Odd (recto) page header: outside=page number, inside=mamar + band + ענף.
    Band appears in three segments:
      seg1: page_num_right → ענף_left  (fade-left, fade-right)
      seg2: ענף_right → mamar_left     (fade-left, fade-right)
      (no band beyond mamar — fades out just before mamar text)
    """
    header_y = C_TOP - 2
    rule_y = header_y + 3.5  # center of band aligns with cap-height of header text
    ...
    # Layout (RTL): mamar at far right, then gap, then ענף label, then band, then page num
    GAP = 6.0  # gap between text and band edge
    mamar_x = C_RIGHT - mw           # left edge of mamar text
    label_x = mamar_x - GAP - lw     # left edge of ענף text
    label_right = mamar_x - GAP      # right edge of ענף text

    band_start = C_LEFT + pg_w + GAP  # left edge of first band segment

    # Segment 1: from page-number to ענף label (if ענף present)
    if label_vis and label_x > band_start + 8:
        seg1_right = label_x - GAP
        if seg1_right > band_start + 8:
            _draw_header_band_leader(c, rule_y, x_left=band_start, x_right=seg1_right)

    # Segment 2: from ענף right edge to mamar left edge
    if label_vis:
        seg2_left  = label_right + GAP
        seg2_right = mamar_x - GAP
    else:
        seg2_left  = band_start
        seg2_right = mamar_x - GAP
    if seg2_right > seg2_left + 8:
        _draw_header_band_leader(c, rule_y, x_left=seg2_left, x_right=seg2_right)

    # Draw ענף label ON TOP
    if label_vis:
        c.setFont(HEADING_FONT, HEADER_ANAF_FS)
        c.setFillColorRGB(0.15, 0.15, 0.15)
        c.drawString(label_x, header_y, label_vis)

    # Draw mamar name ON TOP (rightmost)
    if mamar_vis:
...
def draw_running_header_even(c, page_num, anaf_label, anaf_name):
    ...
    # Center vertically on cap-height of header text
    rule_y = header_y + 3.5
...
def draw_contact_page(c):
    ...
    if not strip_path:
        ...
        # Divider removed per user request — frame provides enough style
        pass
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # ── Title: תוכן הענינים with ornamental rule pair ─────────────────────
    title_text = vis("תוכן הענינים")
    title_rule_pad = 11.0   # gap between rule center and nearest text edge
    title_rule_w   = C_W * 0.78
    title_cx = C_LEFT + C_W / 2
    # Anchor on content top; center the text+rules block from C_TOP - 10
    # Text block height (baseline to cap-top) ≈ TOC_TITLE_FS * 0.72
    text_block_h   = TOC_TITLE_FS * 0.72
    # Total span from bottom of bot-rule to top of top-rule
    total_span_h   = text_block_h + title_rule_pad * 2
    # Place midpoint of that span at C_TOP - 18 - total_span_h/2
    block_mid_y    = C_TOP - 18 - total_span_h / 2
    # Baseline: cap-height/2 below midpoint
    title_y        = block_mid_y + text_block_h / 2
    tw = Wid(title_text, HEADING_BOLD_FONT, TOC_TITLE_FS)
    title_top_rule_y = title_y + text_block_h / 2 + title_rule_pad
    title_bot_rule_y = title_y - text_block_h / 2 - title_rule_pad
    draw_title_rule_pair(c, title_cx, title_top_rule_y, title_bot_rule_y, title_rule_w)
    c.setFont(HEADING_BOLD_FONT, TOC_TITLE_FS)
    c.setFillColorRGB(0.1, 0.1, 0.1)
    c.drawString(C_LEFT + (C_W - tw) / 2, title_y, title_text)

    # ── Y-anchor below title ─────────────────────────────────────────────
    title_orn_y = title_y - TOC_TITLE_FS * 1.75

    # ── Mamar label + name with line+curl rule pair ───────────────────────
    mamar_y = title_orn_y - TOC_TITLE_FS * 1.8
    if MAMAR_NAME:
        ...
        c.drawString(C_LEFT + (C_W - Wid(mamar_name, HEADING_BOLD_FONT, TOC_MAMAR_FS)) / 2,
                     mamar_y, mamar_name)
        # Rule pair framing the mamar block
        mamar_rule_w = C_W * 0.72
        mamar_rule_pad = 9.0
        mamar_top_rule_y = mamar_y + TOC_MAMAR_FS + mamar_gap + mamar_label_fs + mamar_rule_pad
        mamar_bot_rule_y = mamar_y - mamar_rule_pad
        draw_title_rule_pair(c, title_cx, mamar_top_rule_y, mamar_bot_rule_y, mamar_rule_w)

    # ── Gap to entry list ────────────────────────────────────────────────
    rule_y = mamar_y - TOC_MAMAR_FS * 1.8
    ...
    # Ornament after last entry removed per user request — frame provides enough style
    ...
    for i, ld in enumerate(line_descs):
        ...
        if ld['type'] == 'subhead':
            ...
            if tw <= col_w:
                ...
                # Flankers on both sides of the centered text
                txt_right = cx_center + tw / 2
                txt_left  = cx_center - tw / 2
                flanker_y = text_y + SUB_FS * 0.28
                flank_gap = 4.0
                rule_right = col_left + col_w - SUB_RULE_INSET
                rule_left  = col_left + SUB_RULE_INSET
                if txt_right + flank_gap + 8 < rule_right:
                    _draw_anaf_flanker(c, txt_right + flank_gap, rule_right, flanker_y, side='right')
                if rule_left + 8 < txt_left - flank_gap:
                    _draw_anaf_flanker(c, rule_left, txt_left - flank_gap, flanker_y, side='left')
            else:
                ...
                # Flankers on first line only
                if first_line_w > 0:
                    flanker_y = text_y + SUB_FS * 0.28
                    flank_gap = 4.0
                    rule_right = col_left + col_w - SUB_RULE_INSET
                    rule_left  = col_left + SUB_RULE_INSET
                    txt_right = cx_center + first_line_w / 2
                    txt_left  = cx_center - first_line_w / 2
                    if txt_right + flank_gap + 8 < rule_right:
                        _draw_anaf_flanker(c, txt_right + flank_gap, rule_right, flanker_y, side='right')
                    if rule_left + 8 < txt_left - flank_gap:
                        _draw_anaf_flanker(c, rule_left, txt_left - flank_gap, flanker_y, side='left')
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        c.setFillColorRGB(0, 0, 0)
        max_title_w = C_W * 0.80  # cap title width at 80% of content width
        # Always use balanced wrapping for symmetrical line lengths
        ...
        if len(lines) == 1:
            ...
            total_h = len(lines) * fs * 1.18
            max_lw = max(Wid(vis(' '.join(lw)), font, fs) for lw in lines)
        frame_pad_x = 54
        frame_bw = max_lw + frame_pad_x * 2
        rule_pad = 11.0  # gap between rule center and nearest text edge

        # Text block height: from baseline of last line to cap-top of first line
        text_block_h = total_h  # total_h already accounts for line step * n_lines
        # Center the text block around y (the caller's anchor)
        cy = y + (text_block_h - fs) / 2  # top-most baseline
        ...
        for line_words in lines:
            ...
            text_cy -= fs * 1.18
        last_baseline = text_cy + fs * 1.18

        # Rules: symmetric around the text block
        top_rule_y = cy + fs * 0.75 + rule_pad
        bot_rule_y = last_baseline - rule_pad
        draw_title_rule_pair(c, x_center, top_rule_y, bot_rule_y, frame_bw)
        return max(total_h, fs)
    else:
        fs = SEC_FS
