...
def draw_title_rule_pair(c, x_center, y_top_rule, y_bot_rule, width):
    """Draw matching curl+line rules above and below a title block.
    Uses draw_generated_subhead_rule (subhead_rule_right.png) which is proven visible.
    Top rule: curl on right, fades left.  Bottom rule: mirrored + flipped vertically."""
    draw_generated_subhead_rule(c, x_center, y_top_rule, width,
                                mirror=False, flip_vertical=False)
    draw_generated_subhead_rule(c, x_center, y_bot_rule, width,
                                mirror=True, flip_vertical=True)
    return True
...
def draw_running_header_even(c, page_num, anaf_label, anaf_name):
    """Even (verso) page header: title on left, mamar|anaf on right, page number outermost right."""
    header_y = C_TOP - 2
    pg_str = heb_page(page_num)
    ...
    # Mamar name | ענף label — drawn right-to-left just inside the page number
    mamar_vis = vis(livorna_fix_quotes(strip_nikud(MAMAR_NAME))) if MAMAR_NAME else ""
    label_vis = vis(livorna_fix_quotes(strip_nikud(anaf_label))) if anaf_label else ""
    GAP = 5.0
    bar_gap = 4.0  # space either side of |

    right_anchor = C_RIGHT - pw - GAP  # left edge of page number minus gap

    if mamar_vis or label_vis:
        # Build the composite string width: "mamar | anaf" or just "mamar"
        mw = Wid(mamar_vis, HEADING_BOLD_FONT, HEADER_MAMAR_FS) if mamar_vis else 0.0
        lw = Wid(label_vis, HEADING_FONT, HEADER_ANAF_FS) if label_vis else 0.0
        bar_w = Wid("|", HEADING_FONT, HEADER_ANAF_FS) if (mamar_vis and label_vis) else 0.0

        total_w = mw + (bar_gap * 2 + bar_w if (mamar_vis and label_vis) else 0.0) + lw
        block_left = right_anchor - total_w

        # Draw: rightmost = mamar name, then | , then ענף label to the left
        x = block_left
        if label_vis:
            c.setFont(HEADING_FONT, HEADER_ANAF_FS)
            c.setFillColorRGB(0.15, 0.15, 0.15)
            ...
            x += lw
        if mamar_vis and label_vis:
            c.setFont(HEADING_FONT, HEADER_ANAF_FS)
            c.setFillColorRGB(0.50, 0.50, 0.50)
            c.drawString(x + bar_gap, header_y, "|")
            x += bar_gap * 2 + bar_w
        if mamar_vis:
            c.setFont(HEADING_BOLD_FONT, HEADER_MAMAR_FS)
            c.setFillColorRGB(0.15, 0.15, 0.15)
