    ...
    def _render_page(self, elements, el_idx, line_off, fn_off, carry_fn_lds):
        ...
        for bi, block in enumerate(plan):
            ...
            if (next_el_is_anaf or final_block_of_book) and S.SECTION_END_ORNAMENT:
                div_w = C_W * 0.72
                if final_block_of_book:
                    # Pin the closing ornament to a fixed position near the bottom of
                    # the text area so it fills the gap instead of floating right after
                    # the last text block. Place it 1/3 of the way up from C_BOT.
                    orn_gap = render_y - C_BOT
                    orn_y = C_BOT + orn_gap * 0.5
                    drawn_h = draw_post_anaf_divider_band(c, x_center, orn_y, div_w)
                    if not drawn_h:
                        if not draw_generated_divider(c, x_center, orn_y - 18, div_w, kind='chapter'):
                            draw_gray_bottom_orn(c, x_center, orn_y - 18, div_w, gray=0.55, glow=False)
                    render_y = orn_y - LH * 2.2
                else:
                    render_y -= HSEP * 1.4
