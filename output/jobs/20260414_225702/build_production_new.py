...
def draw_title_rule_pair(c, x_center, y_top_rule, y_bot_rule, width):
    """Draw matching double-line+curl rules above and below a title block.
    Uses title_rule_right.png (fal.ai nano-banana-2 generated).
    Top rule: curl on right.  Bottom rule: mirrored (curl on left) + flipped vertically."""
    render_h = 20.0  # taller than subhead rule to accommodate the larger curl
    png_path = _PNG_TITLE_RULE_RIGHT if os.path.exists(_PNG_TITLE_RULE_RIGHT) else _PNG_SUBHEAD_RULE_RIGHT
    # Top rule: curl at right (as-is)
    draw_png_ornament(c, png_path, x_center, y_top_rule, width=width, height=render_h,
                      preserve_aspect=False, mirror=False, flip_vertical=False)
    # Bottom rule: mirror horizontally (curl at left) and flip vertically
    draw_png_ornament(c, png_path, x_center, y_bot_rule, width=width, height=render_h,
                      preserve_aspect=False, mirror=True, flip_vertical=True)
    return True
