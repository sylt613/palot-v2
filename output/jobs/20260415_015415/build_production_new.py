...
def draw_title_rule_pair(c, x_center, y_top_rule, y_bot_rule, width):
    """Draw matching double-line+acanthus rules above and below a title block.
    Uses title_rule_right.png tiled with terminal on both left and right ends.
    Top rule: as-is.  Bottom rule: flipped vertically."""
    render_h = 20.0  # taller than subhead rule to accommodate the acanthus terminal
    ppi = 300
    needed_w_px = max(1, round(width * ppi / 72))
    ...
    for y_rule, flip_v in [(y_top_rule, False), (y_bot_rule, True)]:
        ...
        if reader:
            ...
            # Fallback: stretch the PNG directly
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    if MAMAR_NAME:
        ...
        c.drawString(C_LEFT + (C_W - Wid(mamar_name, HEADING_BOLD_FONT, TOC_MAMAR_FS)) / 2,
                     mamar_y, mamar_name)
        # Rule pair framing the mamar block — removed; mamar name on TOC needs no rules
        pass
...
""")
    ...
    _pdftoppm_candidates = [
        'pdftoppm',
        '/data/.persist/apt-bin/pdftoppm',
    ]
    _pdftoppm_bin = next((p for p in _pdftoppm_candidates if os.path.exists(p) or p == 'pdftoppm'), 'pdftoppm')
    _extra_libs = '/data/.persist/apt-lib:/tmp'
    _env = dict(os.environ)
    _env['LD_LIBRARY_PATH'] = _extra_libs + ':' + _env.get('LD_LIBRARY_PATH', '')
    try:
        subprocess.run(
