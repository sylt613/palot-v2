...
def _get_tiled_title_rule_reader(needed_w_px, canvas_h_px, flip_vertical=False):
    """Build a tiled title rule with the acanthus terminal on BOTH ends.
    Source: title_rule_right.png (curl on right). Left end = mirrored curl.
    The plain double-rule body is tiled between the two terminals.
    Returns an ImageReader (RGB, white background) ready for drawImage."""
    cache_key = (needed_w_px, canvas_h_px, flip_vertical)
    ...
    try:
        ...
        # Do NOT bbox-crop: the source image may have intentional top/bottom
        # padding that keeps the curl within the canvas — cropping clips the curl.
        tile, flourish = _extract_flourish_terminal(img)
        if tile is None:
            return None
        # Scale flourish to canvas height
        ...
        # Left terminal = mirror of right terminal
        left_flourish = flourish.transpose(Image.FLIP_LEFT_RIGHT)
        # Body width = total width minus both terminals
        body_w = max(0, needed_w_px - 2 * fl_w)
        canvas = Image.new("RGBA", (needed_w_px, canvas_h_px), (255, 255, 255, 0))
        # Paste left terminal
        canvas.alpha_composite(left_flourish, (0, 0))
        # Tile body
        tile_w = tile.width
        for bx in range(fl_w, fl_w + body_w, tile_w):
            paste_w = min(tile_w, fl_w + body_w - bx)
            canvas.alpha_composite(tile.crop((0, 0, paste_w, canvas_h_px)), (bx, 0))
        # Paste right terminal
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    title_rule_pad = 16.0   # gap from text edge to rule centre; render_h=20pt → 6pt clear
    ...
    # Sub-rule height = 14pt, text padding mirrors in-column subheads.
    sub_rule_h       = 14.0   # matches draw_generated_subhead_rule render_h
    sub_rule_top_pad = 7.0    # gap from top rule centre to top of text block
    sub_rule_bot_pad = 9.0    # gap from bottom of text block to bottom rule centre
    ...
    if MAMAR_NAME:
        ...
        mamar_gap      = TOC_MAMAR_FS * 0.45   # gap between label baseline and name cap-top

        # Total text block height: label cap-top down to name baseline
        label_cap_h    = mamar_label_fs * 0.72
        name_cap_h     = TOC_MAMAR_FS  * 0.72
        text_block_h   = label_cap_h + mamar_gap + TOC_MAMAR_FS

        # Anchor: position top rule at title_orn_y - gap
        top_rule_y  = title_orn_y - TOC_TITLE_FS * 1.4
        # Label baseline: sub_rule_top_pad below top rule centre + cap correction
        label_y     = top_rule_y - sub_rule_top_pad - label_cap_h
        # Name baseline: below label
        name_y      = label_y - mamar_gap - TOC_MAMAR_FS
        # Bottom rule: sub_rule_bot_pad below name baseline
        bot_rule_y  = name_y - sub_rule_bot_pad
        ...
        # Anchor for entry list is below the bottom rule
    ...
    # Post-anaf style ornament — anchored near bottom margin for breathing room
    post_orn_h  = 11.0 * 2 + 14.0 + 1.5 * 2   # total height of the band (~39pt)
    post_orn_top_y = C_BOT + post_orn_h + 28.0  # 28pt above bottom margin
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        rule_pad = 16.0  # gap from text edge (cap-top / baseline) to rule centre
                         # render_h=20pt so rule half-height=10pt → 6pt clear breathing
        ...
        # Rules: symmetric around the visual centre of the text block.
        # cap_top = top of first line glyphs; last_baseline = baseline of last line.
        # We treat cap-height as 0.72 * fs (typical for the heading font).
        cap_top    = cy + fs * 0.72
        text_mid_y = (cap_top + last_baseline) / 2
        half_span  = (cap_top - last_baseline) / 2
        top_rule_y = text_mid_y + half_span + rule_pad
        bot_rule_y = text_mid_y - half_span - rule_pad
