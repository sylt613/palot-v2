...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    if MAMAR_NAME:
        ...
        # Top rule: centre = label cap-top + sub_rule_pad
        # → label_y = top_rule_y - sub_rule_pad - label_cap_h  (same as before)
        # But anchor from title so spacing is stable:
        top_rule_y  = title_orn_y - TOC_TITLE_FS * 1.4
        label_y     = top_rule_y - sub_rule_pad - label_cap_h  # rule_bot → label cap-top = sub_text_gap
        name_y      = label_y - mamar_gap - name_cap_h    # baseline of name
        # Bottom rule: centre = name baseline - sub_rule_pad
