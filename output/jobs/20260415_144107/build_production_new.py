...
def _build_frame_image(target_w_px, target_h_px, intensity=1.0):
    """Build a frame of exactly (target_w_px x target_h_px) by:
      - cropping the 4 scroll corners from frame_master.png
      - tiling the straight edge strips to fill the remaining span
    Returns an RGB PIL Image (white background), or None on failure.

    The master image is the FAL-generated baroque scroll frame.
    Measured dimensions (in master-cropped coords, 986x645):
      corner region: 75px each side
      top/bot band height: ~53px
      left band width: ~20px, right band: ~9px (asymmetric — left side thicker)
    """
    ...
    if not os.path.exists(_PNG_FRAME_MASTER):
        return None
    try:
        src = Image.open(_PNG_FRAME_MASTER).convert("RGBA")
        SW, SH = src.size  # full source including whitespace: 1536x1024

        # Frame bounding box in source image (measured)
        F_TOP, F_BOT, F_LEFT, F_RIGHT = 184, 828, 273, 1258
        frame = src.crop((F_LEFT, F_TOP, F_RIGHT + 1, F_BOT + 1))
        FW, FH = frame.size  # ~986x645

        if intensity < 1.0:
            frame = _lighten_pil_image(frame, intensity)

        CORNER = 75   # px in source frame coords — corner ornament size
        TOP_H  = 53   # px — top band height (includes double rule + scroll)
        BOT_H  = 53   # px — bottom band height
        L_W    = 20   # px — left side band width
        R_W    = 9    # px — right side band width (thinner in source)

        # Crop the 4 corners at source scale
        tl = frame.crop((0,        0,        CORNER,    CORNER))
        tr = frame.crop((FW-CORNER, 0,        FW,        CORNER))
        bl = frame.crop((0,        FH-CORNER, CORNER,    FH))
        br = frame.crop((FW-CORNER, FH-CORNER, FW,       FH))

        # Scale corners to fit target — corners scale proportionally with the smaller dim
        scale = min(target_w_px / FW, target_h_px / FH, 1.0)
        # But don't let corners exceed half of target dimensions
        c_px = max(4, min(round(CORNER * scale), target_w_px // 3, target_h_px // 3))
        top_h = max(2, round(TOP_H * scale))
        bot_h = max(2, round(BOT_H * scale))
        l_w   = max(1, round(L_W  * scale))
        r_w   = max(1, round(R_W  * scale))

        def _resize(im, w, h):
            return im.resize((max(1,w), max(1,h)), Image.LANCZOS)

        tl_s = _resize(tl, c_px, c_px)
        tr_s = _resize(tr, c_px, c_px)
        bl_s = _resize(bl, c_px, c_px)
        br_s = _resize(br, c_px, c_px)

        # Edge tile strips (1-pixel-wide slice from middle of each edge)
        mid_x = FW // 2
        mid_y = FH // 2
        top_tile_src = frame.crop((mid_x, 0,        mid_x+1,  TOP_H))
        bot_tile_src = frame.crop((mid_x, FH-BOT_H, mid_x+1,  FH))
        lft_tile_src = frame.crop((0,     mid_y,    L_W,      mid_y+1))
        rgt_tile_src = frame.crop((FW-R_W, mid_y,   FW,       mid_y+1))

        top_tile = _resize(top_tile_src, 1, top_h)
        bot_tile = _resize(bot_tile_src, 1, bot_h)
        lft_tile = _resize(lft_tile_src, l_w, 1)
        rgt_tile = _resize(rgt_tile_src, r_w, 1)

        # Assemble canvas
        canvas = Image.new("RGBA", (target_w_px, target_h_px), (255, 255, 255, 255))

        # Tile top edge between corners
        top_span = target_w_px - 2 * c_px
        if top_span > 0:
            top_band = Image.new("RGBA", (top_span, top_h), (255,255,255,255))
            for x in range(0, top_span):
                top_band.paste(top_tile, (x, 0))
            canvas.alpha_composite(top_band, (c_px, 0))

        # Tile bottom edge between corners
        if top_span > 0:
            bot_band = Image.new("RGBA", (top_span, bot_h), (255,255,255,255))
            for x in range(0, top_span):
                bot_band.paste(bot_tile, (x, 0))
            canvas.alpha_composite(bot_band, (c_px, target_h_px - bot_h))

        # Tile left edge between corners
        side_span = target_h_px - 2 * c_px
        if side_span > 0:
            lft_band = Image.new("RGBA", (l_w, side_span), (255,255,255,255))
            for y in range(0, side_span):
                lft_band.paste(lft_tile, (0, y))
            canvas.alpha_composite(lft_band, (0, c_px))

        # Tile right edge between corners
        if side_span > 0:
            rgt_band = Image.new("RGBA", (r_w, side_span), (255,255,255,255))
            for y in range(0, side_span):
                rgt_band.paste(rgt_tile, (0, y))
            canvas.alpha_composite(rgt_band, (target_w_px - r_w, c_px))

        # Paste corners
        canvas.alpha_composite(tl_s, (0, 0))
        canvas.alpha_composite(tr_s, (target_w_px - c_px, 0))
        canvas.alpha_composite(bl_s, (0, target_h_px - c_px))
        canvas.alpha_composite(br_s, (target_w_px - c_px, target_h_px - c_px))

        # Flatten to RGB
        flat = Image.new("RGB", (target_w_px, target_h_px), (255, 255, 255))
        flat.paste(canvas, mask=canvas.split()[3])
        _FRAME_CACHE[cache_key] = flat
        return flat
...
def draw_contact_page(c):
    ...
    # The inset column is near the right (spine) side for an even page.
    # Two strips, each ~11pt wide (same as the post-anaf rule_h), separated
    # by an inset_gap.  Strip centres at strip1_x and strip2_x.
    strip_w_pt   = 11.0   # rendered width of each vertical strip (= rule_h of post-anaf rule)
    inset_gap    = 52.0   # gap between the two strip centres = width of inset text column
    # Right strip centre: 48pt from the right content edge (moved further in)
    strip2_x     = C_RIGHT - 48.0
    strip1_x     = strip2_x - inset_gap

    # Vertical extent: from C_BOT to C_TOP (full content height)
    strip_top_y  = C_TOP
    strip_bot_y  = C_BOT
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    cap_frac     = 0.72                    # cap-height as fraction of font size
    rule_h_half  = 20.0 / 2               # half the rendered rule height (render_h=20pt)
    text_gap     = 7.0                     # clear gap between rule edge and text edge
    rule_pad     = rule_h_half + text_gap  # rule centre to text edge = 17pt

    # Place title baseline so the top rule clears C_TOP comfortably
    # top_rule_centre = title_y + TOC_TITLE_FS*cap_frac + rule_pad
    # We want top_rule_centre + rule_h_half <= C_TOP - 6
    title_y          = C_TOP - 6 - rule_h_half - rule_pad - TOC_TITLE_FS * cap_frac
    title_top_rule_y = title_y + TOC_TITLE_FS * cap_frac + rule_pad
    title_bot_rule_y = title_y - rule_pad
    tw = Wid(title_text, HEADING_BOLD_FONT, TOC_TITLE_FS)
    draw_title_rule_pair(c, title_cx, title_top_rule_y, title_bot_rule_y, title_rule_w, intensity=0.8)
    ...
    # ── Mamar label + name inside baroque scroll frame ───────────────────
    ...
    if MAMAR_NAME:
        ...
        # Frame: sized to contain both lines with comfortable inner padding
        frame_pad_x  = 28.0   # horizontal inner padding
        frame_pad_y  = 14.0   # vertical inner padding top & bottom
        text_block_h = label_cap_h + mamar_gap + name_cap_h  # visual height of text content
        frame_w      = C_W * 0.72
        frame_h      = text_block_h + frame_pad_y * 2

        # Anchor frame top just below title ornament
        frame_top_y  = title_orn_y - TOC_TITLE_FS * 1.0
        frame_bot_y  = frame_top_y - frame_h
        frame_x_left = mamar_cx - frame_w / 2

        draw_frame_box(c, frame_x_left, frame_bot_y, frame_w, frame_h, intensity=0.8)

        # Text positions: centre the text block vertically within the frame interior
        frame_mid_y = (frame_top_y + frame_bot_y) / 2
        # label baseline: place cap-top of label at frame_mid_y + text_block_h/2
        label_y = frame_mid_y + text_block_h / 2 - label_cap_h
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        frame_pad_x = 54
        frame_pad_y = 24.0   # vertical inner padding top & bottom inside frame

        # Frame dimensions: wide enough for text + horizontal padding
        frame_bw = max_lw + frame_pad_x * 2
        # Frame height: text block + vertical padding
        text_block_h = total_h
        frame_bh = text_block_h + frame_pad_y * 2

        # Center the frame around y (the caller's anchor)
        frame_cx   = x_center
        frame_cy   = y + (text_block_h - fs) / 2 + (text_block_h / 2 - text_block_h / 2)
        # frame_cy is the vertical centre of the text block; frame is centred on it
        frame_bot  = y - frame_pad_y - (text_block_h - fs)
        frame_left = frame_cx - frame_bw / 2

        draw_frame_box(c, frame_left, frame_bot, frame_bw, frame_bh, intensity=1.0)

        # Render text lines centred within the frame
