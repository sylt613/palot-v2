...
def _build_frame_image(target_w_px, target_h_px, intensity=1.0):
    """Build a frame of exactly (target_w_px x target_h_px) by:
      ...
      - tiling a 1-px representative slice of each edge band to fill the span
    Returns an RGB PIL Image (white background), or None on failure.

    Measured dimensions in the 986x645 cropped frame:
      CORNER width  = 100px each side (scroll ornament)
      TOP_H         = 52px  (rows 0–51: 3-rule engraved band at top)
      BOT_H         = 36px  (rows 609–644: bottom rules)
      L_W           = 20px  (left side band, cols 0–19)
      R_W           = 9px   (right side band, cols 977–985)
    The corner box is CORNER × TOP_H at top and CORNER × BOT_H at bottom,
    but we take a square corner of size CORNER × CORNER.
    """
    ...
    try:
        ...
        # Measured constants
        CORNER = 100  # px — horizontal/vertical extent of corner scroll ornament
        TOP_H  = 52   # px — height of top rule band (rows 0–51)
        BOT_H  = 36   # px — height of bottom rule band (rows 609–644)
        L_W    = 20   # px — width of left side band (cols 0–19)
        R_W    = 9    # px — width of right side band (cols 977–985)

        # Scale: fit corners proportionally, capped at 1/3 of target dimension
        scale = min(target_w_px / FW, target_h_px / FH, 1.0)
        c_w   = max(4, min(round(CORNER * scale), target_w_px // 3))
        top_h = max(2, round(TOP_H * scale))
        bot_h = max(2, round(BOT_H * scale))
        l_w   = max(1, round(L_W   * scale))
        r_w   = max(1, round(R_W   * scale))
        # Corner height matches the band it belongs to at top/bottom, but use
        # the larger of top_h/bot_h so the corner is at least that tall.
        c_h_top = max(top_h, round(CORNER * scale * TOP_H / CORNER))
        c_h_bot = max(bot_h, round(CORNER * scale * BOT_H / CORNER))
        # Simpler: just use c_w as the corner square size for both top & bottom corners
        c_h = max(top_h, bot_h, c_w // 2)

        def _resize(im, w, h):
            return im.resize((max(1, w), max(1, h)), Image.LANCZOS)

        # ── Crop the 4 corners (full CORNER width, full band height each side) ──
        # top corners span the full TOP_H rows; bottom corners span full BOT_H rows
        tl = frame.crop((0,          0,        CORNER,    TOP_H))        # top-left
        tr = frame.crop((FW-CORNER,  0,        FW,        TOP_H))        # top-right
        bl = frame.crop((0,          FH-BOT_H, CORNER,    FH))           # bottom-left
        br = frame.crop((FW-CORNER,  FH-BOT_H, FW,        FH))           # bottom-right

        # Also need the side corners (vertical sides where corner meets L/R band)
        # We'll use the top-LEFT corner's height as the corner height.
        # For corners: scale to (c_w × top_h) and (c_w × bot_h)
        tl_s = _resize(tl, c_w, top_h)
        tr_s = _resize(tr, c_w, top_h)
        bl_s = _resize(bl, c_w, bot_h)
        br_s = _resize(br, c_w, bot_h)

        # ── Edge tile strips — use a representative 1-px-wide slice from the middle ──
        mid_x = FW // 2  # safe middle column (uniform rule)
        mid_y = FH // 2  # safe middle row
        # Top: 1×TOP_H slice, will tile horizontally → stretch to 1×top_h
        top_tile = _resize(frame.crop((mid_x, 0,       mid_x+1, TOP_H)),    1, top_h)
        # Bottom: 1×BOT_H slice
        bot_tile = _resize(frame.crop((mid_x, FH-BOT_H, mid_x+1, FH)),      1, bot_h)
        # Left: L_W×1 slice, will tile vertically → stretch to l_w×1
        lft_tile = _resize(frame.crop((0,      mid_y,   L_W,     mid_y+1)), l_w, 1)
        # Right: R_W×1 slice
        rgt_tile = _resize(frame.crop((FW-R_W, mid_y,   FW,      mid_y+1)), r_w, 1)

        # ── Assemble on white canvas ──
        canvas = Image.new("RGB", (target_w_px, target_h_px), (255, 255, 255))

        # Top edge band (between corners)
        top_span = target_w_px - 2 * c_w
        if top_span > 0:
            top_band = Image.new("RGB", (top_span, top_h), (255, 255, 255))
            for x in range(top_span):
                top_band.paste(top_tile, (x, 0))
            canvas.paste(top_band, (c_w, 0))

        # Bottom edge band (between corners)
        if top_span > 0:
            bot_band = Image.new("RGB", (top_span, bot_h), (255, 255, 255))
            for x in range(top_span):
                bot_band.paste(bot_tile, (x, 0))
            canvas.paste(bot_band, (c_w, target_h_px - bot_h))

        # Left side band (between corner rows)
        side_top    = top_h   # side band starts after top band
        side_bot    = target_h_px - bot_h  # and ends before bottom band
        side_span_h = side_bot - side_top
        if side_span_h > 0 and l_w > 0:
            lft_band = Image.new("RGB", (l_w, side_span_h), (255, 255, 255))
            for y in range(side_span_h):
                lft_band.paste(lft_tile, (0, y))
            canvas.paste(lft_band, (0, side_top))

        # Right side band
        if side_span_h > 0 and r_w > 0:
            rgt_band = Image.new("RGB", (r_w, side_span_h), (255, 255, 255))
            for y in range(side_span_h):
                rgt_band.paste(rgt_tile, (0, y))
            canvas.paste(rgt_band, (target_w_px - r_w, side_top))

        # Corners (drawn last so they sit on top of the edge bands)
        canvas.paste(tl_s, (0,                   0))
        canvas.paste(tr_s, (target_w_px - c_w,   0))
        canvas.paste(bl_s, (0,                   target_h_px - bot_h))
        canvas.paste(br_s, (target_w_px - c_w,   target_h_px - bot_h))
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Text baseline: visually centred in frame interior
    # Interior top = frame_top - frame_pad_y; cap-top of text = interior_top - small margin
    interior_top = title_frame_top - frame_pad_y
    title_y = interior_top - TOC_TITLE_FS * cap_frac
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # Frame top = y, frame bottom = y - frame_bh
        frame_top  = y
        ...
        # Text: vertically centre the text block within the frame interior
        # Interior top (below top band) = frame_top - frame_pad_y
        # Interior bot (above bot band) = frame_bot + frame_pad_y
        # Top baseline of first line = interior_top - (cap ascent above baseline)
        # cap ascent ≈ 0.72 * fs; so baseline = interior_top - (frame_pad_y accounts for it)
        # Place first baseline so the visual text block (total_h) is centred in interior
        interior_top = frame_top - frame_pad_y
        # centre of text block inside interior
        text_block_centre_y = interior_top - total_h / 2
        # first line baseline = centre + half of total_h
        first_baseline = text_block_centre_y + total_h / 2
        # but baseline is at bottom of cap, so subtract fs*(1-cap_frac) to shift up
        # Actually just: first_baseline is where we want to draw the first line
        # such that cap_top = first_baseline + fs*0.72 hits the interior_top exactly:
        first_baseline = interior_top - fs * 0.72  # cap-top of first line = interior_top
