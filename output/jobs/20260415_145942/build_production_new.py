...
def _build_frame_image(target_w_px, target_h_px, intensity=1.0):
    ...
    try:
        ...
        L_W    = 25   # px — width of left side band (cols 0–24): two line clusters at 8–11, 17–19
        R_W    = 25   # px — width of right side band (cols 961–985): two clusters at 961–969, 975–977
        ...
        # ── Edge bands — crop the band between corners then resize to target span ──
        # Top: full band between corners, rows 0..TOP_H
        top_src = frame.crop((CORNER, 0,        FW - CORNER, TOP_H))
        # Bottom: full band between corners, rows (FH-BOT_H)..FH
        bot_src = frame.crop((CORNER, FH-BOT_H, FW - CORNER, FH))
        # Left: full side band between top/bot bands, cols 0..L_W
        mid_top_src = TOP_H
        mid_bot_src = FH - BOT_H
        lft_src = frame.crop((0,      mid_top_src, L_W,      mid_bot_src))
        # Right: full side band, cols (FW-R_W)..FW
        rgt_src = frame.crop((FW-R_W, mid_top_src, FW,       mid_bot_src))
        ...
        # Top edge band (between corners): resize source to exact span
        if top_span > 0:
            top_band = _resize(top_src, top_span, top_h)
            canvas.paste(top_band, (c_w, 0))

        # Bottom edge band
        if top_span > 0:
            bot_band = _resize(bot_src, top_span, bot_h)
            canvas.paste(bot_band, (c_w, target_h_px - bot_h))

        # Left side band (between top/bot bands)
        side_top    = top_h
        side_bot    = target_h_px - bot_h
        side_span_h = side_bot - side_top
        if side_span_h > 0 and l_w > 0:
            lft_band = _resize(lft_src, l_w, side_span_h)
            canvas.paste(lft_band, (0, side_top))

        # Right side band
        if side_span_h > 0 and r_w > 0:
            rgt_band = _resize(rgt_src, r_w, side_span_h)
