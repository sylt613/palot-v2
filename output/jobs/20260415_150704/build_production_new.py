...
def draw_contact_page(c):
    ...
    # Anchor the strip pair: position toward the left half of the content area
    # so there's plenty of room for text to the left of the strips
    pair_cx      = C_LEFT + C_W * 0.42
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Draw frame FIRST, then text on top
    draw_frame_box(c, title_frame_left, title_frame_bot, title_frame_w, title_frame_h, intensity=0.8)

    # Text baseline: vertically centred in frame interior
    interior_top = title_frame_top - frame_pad_y
    interior_bot = title_frame_bot + frame_pad_y
    interior_h   = interior_top - interior_bot
    title_y = interior_bot + (interior_h + TOC_TITLE_FS * 0.72) / 2 - TOC_TITLE_FS * (1 - 0.72)
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        frame_pad_x = 80     # generous horizontal padding so frame renders large
        frame_pad_y = 36.0   # generous vertical padding so frame renders large
        ...
        # Draw frame FIRST so text renders on top of it (no white bg blocking)
        draw_frame_box(c, frame_left, frame_bot, frame_bw, frame_bh, intensity=1.0)

        # Text: vertically centre within the frame interior
        # Interior vertical centre = midpoint of frame
        interior_top = frame_top - frame_pad_y
        interior_bot = frame_bot + frame_pad_y
        interior_h   = interior_top - interior_bot
        # Centre the text block in the interior
        first_baseline = interior_bot + (interior_h + total_h) / 2 - fs * (1 - 0.72)
