...
def _build_frame_image(target_w_px, target_h_px, intensity=1.0):
    ...
    try:
        ...
        # Scale for edge bands: fit to target
        scale = min(target_w_px / FW, target_h_px / FH, 1.0)

        # Corners: always render at a generous size so scroll detail is visible.
        # We render the corner at CORNER_RENDER px wide (source pixels), then
        # scale that down to fit. This means the corner occupies more of the frame
        # but looks crisp with full scroll detail.
        CORNER_RENDER = 180  # source px — larger crop = more detail in rendered corner
        # Corner rendered width in target px: scale proportionally but floor at 40px
        c_w   = max(40, min(round(CORNER_RENDER * scale), target_w_px // 3))
        top_h = max(6,  round(TOP_H * scale))
        bot_h = max(4,  round(BOT_H * scale))
        l_w   = max(3,  round(L_W   * scale))
        r_w   = max(3,  round(R_W   * scale))

        def _resize(im, w, h):
            return im.resize((max(1, w), max(1, h)), Image.LANCZOS)

        # ── Crop the 4 corners using CORNER_RENDER width for maximum scroll detail ──
        tl = frame.crop((0,                0,        CORNER_RENDER, TOP_H))
        tr = frame.crop((FW-CORNER_RENDER, 0,        FW,            TOP_H))
        bl = frame.crop((0,                FH-BOT_H, CORNER_RENDER, FH))
        br = frame.crop((FW-CORNER_RENDER, FH-BOT_H, FW,            FH))
        tl_s = _resize(tl, c_w, top_h)
        tr_s = _resize(tr, c_w, top_h)
        bl_s = _resize(bl, c_w, bot_h)
        br_s = _resize(br, c_w, bot_h)

        # ── Edge bands — crop between corners (using CORNER_RENDER) then resize ──
        top_src = frame.crop((CORNER_RENDER, 0,        FW - CORNER_RENDER, TOP_H))
        bot_src = frame.crop((CORNER_RENDER, FH-BOT_H, FW - CORNER_RENDER, FH))
        lft_src = frame.crop((0,      TOP_H,    L_W,      FH - BOT_H))
        rgt_src = frame.crop((FW-R_W, TOP_H,    FW,       FH - BOT_H))
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # Text: vertically centred within frame interior
