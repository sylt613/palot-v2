...
def _build_frame_image(target_w_px, target_h_px, intensity=1.0):
    ...
    Corners are cropped as squares from the source and rendered at a fixed
    minimum size so the scroll detail is always visible regardless of frame scale.
    Returns an RGB PIL Image (white background), or None on failure.

    Source frame (986x645 after crop):
      Corner scroll extends: ~75px wide × ~83px tall in source
      TOP_H = 52px, BOT_H = 36px, L_W = 25px, R_W = 25px
    Strategy: render corners at CORNER_OUT_PX (fixed output px) so they are
    always large enough to see; tile edge bands to fill the rest.
    """
    ...
    try:
        ...
        # Source measurements
        CORNER_SRC = 75   # px — horizontal extent of scroll ornament in source
        CORNER_H_SRC = 83 # px — vertical extent of scroll ornament in source
        TOP_H  = 52        # px — top rule band height
        BOT_H  = 36        # px — bottom rule band height
        L_W    = 25        # px — left side band width
        R_W    = 25        # px — right side band width

        # Corners are always rendered at a fixed minimum size in output pixels
        # so scroll detail is visible. Cap at 1/3 of target dimension.
        CORNER_OUT_MIN = 55   # minimum output px for corner width and height
        c_w = max(CORNER_OUT_MIN, min(round(CORNER_SRC * target_w_px / FW), target_w_px // 3))
        c_h = max(CORNER_OUT_MIN, min(round(CORNER_H_SRC * target_h_px / FH), target_h_px // 3))

        # Edge band thicknesses — scale with target, small minimum
        scale  = min(target_w_px / FW, target_h_px / FH)
        top_h  = max(4, round(TOP_H * scale))
        bot_h  = max(3, round(BOT_H * scale))
        l_w    = max(2, round(L_W   * scale))
        r_w    = max(2, round(R_W   * scale))

        def _resize(im, w, h):
            return im.resize((max(1, w), max(1, h)), Image.LANCZOS)

        # ── Corners: crop as CORNER_SRC × CORNER_H_SRC squares, resize to c_w × c_h ──
        ...
        # ── Edge bands between corners ──
        top_src = frame.crop((CORNER_SRC, 0,        FW-CORNER_SRC, TOP_H))
        bot_src = frame.crop((CORNER_SRC, FH-BOT_H, FW-CORNER_SRC, FH))
        lft_src = frame.crop((0,      CORNER_H_SRC, L_W,   FH-CORNER_H_SRC))
        rgt_src = frame.crop((FW-R_W, CORNER_H_SRC, FW,    FH-CORNER_H_SRC))
        ...
        if top_span > 0:
            canvas.paste(_resize(top_src, top_span, top_h), (c_w, 0))

        # Bottom edge band
        if top_span > 0:
            canvas.paste(_resize(bot_src, top_span, bot_h), (c_w, target_h_px - bot_h))
        ...
        if side_span_h > 0:
            canvas.paste(_resize(lft_src, l_w, side_span_h), (0, side_top))
            canvas.paste(_resize(rgt_src, r_w, side_span_h), (target_w_px - r_w, side_top))
