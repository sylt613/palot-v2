...
def _build_frame_image(target_w_px, target_h_px, intensity=1.0):
    ...
    Corners are cropped from the source and rendered at a generous minimum size
    so scroll curl detail is always visible.  Edge bands are TILED (not stretched)
    from a 1-pixel-wide/tall representative slice taken from a position safely
    past the corner ornaments.
    Returns an RGB PIL Image (white background), or None on failure.

    Source frame (986×645 after crop from frame_master.png):
      Corner scroll extends: ~75px wide × ~83px tall
      Top band: 52px tall, with triple rule at rows 34-51
      Bottom band: 36px tall, single rule at row 609 (=FH-36)
      Left band: 25px wide, double vertical rules at cols 8-11 & 17-19
      Right band: 25px wide, double vertical rules at cols 966-969 & 975-977
    Tile source columns/rows (safely past corner ornaments):
      Horizontal bands: 1px column at col=200
      Vertical bands:   1px row at row=322
    """
    ...
    try:
        ...
        # ── Source measurements ──
        CORNER_SRC   = 75    # px — horizontal extent of scroll ornament
        CORNER_H_SRC = 83    # px — vertical extent of scroll ornament
        TOP_H  = 52          # px — top rule band height in source
        BOT_H  = 36          # px — bottom rule band height in source
        L_W    = 25          # px — left side band width in source
        R_W    = 25          # px — right side band width in source

        # Tile slice source positions (past corners, in uniform rule region)
        TILE_COL = 200       # column for top/bottom 1px slice
        TILE_ROW = 322       # row for left/right 1px slice

        # ── Corner output sizing ──
        # Generous minimum so scroll curls are large and visible.
        # Cap at 40% of target dimension to leave room for bands.
        CORNER_OUT_MIN = 70
        c_w = max(CORNER_OUT_MIN, min(round(CORNER_SRC * target_w_px / FW), target_w_px * 2 // 5))
        c_h = max(CORNER_OUT_MIN, min(round(CORNER_H_SRC * target_h_px / FH), target_h_px * 2 // 5))

        # ── Edge band thicknesses — scale proportionally, with minimum ──
        ...
        def _tile_h(slice_1px, span_w, band_h):
            ...
            # slice_1px is 1×src_h; resize to 1×band_h first
        ...
        def _tile_v(slice_1px, band_w, span_h):
            ...
            # slice_1px is src_w×1; resize to band_w×1 first
        ...
        # ── Corners: crop from source, resize to output size ──
        tl = _resize(frame.crop((0,              0,               CORNER_SRC,    CORNER_H_SRC)), c_w, c_h)
        tr = _resize(frame.crop((FW-CORNER_SRC,  0,               FW,            CORNER_H_SRC)), c_w, c_h)
        bl = _resize(frame.crop((0,              FH-CORNER_H_SRC, CORNER_SRC,    FH)),           c_w, c_h)
        br = _resize(frame.crop((FW-CORNER_SRC,  FH-CORNER_H_SRC, FW,           FH)),           c_w, c_h)

        # ── 1px tile slices from uniform regions ──
        # Top band: 1px column at TILE_COL, rows 0..TOP_H
        top_slice = frame.crop((TILE_COL, 0,        TILE_COL + 1, TOP_H))
        # Bottom band: 1px column at TILE_COL, rows FH-BOT_H..FH
        bot_slice = frame.crop((TILE_COL, FH-BOT_H, TILE_COL + 1, FH))
        # Left band: 1px row at TILE_ROW, cols 0..L_W
        lft_slice = frame.crop((0,      TILE_ROW, L_W,  TILE_ROW + 1))
        # Right band: 1px row at TILE_ROW, cols FW-R_W..FW
        ...
        # Horizontal span between corners
        top_span = target_w_px - 2 * c_w

        # Top edge band (tiled)
        if top_span > 0:
            canvas.paste(_tile_h(top_slice, top_span, top_h), (c_w, 0))

        # Bottom edge band (tiled)
        if top_span > 0:
            canvas.paste(_tile_h(bot_slice, top_span, bot_h), (c_w, target_h_px - bot_h))

        # Side bands (between corner rows, tiled)
        side_top    = c_h
        side_bot    = target_h_px - c_h
        ...
        # Corners drawn last (on top of edge bands)
        canvas.paste(tl, (0,                 0))
        canvas.paste(tr, (target_w_px - c_w, 0))
        canvas.paste(bl, (0,                 target_h_px - c_h))
        canvas.paste(br, (target_w_px - c_w, target_h_px - c_h))
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Vertically centre text in frame: baseline = frame centre + half cap height
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # Vertical centre: place text block in the middle of the frame height.
        # baseline of first line = frame centre + half text block height - descender room
        frame_cy = frame_bot + frame_bh / 2          # vertical centre of frame in PDF coords
