...
def _build_frame_image(target_w_px, target_h_px, intensity=1.0):
    ...
    are TILED (not stretched) using a 1-pixel-wide/tall slice taken from
    a column/row safely inside the uniform-rule region (past corners).
    The tile slice and corner crop share the SAME boundary column/row so
    they join seamlessly.

    Source frame (986×645 after crop from frame_master.png):
      Pixel analysis of transition zones:
        Top-left  : scroll curls end col 74; all 3 horizontal rules present
                    and no scroll above from col 78 onward. Clean from row 87
                    downward for left vertical rules.
        Bottom-left: scroll curls end col 73; bottom rule clean from col 74.
                    Bottom corner scroll starts ~row 557 from top (=88px from bottom).
        Right side : mirrors left (FW-78 = 908 for clean region start).

    Corner source crops (generous, include full scroll + rule overlap):
        Horizontal: CX = 80px from each side edge
        Vertical top: CY_TOP = 90px from top
        Vertical bot: CY_BOT = 100px from bottom

    Tile source: 1px slice at col 200 (horiz) or row 322 (vert), well inside
    the uniform region.  The band heights/widths match the FULL source band
    thickness (TOP_H=52 from top edge, BOT_H=52 from bottom edge, L_W/R_W=25).
    """
    ...
    try:
        ...
        # These MUST be large enough to include the full scroll ornament
        # plus where the rules overlap with the curl, so the boundary
        # between corner and tiled band is seamless.
        ...
        # Band thicknesses in source (for the tiled 1px slice)
        TOP_H = 52       # top rule band: rows 0..52
        BOT_H = 52       # bottom rule band: rows FH-52..FH (captures the rule at row 609 plus margin)

        # Tile slice source positions (well past corner ornaments)
        TILE_COL = 200
        TILE_ROW = 322

        # ── Output corner sizing ──
        # Scale corners proportionally but enforce a generous minimum so
        # scroll curls are large and visible.  Cap at 40% of target.
        CORNER_OUT_MIN_W = 75
        CORNER_OUT_MIN_H = 80
        c_w   = max(CORNER_OUT_MIN_W, min(round(CX     * target_w_px / FW), target_w_px * 2 // 5))
        c_h_t = max(CORNER_OUT_MIN_H, min(round(CY_TOP * target_h_px / FH), target_h_px * 2 // 5))
        c_h_b = max(CORNER_OUT_MIN_H, min(round(CY_BOT * target_h_px / FH), target_h_px * 2 // 5))

        # ── Output band thicknesses ──
        scale = min(target_w_px / FW, target_h_px / FH)
        top_h = max(6, round(TOP_H * scale))
        bot_h = max(6, round(BOT_H * scale))
        ...
        # ── 1px tile slices from uniform-rule regions ──
        ...
        # Top edge band (tiled between corners)
        if h_span > 0:
            canvas.paste(_tile_h(top_slice, h_span, top_h), (c_w, 0))

        # Bottom edge band (tiled between corners)
        if h_span > 0:
            canvas.paste(_tile_h(bot_slice, h_span, bot_h), (c_w, target_h_px - bot_h))

        # Left side band (tiled between top and bottom corners)
        side_top    = c_h_t
