...
def draw_frame_box(c, x_left, y_bot, width_pt, height_pt, intensity=1.0, png_path=None):
    """Draw the ornamental scroll frame at any size using tiled divider strips
    and a corner ornament placed at each corner.
    png_path parameter is accepted but ignored (legacy compat — strip+corner
    approach is always used now).
    x_left, y_bot are PDF coordinates (bottom-left of frame). Returns True on success."""
    ppi = 150   # 150 dpi is plenty for a frame border; keeps PDF small
    w_px = max(8, round(width_pt  * ppi / 72))
    h_px = max(8, round(height_pt * ppi / 72))
    img = _build_strip_frame(w_px, h_px, intensity=intensity)
    if img is None:
        return False
    buf = io.BytesIO()
