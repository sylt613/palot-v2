...
def draw_frame_box(c, x_left, y_bot, width_pt, height_pt, intensity=1.0, png_path=None):
    """Draw the ornamental scroll frame at any size.
    png_path: pre-generated PNG drawn directly (stretched to fit the box).
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    print(f"  [DEBUG] TOC frame box: {title_frame_w:.1f} x {title_frame_h:.1f} pt  ratio={title_frame_w/title_frame_h:.2f}:1")
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        print(f"  [DEBUG] Mamar frame box: {frame_bw:.1f} x {frame_bh:.1f} pt  ratio={frame_bw/frame_bh:.2f}:1")
