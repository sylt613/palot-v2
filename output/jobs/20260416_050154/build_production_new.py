...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Pre-compute fixed dot_end based on widest name so all rows end at same x
    _all_nw = [Wid(vis(livorna_fix_quotes(strip_nikud(nm))), HEADING_BOLD_FONT, TOC_ENTRY_FS)
               for _, nm, _ in anaf_entries]
    _max_nw = max(_all_nw) if _all_nw else 0
    common_dot_end = name_right_x - min(_max_nw, name_avail_w) - dot_w * 2
    ...
    for label_str, name_str, pg in anaf_entries:
        ...
        # Clamp dot_end per row so dots never run into this row's name
        row_name_left = name_right_x - min(nw, name_avail_w)
        dot_end = min(common_dot_end, row_name_left - dot_w * 2)
        dot_x = dot_start
