...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    _max_nw = max(_all_first_nw) if _all_first_nw else 0
    common_dot_end = name_right_x - _max_nw - dot_w * 2
    ...
    for label_str, name_str, pg in anaf_entries:
        ...
        # Dot leaders between page number and fixed stop (same for all rows)
        row_first_w = _first_line_w(name_v)
        dot_start = pgnum_x + pw + dot_w * 2
        # Clamp per-row so dots never collide with this row's first-line text
        dot_end = min(common_dot_end, name_right_x - row_first_w - dot_w * 2)
        dot_x = dot_start
