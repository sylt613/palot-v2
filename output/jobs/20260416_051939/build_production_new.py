...
H1_TOP_GAP = 0.0                  # no extra top gap — header-to-body padding handles the spacing
...
def _draw_header_band_leader(c, y, x_left, x_right):
    ...
    if reader is None:
        ...
        if band_img is None:
            return
        # Lighten the band toward white (same as frame intensity)
        _BAND_INTENSITY = 0.72
        r, g, b, a = band_img.split()
        lut = [int(v * _BAND_INTENSITY + 255 * (1 - _BAND_INTENSITY)) for v in range(256)]
        r = r.point(lut); g = g.point(lut); b = b.point(lut)
        band_img = Image.merge('RGBA', (r, g, b, a))
