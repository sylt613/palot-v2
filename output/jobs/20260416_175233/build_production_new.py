...
def _draw_anaf_flanker(c, x_left, x_right, y_center, side='right'):
    ...
    if reader is None:
        ...
        try:
            ...
            if tile is None:
                return
            # Build with curl on right end, then position curl near label
            tiled = _build_tiled_leader(tile, flourish, needed_w_px, canvas_h_px)
            if side == 'right':
                # Curl near label = curl on LEFT, mirror
                tiled = tiled.transpose(Image.FLIP_LEFT_RIGHT)
            # side=='left': curl already on right end (near label), no mirror
            # No fade -- tile runs to the far end cleanly
...
def draw_running_header_odd_full(c, page_num, anaf_label, anaf_name):
    ...
    header_y = C_TOP - 2
    _rule_render_h = 9.0
    rule_y = header_y + _rule_render_h / 2  # bottom of rule flush with page-number baseline
...
def draw_running_header_even(c, page_num, anaf_label, anaf_name):
    ...
    # Leader: curly ornament, bottom flush with page-number baseline.
    _rule_render_h = 9.0
    rule_y = header_y + _rule_render_h / 2
