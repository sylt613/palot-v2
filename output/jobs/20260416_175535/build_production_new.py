...
def _draw_anaf_flanker(c, x_left, x_right, y_center, side='right'):
    """Draw a horizontal flanker: full curly image near the anaf label, 1px tiling extending
    outward away from label, fading out at the far end.
    side='right': flanker is to the RIGHT of the label — curl faces LEFT (near label).
    side='left' : flanker is to the LEFT  of the label — curl faces RIGHT (near label).
    """
    render_h = 12.0
    ppi = 300
    ...
    if reader is None:
        ...
        try:
            ...
            # Scale source image to canvas height
            src_h = img.height
            scale = canvas_h_px / src_h
            src_w_scaled = max(1, round(img.width * scale))
            img_scaled = img.resize((src_w_scaled, canvas_h_px), Image.LANCZOS)

            # The source image has its curl on the RIGHT end.
            # For side='left' (curl near label on right): use as-is — curl already on right.
            # For side='right' (curl near label on left): flip horizontally so curl is on left.
            if side == 'right':
                img_scaled = img_scaled.transpose(Image.FLIP_LEFT_RIGHT)

            # Clamp curl image width to at most 60% of total span
            curl_w = min(src_w_scaled, max(1, int(needed_w_px * 0.6)))
            curl_img = img_scaled.crop((0, 0, curl_w, canvas_h_px))

            # Extract 1px tile from the straight end of the (pre-flip) source
            # After possible flip, the straight end is at the FAR side (away from label).
            # Take the 1px column at the far edge of the scaled image.
            tile_x = src_w_scaled - 1 if side == 'right' else 0
            tile_col = img_scaled.crop((tile_x, 0, tile_x + 1, canvas_h_px))

            # Build canvas: [curl near label] + [1px tiled body extending outward]
            canvas = Image.new("RGBA", (needed_w_px, canvas_h_px), (255, 255, 255, 0))
            body_start = curl_w
            body_w = max(0, needed_w_px - curl_w)

            if side == 'right':
                # Curl on LEFT, tile extends RIGHT then fades at far right
                canvas.alpha_composite(curl_img, (0, 0))
                for bx in range(body_start, needed_w_px):
                    canvas.alpha_composite(tile_col, (bx, 0))
                # Fade out the far (right) end
                fade_zone = max(10, body_w // 3)
                canvas = _fade_rgba_alpha(canvas, fade_side='right', fade_zone_px=fade_zone)
            else:
                # Curl on RIGHT, tile extends LEFT then fades at far left
                curl_x = needed_w_px - curl_w
                canvas.alpha_composite(curl_img, (curl_x, 0))
                for bx in range(0, curl_x):
                    canvas.alpha_composite(tile_col, (bx, 0))
                # Fade out the far (left) end
                fade_zone = max(10, (needed_w_px - curl_w) // 3)
                canvas = _fade_rgba_alpha(canvas, fade_side='left', fade_zone_px=fade_zone)
...
DC_FONT = S.BOLD_BODY_FONT_NAME   # first word uses bold body font (1CarizmaBold)
