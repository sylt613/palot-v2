...
def _draw_anaf_flanker(c, x_left, x_right, y_center, side='right'):
    """Draw flanker ornament: curl near the anaf (center), fades toward outer edge.
    No tiling — full image scaled to span, gray tint, fade at far end.
    side='right': flanker right of label, curl faces LEFT (toward center).
    side='left' : flanker left  of label, curl faces RIGHT (toward center).
    """
    render_h = 18.0
    ppi = 300
    ...
    if reader is None:
        ...
        try:
            ...
            # Scale to canvas height, stretch width to fill span
            img_scaled = img.resize((needed_w_px, canvas_h_px), Image.LANCZOS)

            # Source image has curl at one end. For side='left' (curl near label on right): flip.
            # For side='right' (curl near label on left): no flip.
            if side == 'left':
                img_scaled = img_scaled.transpose(Image.FLIP_LEFT_RIGHT)

            # Gray tint: blend toward gray at 80% intensity
            gray_val = 80  # 0=black; 80 = fairly dark gray
            arr = img_scaled.split()
            r, g, b, a = arr
            lut = [int(v * 0.2 + gray_val * 0.8) for v in range(256)]
            r = r.point(lut); g = g.point(lut); b = b.point(lut)
            img_scaled = Image.merge('RGBA', (r, g, b, a))

            # Fade toward the outer edge (far from label)
            fade_zone = max(12, needed_w_px // 3)
            if side == 'right':
                img_scaled = _fade_rgba_alpha(img_scaled, fade_side='right', fade_zone_px=fade_zone)
            else:
                img_scaled = _fade_rgba_alpha(img_scaled, fade_side='left', fade_zone_px=fade_zone)

            flat = Image.new("RGB", img_scaled.size, (255, 255, 255))
            flat.paste(img_scaled, mask=img_scaled.split()[3])
            reader = ImageReader(flat)
