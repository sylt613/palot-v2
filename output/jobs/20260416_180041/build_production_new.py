...
def _draw_anaf_flanker(c, x_left, x_right, y_center, side='right'):
    """Flanker ornament: full curl image (aspect-ratio locked) near anaf, tiny 1px
    tile extension at far end, fading out. Slightly gray (80% opacity on white).
    side='right': right of label, curl on LEFT (near center).
    side='left' : left  of label, curl on RIGHT (near center).
    """
    render_h = 16.0        # a bit bigger than before, height in pts
    ppi = 300
    ...
    if reader is None:
        ...
        try:
            ...
            # Scale keeping aspect ratio — height = canvas_h_px
            src_scale = canvas_h_px / img.height
            curl_w_px = max(1, round(img.width * src_scale))
            img_scaled = img.resize((curl_w_px, canvas_h_px), Image.LANCZOS)

            # side='right': curl near label = curl on LEFT → flip
            # side='left' : curl near label = curl on RIGHT → no flip
            if side == 'right':
                img_scaled = img_scaled.transpose(Image.FLIP_LEFT_RIGHT)

            # Gray: reduce alpha to 80% so it reads as soft gray on white
            ...
            if curl_w_px >= needed_w_px:
                # Curl image fills the whole span — just crop
                canvas.alpha_composite(img_scaled.crop((0, 0, needed_w_px, canvas_h_px)), (0, 0))
            else:
                # Place full curl image at the inner (label) side
                canvas.alpha_composite(img_scaled, (0, 0))

                # Tile the outermost 1px of the curl image to fill remaining span
                far_col_x = curl_w_px - 1 if side == 'right' else 0
                tile_col = img_scaled.crop((far_col_x, 0, far_col_x + 1, canvas_h_px))
                tail_w = needed_w_px - curl_w_px
                for bx in range(curl_w_px, needed_w_px):
                    canvas.alpha_composite(tile_col, (bx, 0))

                # Fade the tiled tail out toward the far edge
                fade_zone = max(8, tail_w)
                canvas = _fade_rgba_alpha(canvas, fade_side='right', fade_zone_px=fade_zone)

            if side == 'left':
                # Mirror the whole canvas so curl ends up on the right
