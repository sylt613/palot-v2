...
def _draw_anaf_flanker(c, x_left, x_right, y_center, side='right'):
    ...
    if reader is None:
        ...
        try:
            ...
            # Source curl is on the LEFT. Straight end is on the RIGHT.
            # side='right': curl must be at LEFT of canvas (near label) → no flip
            # side='left' : curl must be at RIGHT of canvas (near label) → flip
            if side == 'left':
                img_sc = img_sc.transpose(Image.FLIP_LEFT_RIGHT)
                # Now curl on RIGHT, straight on LEFT
                ...
                # side='right': curl on LEFT end of canvas (near label), tile extends RIGHT
