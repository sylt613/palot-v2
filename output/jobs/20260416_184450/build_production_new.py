...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        try:
            ...
            canvas = Image.new('RGB', (needed_w_px, band_h_px), (255, 255, 255))
            # Center the ornament horizontally
            paste_x = max(0, (needed_w_px - new_w) // 2)
            # Alpha → gray ink: dark pixels from source become gray ~130
            arr = np.array(src, dtype=np.float32)
            alpha = arr[:, :, 3] / 255.0
            # Ink intensity: 0.72 (matching header ornament)
            ink_gray = 255 - alpha * (255 - 130)
