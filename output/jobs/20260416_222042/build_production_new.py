...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    The ornament is rendered gray with a soft halo + lighter gray outline,
    matching the header ornament style.  y_top is the top edge of the band."""
    render_h = 22.0   # rendered height in points
    ppi = 300
    ...
    if cached is None:
        ...
        try:
            ...
            # Build RGB canvas (white background)
            # 253 not 255 so _apply_gray_halo condition (gpix < 254) passes on bg pixels
            canvas = Image.new('RGB', (needed_w_px, band_h_px), (253, 253, 253))
            # Center the ornament horizontally
            paste_x = max(0, (needed_w_px - new_w) // 2)
            # Alpha → gray ink: dark core at 75 so halo (max 120) is visibly lighter
            arr = np.array(src, dtype=np.float32)
            alpha = arr[:, :, 3] / 255.0
            ink_gray = 255 - alpha * (255 - 75)   # 75 = dark gray core
            ink_img = Image.fromarray(ink_gray.clip(0, 255).astype(np.uint8), 'L')
            rgb_orn = Image.merge('RGB', (ink_img, ink_img, ink_img))
            # Paste onto white canvas using alpha as mask
            alpha_mask = Image.fromarray((alpha * 255).clip(0,255).astype(np.uint8), 'L')
            canvas.paste(rgb_orn, (paste_x, 0), mask=alpha_mask)
            # Apply halo (soft gray glow + lighter gray outline)
            canvas = _apply_gray_halo(canvas)
            ...
            return False
    c.saveState()
    c.drawImage(cached, x_center - width / 2, y_top - render_h,
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # ── Mamar label + name inside curl-ornament box ───────────────────────
    # Curl ornament bands (from good curls svg.svg):
    #   top ornament = line 3,  bottom ornament = line 2
    # The text block sits between the two ornament band centres.
    # A rounded rect is drawn first (stroke only, no fill); its top/bottom edges
    # are hidden behind the ornament bands so only the vertical sides + corners show.
    curl_orn_h   = 28.0         # rendered height of each ornament band (pts)
    curl_text_gap = 4.0         # gap between ornament band edge and text cap-top
    ...
    if MAMAR_NAME:
        ...
        # Top ornament centre: anchor from title
        top_orn_y   = title_orn_y - TOC_TITLE_FS * 1.4
        # Text positions: label just below top ornament band bottom
        label_y     = top_orn_y - curl_orn_h / 2.0 - curl_text_gap - label_cap_h
        name_y      = label_y - mamar_gap - name_cap_h
        # Bottom ornament centre: just below name baseline
        bot_orn_y   = name_y - curl_text_gap - curl_orn_h / 2.0

        # ── 1. Rounded rect (drawn first, behind ornaments) ──────────────
        rect_x      = mamar_cx - mamar_rule_w / 2.0
        rect_y      = bot_orn_y   # bottom of box = bottom ornament centre
        rect_h      = top_orn_y - bot_orn_y
        c.saveState()
        c.setStrokeColorRGB(0.55, 0.55, 0.55)
        c.setLineWidth(0.7)
        c.roundRect(rect_x, rect_y, mamar_rule_w, rect_h, radius=8, stroke=1, fill=0)
        c.restoreState()
        ...
        # ── 3. Curl ornament lines (drawn on top, hide rect's horiz edges) ─
        _draw_curl_orn_line(c, 3, mamar_cx, top_orn_y, mamar_rule_w, curl_orn_h)
        _draw_curl_orn_line(c, 2, mamar_cx, bot_orn_y, mamar_rule_w, curl_orn_h)

        mamar_bottom_y = bot_orn_y - curl_orn_h / 2.0
    else:
