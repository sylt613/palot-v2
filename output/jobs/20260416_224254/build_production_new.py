...
def _apply_white_gray_rings(canvas, near_dil=4, far_dil=10,
                             white_val=248, gray_val=178):
    """Two-ring outline effect matching even-page header glow style.

    Applies to an RGB canvas (white background, dark ink):
      - White ring  : pixels in near-dilation zone (not ink) → near-white
      - Gray ring   : pixels in far-dilation zone only (not ink, not white ring) → medium gray

    near_dil / far_dil are MaxFilter kernel sizes (odd integers ≥ 3).
    Larger = thicker ring.
    """
    gray_l = canvas.convert('L')
    ink_mask = gray_l.point(lambda p: 255 if p < _INK_THRESHOLD else 0)
    # Dilate via MaxFilter (approximates morphological dilation)
    near_dil_k = near_dil if near_dil % 2 == 1 else near_dil + 1
    far_dil_k  = far_dil  if far_dil  % 2 == 1 else far_dil  + 1
    near_zone  = ink_mask.filter(ImageFilter.MaxFilter(near_dil_k))
    far_zone   = ink_mask.filter(ImageFilter.MaxFilter(far_dil_k))
...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    """Draw the post-anaf section divider using the new end ornament.

    Rendering: gray ink core, white outline ring, gray outer ring — exactly
    like the even-page header title glow (_draw_title_with_glow).
    Canvas is padded vertically so the rings are never cropped at the top/bottom.
    y_top is the top edge of the visible band."""
    ...
    # Vertical padding so white/gray rings are never clipped at canvas edge
    pad_v_px    = 16
    ...
    if cached is None:
        ...
        try:
            ...
            # Build padded RGB canvas (pure white — will be masked transparent)
            canvas = Image.new('RGB', (needed_w_px, canvas_h_px), (255, 255, 255))

            # Alpha → gray ink: dark core
            arr = np.array(src, dtype=np.float32)
            alpha = arr[:, :, 3] / 255.0
            ink_gray = 255 - alpha * (255 - 72)   # 72 = dark gray core
            ...
            # Center ornament horizontally; paste at vertical offset = pad_v_px
            paste_x = max(0, (needed_w_px - new_w) // 2)
            canvas.paste(rgb_orn, (paste_x, pad_v_px), mask=alpha_mask)

            # White ring then gray ring — like header title glow
            canvas = _apply_white_gray_rings(canvas, near_dil=5, far_dil=13,
                                             white_val=250, gray_val=175)
            cached = ImageReader(canvas)
            _POST_ANAF_DIV_CACHE[cache_key] = cached
        except Exception:
            return False

    # Draw the padded canvas: shift y_bottom down by pad_v_px worth of pts,
    # and use the full padded height so rings outside the ornament are visible.
    pad_v_pt    = pad_v_px * 72.0 / ppi
    total_h_pt  = render_h + 2 * pad_v_pt
    draw_y_bot  = y_top - render_h - pad_v_pt   # bottom of padded canvas on page
    c.saveState()
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # Design: open-top U-shape path (sides + bottom corners + bottom line).
        # Top curl ornament (SVG line 3) covers the open top edge.
        # Bottom curl ornament (SVG line 2) covers the middle of the bottom line,
        # leaving only the corner arcs + a stub of the bottom line visible.
        # Ornament height derived from natural band aspect ratio, clamped to range.
        _band3_rl_h = 509.5 - 393.5   # RL height of line 3 band (~116)
        _band2_rl_h = 742.0 - 613.5   # RL height of line 2 band (~128.5)
        curl_orn_h_top = min(30.0, max(16.0, frame_bw * _band3_rl_h / 1024.0))
        curl_orn_h_bot = min(30.0, max(16.0, frame_bw * _band2_rl_h / 1024.0))
        ...
        # ── 1. U-shape path: left side → BL corner → bottom → BR corner → right side ──
        c.saveState()
        c.setStrokeColorRGB(0.52, 0.52, 0.52)
        c.setLineWidth(0.75)
        p = c.beginPath()
        p.moveTo(x_left, frame_top)           # top-left — open, no line drawn here yet
        p.lineTo(x_left, frame_bot + r)       # left side down to BL arc start
        p.curveTo(x_left,           frame_bot + r*(1-k),   # BL corner
                  x_left + r*(1-k), frame_bot,
                  x_left + r,       frame_bot)
        p.lineTo(x_right - r,       frame_bot)             # bottom line
        p.curveTo(x_right - r*(1-k), frame_bot,            # BR corner
                  x_right,           frame_bot + r*(1-k),
                  x_right,           frame_bot + r)
        p.lineTo(x_right, frame_top)          # right side up — open top
        c.drawPath(p, fill=0, stroke=1)
        c.restoreState()
        ...
        # ── 3. Curl ornaments (drawn last, cover horizontal edges) ────────────
        _draw_curl_orn_line(c, 3, x_center, frame_top, frame_bw, curl_orn_h_top)
        _draw_curl_orn_line(c, 2, x_center, frame_bot, frame_bw, curl_orn_h_bot)
