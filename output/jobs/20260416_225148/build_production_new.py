...
def _apply_white_gray_rings(canvas, near_dil=5, gray_blur_r=28,
                             white_val=250, gray_darkest=172):
    """Two-ring outline: crisp white ring close to ink, soft gray gradient far out.

    - White ring : pixels within near_dil MaxFilter zone → white_val (hard edge)
    - Gray gradient: Gaussian blur of ink mask at gray_blur_r radius; outside white
      ring the gray value grades from gray_darkest (at white ring edge) → white.
    Priority: ink > white ring > gray gradient.
    """
    gray_l   = canvas.convert('L')
    ink_mask = gray_l.point(lambda p: 255 if p < _INK_THRESHOLD else 0)

    # Near zone — hard dilation for crisp white ring
    near_k    = near_dil if near_dil % 2 == 1 else near_dil + 1
    near_zone = ink_mask.filter(ImageFilter.MaxFilter(near_k))

    # Far zone — Gaussian blur for soft gray gradient
    blurred   = ink_mask.filter(ImageFilter.GaussianBlur(radius=gray_blur_r))
    ...
    for y in range(h):
        for x in range(w):
            if ipix[x, y] > 128:
                continue                        # ink pixel — leave as-is
            if npix[x, y] > 128:
                cpix[x, y] = (white_val, white_val, white_val)  # white ring
            else:
                bv = bpix[x, y]
                if bv > 4:                      # in gray gradient zone
                    nv = int(255 - (bv / 255.0) * (255 - gray_darkest))
                    cpix[x, y] = (nv, nv, nv)
    return canvas
...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        try:
            ...
            canvas = _apply_white_gray_rings(canvas, near_dil=19, gray_blur_r=28,
                                             white_val=250, gray_darkest=172)
...
def _get_curl_drawing():
    ...
    if drawing is None:
        _CURL_DRAWING_CACHE['drawing'] = None
        return None
    _curl_gray_shapes(drawing, gray=0.45)
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Draw frame FIRST, then text on top so text is never obscured
    draw_frame_box(c, title_frame_left, title_frame_bot, title_frame_w, title_frame_h, intensity=0.80,
                   png_path=os.path.join(_ORNAMENT_DIR, 'frame_toc_nb2.png'))
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # ── 1. Closed rounded rect (full box outline) ────────────────────────
        c.saveState()
        c.setStrokeColorRGB(0.52, 0.52, 0.52)
        c.setLineWidth(0.75)
        p = c.beginPath()
        # Start at top-left corner arc end, go clockwise (RTL book — mirrors nicely)
        p.moveTo(x_left + r, frame_top)
        p.lineTo(x_right - r, frame_top)                              # top line →
        p.curveTo(x_right - r*(1-k), frame_top,                       # TR corner
                  x_right,           frame_top - r*(1-k),
                  x_right,           frame_top - r)
        p.lineTo(x_right, frame_bot + r)                              # right side ↓
        p.curveTo(x_right,           frame_bot + r*(1-k),             # BR corner
                  x_right - r*(1-k), frame_bot,
                  x_right - r,       frame_bot)
        p.lineTo(x_left + r, frame_bot)                               # bottom line ←
        p.curveTo(x_left + r*(1-k), frame_bot,                        # BL corner
                  x_left,           frame_bot + r*(1-k),
                  x_left,           frame_bot + r)
        p.lineTo(x_left, frame_top - r)                               # left side ↑
        p.curveTo(x_left,           frame_top - r*(1-k),              # TL corner
