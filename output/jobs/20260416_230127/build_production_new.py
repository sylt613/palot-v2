...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        try:
            ...
            canvas = _apply_white_gray_rings(canvas, near_dil=19, far_dil=27,
                                             white_val=250, gray_val=172)
...
def _curl_gray_shapes(node, gray=0.45):
    """Recursively recolor black fills/strokes to gray; leave white fills alone."""
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    c.drawPath(_toc_rrect_path(c), fill=0, stroke=1)
    c.restoreState()
    # White backing rects then curl ornaments on top/bottom edges
    for _orn_cy, _orn_h in ((_toc_ftop, _toc_orn_h_top), (_toc_fbot, _toc_orn_h_bot)):
        c.saveState()
        c.setFillColorRGB(1, 1, 1)
        c.rect(title_cx - _toc_orn_w / 2, _orn_cy - _orn_h / 2, _toc_orn_w, _orn_h, fill=1, stroke=0)
        c.restoreState()
    _draw_curl_orn_line(c, 3, title_cx, _toc_ftop, _toc_orn_w, _toc_orn_h_top)
    _draw_curl_orn_line(c, 2, title_cx, _toc_fbot, _toc_orn_w, _toc_orn_h_bot)
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # Pass 2: white ring (creates separation between halo and dark core)
        c.setStrokeColorRGB(1.0, 1.0, 1.0)
        c.setLineWidth(1.8)
        c.drawPath(_rrect_path(c), fill=0, stroke=1)
        ...
        # White backing rect first so ornament hides the line segment beneath it
        for orn_cy, orn_h in ((frame_top, curl_orn_h_top), (frame_bot, curl_orn_h_bot)):
            c.saveState()
            c.setFillColorRGB(1, 1, 1)
            c.rect(x_center - orn_w / 2, orn_cy - orn_h / 2, orn_w, orn_h, fill=1, stroke=0)
            c.restoreState()
        _draw_curl_orn_line(c, 3, x_center, frame_top, orn_w, curl_orn_h_top)
        _draw_curl_orn_line(c, 2, x_center, frame_bot, orn_w, curl_orn_h_bot)
