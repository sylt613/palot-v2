...
def _apply_white_gray_rings(canvas, near_dil=19, far_dil=27,
                             white_val=250, gray_val=172):
    """Two-ring outline: thick white ring tight against ink, thin gray ring just outside.

    Both rings use MaxFilter dilation (crisp, not diffuse).
    near_dil → white ring width in pixels from ink edge.
    far_dil  → gray ring outer boundary; gray occupies (near_dil .. far_dil).
    ...
    far_k     = far_dil  if far_dil  % 2 == 1 else far_dil  + 1
    near_zone = ink_mask.filter(ImageFilter.MaxFilter(near_k))
    far_zone  = ink_mask.filter(ImageFilter.MaxFilter(far_k))
    ...
    for y in range(h):
        for x in range(w):
            ...
            if npix[x, y] > 128:
                ...
                cpix[x, y] = (gray_val,  gray_val,  gray_val)
    return canvas
...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        try:
            ...
            canvas = _apply_white_gray_rings(canvas, near_dil=10, far_dil=15,
                                             white_val=250, gray_val=172)
...
def _get_curl_drawing():
    """Load and cache the curl SVG as a reportlab Drawing (gray-colored)."""
    if 'drawing' in _CURL_DRAWING_CACHE:
        return _CURL_DRAWING_CACHE['drawing']
    path = _CURL_SVG_PATH
    if not os.path.exists(path):
        _CURL_DRAWING_CACHE['drawing'] = None
        return None
    drawing = svg2rlg(path)
    if drawing is None:
        _CURL_DRAWING_CACHE['drawing'] = None
        return None
    _curl_gray_shapes(drawing, gray=0.22)
    _CURL_DRAWING_CACHE['drawing'] = drawing
    return drawing


def _curl_gray_shapes(node, gray=0.45):
    """Recursively recolor black fills/strokes to gray; thicken strokes; leave white fills alone."""
    from reportlab.graphics.shapes import Group, Shape
    from reportlab.lib.colors import Color, white, black
    gray_c = Color(gray, gray, gray)
    _black_thresh = 0.15   # treat very dark colors as "black"
    _stroke_scale = 3.0    # multiply existing strokeWidth by this factor
    ...
    def _is_black(c):
        ...
        try:
            ...
            return False
        return r < _black_thresh and g < _black_thresh and b < _black_thresh

    def _recurse(node):
        # Process shape attributes
        for attr in ('fillColor', 'strokeColor'):
            val = getattr(node, attr, None)
            if val is not None and _is_black(val):
                setattr(node, attr, gray_c)
        # Thicken stroke
        sw = getattr(node, 'strokeWidth', None)
        if sw is not None and sw > 0:
            node.strokeWidth = sw * _stroke_scale
...
def _draw_curl_orn_line(c, line_id, x_center, y_center_page, target_w, target_h):
    """Draw curl ornament line (line_id=2 or 3) centered at (x_center, y_center_page).

    Clips the relevant band from the 1024×1024 curl drawing and scales it to
    (target_w × target_h) on the page.
    """
    drawing = _get_curl_drawing()
    if drawing is None:
        return False
    ...
    # Scale factors: map drawing 1024×band_rl_h → target_w×target_h on page
    sx = target_w / 1024.0
    sy = target_h / band_rl_h

    # Translation: drawing origin (0,0) maps to page origin after transform
    # We want: page_x = sx * draw_x + tx  →  tx = x_left = x_center - target_w/2
    # We want: page_y = sy * draw_y + ty  such that draw_y=rl_y_bot → page y = y_center_page - target_h/2
