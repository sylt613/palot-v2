...
def _apply_white_gray_rings(canvas, near_dil=6, far_blur=8,
                             white_val=250, gray_val=172):
    """Thin white ring tight against ink, then smooth Gaussian gray halo outside it.

    near_dil → MaxFilter radius for white ring (crisp, tight).
    far_blur  → GaussianBlur radius for the gray halo (smooth, not cropped).
    Priority: ink > white > gray.
    """
    gray_l   = canvas.convert('L')
    ink_mask = gray_l.point(lambda p: 255 if p < _INK_THRESHOLD else 0)

    near_k    = near_dil if near_dil % 2 == 1 else near_dil + 1
    near_zone = ink_mask.filter(ImageFilter.MaxFilter(near_k))

    # Gray halo: blur the ink mask, then subtract white-ring area
    gray_blur = ink_mask.filter(ImageFilter.GaussianBlur(radius=far_blur))

    cpix  = canvas.load()
    npix  = near_zone.load()
    gbpix = gray_blur.load()
    ipix  = ink_mask.load()
    w, h  = canvas.size
    for y in range(h):
        for x in range(w):
            if ipix[x, y] > 128:
                continue          # ink pixel — leave alone
            if npix[x, y] > 128:
                cpix[x, y] = (white_val, white_val, white_val)
            else:
                g = gbpix[x, y]  # 0–255, smooth falloff
                if g > 4:        # skip near-zero pixels
                    # Blend: gray_val at full blur, fading to white background
                    t = g / 255.0
                    v = int(255 - t * (255 - gray_val))
                    # Only paint if darker than current pixel
                    cur = cpix[x, y][0]
                    if v < cur:
                        cpix[x, y] = (v, v, v)
    return canvas
...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        try:
            ...
            canvas = _apply_white_gray_rings(canvas, near_dil=6, far_blur=8,
                                             white_val=250, gray_val=172)
...
def _get_curl_drawing_pass(pass_id):
    """Return a cached curl drawing for a specific render pass.

    pass_id: 'gray'  → outer halo  (thick stroke, light gray)
             'white' → middle ring  (medium stroke, white)
             'dark'  → core         (normal stroke, dark gray)
    """
    if pass_id in _CURL_DRAWING_CACHE:
        return _CURL_DRAWING_CACHE[pass_id]
    drawing = _load_curl_drawing_raw()
    if drawing is None:
        _CURL_DRAWING_CACHE[pass_id] = None
        return None
    if pass_id == 'gray':
        _curl_gray_shapes(drawing, gray=0.68, stroke_scale=8.0)
    elif pass_id == 'white':
        _curl_gray_shapes(drawing, gray=1.0, stroke_scale=5.0)
    else:  # 'dark'
        _curl_gray_shapes(drawing, gray=0.22, stroke_scale=3.0)
    _CURL_DRAWING_CACHE[pass_id] = drawing
    return drawing


def _get_curl_drawing():
    """Legacy helper — returns the 'dark' pass drawing."""
    return _get_curl_drawing_pass('dark')


def _curl_gray_shapes(node, gray=0.45, stroke_scale=3.0):
    """Recursively recolor black fills/strokes to a given gray; scale strokeWidths."""
    from reportlab.lib.colors import Color
    gray_c = Color(gray, gray, gray)
    _black_thresh = 0.85   # treat anything darker than this as "black" to recolor

    def _is_dark(c):
        if c is None:
            return False
        try:
            return (c.red + c.green + c.blue) / 3.0 < _black_thresh
        except Exception:
            return False
    ...
    def _recurse(n):
        for attr in ('fillColor', 'strokeColor'):
            ...
            if val is not None and _is_dark(val):
...
def _draw_curl_orn_line(c, line_id, x_center, y_center_page, target_w, target_h):
    """Draw curl ornament line with 3-pass path glow: gray halo → white ring → dark core."""
    ...
    sx = target_w / 1024.0
    sy = target_h / band_rl_h
    tx = x_center - target_w / 2.0
    ty = (y_center_page - target_h / 2.0) - sy * rl_y_bot

    for pass_id in ('gray', 'white', 'dark'):
        drawing = _get_curl_drawing_pass(pass_id)
        if drawing is None:
            continue
        c.saveState()
        cp = c.beginPath()
        cp.rect(x_center - target_w / 2.0, y_center_page - target_h / 2.0, target_w, target_h)
        c.clipPath(cp, stroke=0, fill=0)
        c.transform(sx, 0, 0, sy, tx, ty)
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Draw curl+rounded-rect frame (same style as body sefer_title box)
    _toc_band3_rl_h = 509.5 - 393.5
    _toc_band2_rl_h = 742.0 - 613.5
    _toc_orn_w      = title_frame_w * 0.5
    _toc_orn_h_top  = _toc_orn_w * _toc_band3_rl_h / 1024.0
    _toc_orn_h_bot  = _toc_orn_w * _toc_band2_rl_h / 1024.0
    _toc_orn_mask_w = _toc_orn_w * 0.80
    _toc_xl   = title_frame_left
    _toc_xr   = title_frame_left + title_frame_w
    _toc_r    = 10.0
    _toc_k    = 0.5523
    _toc_ftop = title_frame_top
    _toc_fbot = title_frame_bot
    def _toc_rrect_path(canvas):
        p = canvas.beginPath()
        p.moveTo(_toc_xl + _toc_r, _toc_ftop)
        p.lineTo(_toc_xr - _toc_r, _toc_ftop)
        p.curveTo(_toc_xr - _toc_r*(1-_toc_k), _toc_ftop,
                  _toc_xr, _toc_ftop - _toc_r*(1-_toc_k),
                  _toc_xr, _toc_ftop - _toc_r)
        p.lineTo(_toc_xr, _toc_fbot + _toc_r)
        p.curveTo(_toc_xr, _toc_fbot + _toc_r*(1-_toc_k),
                  _toc_xr - _toc_r*(1-_toc_k), _toc_fbot,
                  _toc_xr - _toc_r, _toc_fbot)
        p.lineTo(_toc_xl + _toc_r, _toc_fbot)
        p.curveTo(_toc_xl + _toc_r*(1-_toc_k), _toc_fbot,
                  _toc_xl, _toc_fbot + _toc_r*(1-_toc_k),
                  _toc_xl, _toc_fbot + _toc_r)
        p.lineTo(_toc_xl, _toc_ftop - _toc_r)
        p.curveTo(_toc_xl, _toc_ftop - _toc_r*(1-_toc_k),
                  _toc_xl + _toc_r*(1-_toc_k), _toc_ftop,
                  _toc_xl + _toc_r, _toc_ftop)
        p.close()
        return p
    c.saveState()
    c.setStrokeColorRGB(0.68, 0.68, 0.68)
    c.setLineWidth(8.0)
    c.drawPath(_toc_rrect_path(c), fill=0, stroke=1)
    ...
    c.drawPath(_toc_rrect_path(c), fill=0, stroke=1)
    c.restoreState()
    # For each ornament: white backing (narrowed), 3-pass glow rect, then SVG
    for _orn_cy, _orn_h, _lid in ((_toc_ftop, _toc_orn_h_top, 3), (_toc_fbot, _toc_orn_h_bot, 2)):
        _ox = title_cx - _toc_orn_w / 2
        _oy = _orn_cy - _orn_h / 2
        c.saveState()
        c.setFillColorRGB(1, 1, 1)
        c.rect(title_cx - _toc_orn_mask_w / 2, _oy, _toc_orn_mask_w, _orn_h, fill=1, stroke=0)
        c.restoreState()
        c.saveState()
        c.setStrokeColorRGB(0.68, 0.68, 0.68)
        c.setLineWidth(8.0)
        c.rect(_ox, _oy, _toc_orn_w, _orn_h, fill=0, stroke=1)
        c.setStrokeColorRGB(1.0, 1.0, 1.0)
        c.setLineWidth(4.5)
        c.rect(_ox, _oy, _toc_orn_w, _orn_h, fill=0, stroke=1)
        c.setStrokeColorRGB(0.22, 0.22, 0.22)
        ...
        c.rect(_ox, _oy, _toc_orn_w, _orn_h, fill=0, stroke=1)
        c.restoreState()
        _draw_curl_orn_line(c, _lid, title_cx, _orn_cy, _toc_orn_w, _orn_h)
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # ── Curl ornament box (replaces baroque frame_mamar_nb2.png) ───────
        # Design: CLOSED rounded rect as stroke path.
        # Ornaments (HALF the box width, aspect-ratio locked) centered on
        # top and bottom edges, on top of a white rect that masks the line
        # underneath.  Visible result: corner arcs + line stubs on all 4 sides,
        # with the ornament band occupying the centre of each horizontal edge.
        # ⚠️ HARD RULE: ornament width drives height via locked aspect ratio.
        _band3_rl_h = 509.5 - 393.5   # ~116 RL units
        _band2_rl_h = 742.0 - 613.5   # ~128.5 RL units
        orn_w       = frame_bw * 0.5   # ornaments are half the box width
        curl_orn_h_top = orn_w * _band3_rl_h / 1024.0   # aspect-locked height
        curl_orn_h_bot = orn_w * _band2_rl_h / 1024.0
        # White backing rect is narrower than ornament so box line runs right up to ornament ink
        orn_mask_w = orn_w * 0.80

        x_left  = frame_left
        x_right = frame_left + frame_bw
        r       = 10.0     # corner radius
        k       = 0.5523   # bezier quarter-circle approximation

        # ── 1. Closed rounded rect (full box outline) — 3-pass glow ────────────
        def _rrect_path(canvas):
            p = canvas.beginPath()
            p.moveTo(x_left + r, frame_top)
            p.lineTo(x_right - r, frame_top)
            p.curveTo(x_right - r*(1-k), frame_top,
                      x_right,           frame_top - r*(1-k),
                      x_right,           frame_top - r)
            p.lineTo(x_right, frame_bot + r)
            p.curveTo(x_right,           frame_bot + r*(1-k),
                      x_right - r*(1-k), frame_bot,
                      x_right - r,       frame_bot)
            p.lineTo(x_left + r, frame_bot)
            p.curveTo(x_left + r*(1-k), frame_bot,
                      x_left,           frame_bot + r*(1-k),
                      x_left,           frame_bot + r)
            p.lineTo(x_left, frame_top - r)
            p.curveTo(x_left,           frame_top - r*(1-k),
                      x_left + r*(1-k), frame_top,
                      x_left + r,       frame_top)
            p.close()
            return p
        c.saveState()
        # Pass 1: wide outer gray halo
        c.setStrokeColorRGB(0.68, 0.68, 0.68)
        c.setLineWidth(8.0)
        c.drawPath(_rrect_path(c), fill=0, stroke=1)
        # Pass 2: white ring
        c.setStrokeColorRGB(1.0, 1.0, 1.0)
        c.setLineWidth(4.5)
        c.drawPath(_rrect_path(c), fill=0, stroke=1)
        # Pass 3: dark core line
        c.setStrokeColorRGB(0.22, 0.22, 0.22)
        c.setLineWidth(2.0)
        c.drawPath(_rrect_path(c), fill=0, stroke=1)
        c.restoreState()
        ...
        # ── 3. Curl ornaments over horizontal edges ───────────────────────────
        # For each ornament: white backing (narrowed so box line runs close to ink),
        # then 3-pass glow rect around the ornament bounding box, then SVG.
        for orn_cy, orn_h, line_id in (
                (frame_top, curl_orn_h_top, 3),
                (frame_bot, curl_orn_h_bot, 2)):
            ox = x_center - orn_w / 2
            oy = orn_cy - orn_h / 2
            # White backing (narrower than orn_w so line comes close to ornament)
            c.saveState()
            c.setFillColorRGB(1, 1, 1)
            c.rect(x_center - orn_mask_w / 2, oy, orn_mask_w, orn_h, fill=1, stroke=0)
            c.restoreState()
            # 3-pass glow rect around the ornament bounding box
            c.saveState()
            c.setStrokeColorRGB(0.68, 0.68, 0.68)
            c.setLineWidth(8.0)
            c.rect(ox, oy, orn_w, orn_h, fill=0, stroke=1)
            c.setStrokeColorRGB(1.0, 1.0, 1.0)
            c.setLineWidth(4.5)
            c.rect(ox, oy, orn_w, orn_h, fill=0, stroke=1)
            c.setStrokeColorRGB(0.22, 0.22, 0.22)
            ...
            c.rect(ox, oy, orn_w, orn_h, fill=0, stroke=1)
            c.restoreState()
            _draw_curl_orn_line(c, line_id, x_center, orn_cy, orn_w, orn_h)
