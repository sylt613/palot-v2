...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        try:
            ...
            canvas = _apply_white_gray_rings(canvas, near_dil=7, far_blur=12,
                                             white_val=252, gray_val=120)
...
def apply_halo_glow(canvas_pil, near_dil=6, far_blur=10,
                    white_val=252, gray_val=120, ink_threshold=None):
    """Apply white ring + soft gray halo around any dark shape on a white canvas.

    Pixel priority: ink (untouched) > white ring (crisp) > gray halo (smooth Gaussian).
    Vectorized via numpy for speed.

    near_dil       MaxFilter kernel for white ring width (px). Smaller → tighter.
    far_blur       GaussianBlur radius for gray halo (px). Larger → smoother fade.
    white_val      brightness of white ring (250–255). Lower = slight gray tint.
    gray_val       darkest pixel value of gray halo at full density (lower = darker).
    ...
    # Gray halo: Gaussian-blurred ink mask, normalized 0..1
    gb_arr   = np.array(ink_pil.filter(ImageFilter.GaussianBlur(radius=far_blur))).astype(np.float32) / 255.0
    # Map to pixel value: 255 (white bg) → gray_val (full density)
    gv_arr   = (255.0 - gb_arr * (255.0 - gray_val)).clip(0, 255).astype(np.uint8)

    out = np.array(canvas_pil)
    # White ring: not ink, but inside near zone
    white_mask = near_zone & ~ink
    # Gray halo: not ink, not in white ring, has any blur intensity, AND darker than current pixel
    cur_val   = out[..., 0]   # canvas is white-bg so all 3 channels equal
    gray_mask = (~near_zone) & (~ink) & (gb_arr > 0.015) & (gv_arr < cur_val)
...
def render_mamar_box_image(width_pt, height_pt, dpi=300,
    ...
    for line_id, edge_y_px in ((3, box_t), (2, box_b)):
        ...
        if band is None:
            continue
        # Convert to RGB ink (band is 'L', white bg, dark ink)
        # Paste centered horizontally on the relevant edge
        bx = pad_px + (w_px - band.width) // 2
        by = edge_y_px - band.height // 2
        # White-mask paste so only the ink overrides background
        ink_mask = band.point(lambda p: 255 - p)  # ink → 255 alpha, white → 0
        canvas.paste((0, 0, 0), (bx, by), mask=ink_mask)
        # Erase any rounded-rect line behind the ornament's bounding box
        # (the curl path will then fill it back in cleanly)
        # — actually we want box line to "join" curl, so don't erase under the curl ink itself.
        # We erase only between the curl and behind, where the rounded-rect line ran:
        # leave ink alone (the curl), but clear the line strip behind the curl bbox.
        # Done implicitly: we redrew the line first, the curl ink overrides via paste.
