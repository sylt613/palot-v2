...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    # Generous vertical padding — gray gradient at gray_blur_r=28px needs room
    pad_v_px    = 40
    ...
    if cached is None:
        ...
        try:
            ...
            canvas = _apply_white_gray_rings(canvas, near_dil=7, far_blur=14,
                                             white_val=252, gray_val=90)
...
def apply_halo_glow(canvas_pil, near_dil=8, far_blur=20,
    ...
    # 4 concentric rings from ink outward, each progressively lighter gray.
    # Rings are built via MaxFilter dilations, then slightly smoothed.
    rings = [
        (near_dil + 3,  gray_val),
        (near_dil + 9,  min(250, gray_val + 35)),
        (near_dil + 18, min(252, gray_val + 70)),
        (near_dil + max(28, far_blur), min(253, gray_val + 105)),
    ]
