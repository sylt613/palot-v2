...
def _get_curl_band_pil(line_id, band_w_px, stroke_scale=1.0):
    ...
    """
    cache_key = (line_id, band_w_px, round(stroke_scale, 3))
...
def apply_halo_glow(canvas_pil, near_dil=8, far_blur=20,
                    white_val=252, gray_val=100, halo_boost=0.55,
                    ink_threshold=None):
    """Apply white ring + visible soft gray halo around any dark shape.

    Pixel priority: ink (untouched) > white ring (crisp) > gray halo (smooth).

    near_dil       MaxFilter kernel for white ring width (px). Tight ring.
    far_blur       GaussianBlur radius for gray halo (px). Larger → wider glow.
    white_val      brightness of white ring (250–255).
    gray_val       darkest pixel value of gray halo at full density.
    halo_boost     0..1 — power-curve exponent offset for gray intensity.
                   Higher = more saturated gray throughout the halo ring
                   (matches the v1 _draw_title_with_glow multi-ring feel).
    ink_threshold  pixel < this is treated as ink. Default _INK_THRESHOLD.
    """
    import numpy as np
    ...
    # ── Multi-ring gray halo (simulates _draw_title_with_glow) ───────────────
    # Structure from ink outward:
    #   • white ring      (near_dil kernel)    — crisp outline
    #   • DARK gray band  (near_dil + band_w)  — wide, saturated, reads as a line
    #   • medium gray     (+ band_w + 10)      — smooth fade
    #   • light gray      (+ band_w + 22)      — feathered edge
    # band_w is the thickness of the "gray line" in px — roughly matches the
    # white ring for a balanced layered look.
    ...
    # Apply halo_boost as a contrast tweak: values closer to 0 (dark) get pulled darker.
    if halo_boost != 1.0 and halo_boost > 0:
        norm = gv_arr.astype(np.float32) / 255.0
        # Power > 1 lightens; < 1 darkens. We want darker mids → power < 1.
        norm = np.power(norm, halo_boost)
        gv_arr = (norm * 255.0).clip(0, 255).astype(np.uint8)

    out = np.array(canvas_pil)
    cur_val   = out[..., 0]   # white-bg canvas → R=G=B
...
def render_mamar_box_image(width_pt, height_pt, dpi=300,
                            line_w_pt=1.9, orn_w_frac=0.5, corner_r_pt=10.0,
                            halo_pad_pt=26.0,
    ...
    # White ring thickness ≈ 2/3 of line thickness (on each side of ink).
    # MaxFilter kernel = 2*thickness + 1.
    if near_dil is None:
        white_ring_px = max(2, round(line_w_px * 0.66))
        near_dil = 2 * white_ring_px + 1
    if far_blur is None:
        far_blur = max(12, line_w_px * 3)
    ...
    # Render the curl bands. We thicken the SVG strokes so the rasterized curl
    # has the same visual stroke weight as the box line.
    orn_w_px       = max(1, round(width_pt * orn_w_frac * dpi / 72.0))
    # Heuristic: native SVG strokeWidth ≈ 1.0 RL unit; one RL unit at this scale
    # = orn_w_px / 1024 px. We want target_visible_stroke_px ≈ line_w_px.
    target_stroke_px = float(line_w_px)
    native_stroke_px = max(0.5, orn_w_px / 1024.0)  # most strokes are ~1 RL unit
    stroke_scale     = max(1.0, target_stroke_px / native_stroke_px)

    for line_id, edge_y_px in ((3, box_t), (2, box_b)):
        band = _get_curl_band_pil(line_id, orn_w_px, stroke_scale=stroke_scale)
        if band is None:
            continue
    ...
    # Apply halo to all ink in one pass
