...
def render_mamar_box_image(width_pt, height_pt, dpi=300,
    ...
    # White canvas, draw all ink in pure black so apply_halo_glow detects it.
    canvas = Image.new('RGB', (canvas_w, canvas_h), (255, 255, 255))
    draw   = ImageDraw.Draw(canvas)

    # Box coords (PIL — y increases downward)
    box_l = pad_px
    box_t = pad_px
    box_r = pad_px + w_px - 1
    box_b = pad_px + h_px - 1
    ...
    # Render the curl bands. Curl SVG is FILLED (no strokes), so we thicken its
    # rasterized ink via PIL morphological dilation to match the box line weight.
    orn_w_px       = max(1, round(width_pt * orn_w_frac * dpi / 72.0))
    # Native ink in the curl raster is ~2-3 px thick at our render scale.
    # We dilate by (line_w_px - native) / 2 per side to match box line visually.
    native_ink_px  = max(2, round(orn_w_px / 400.0))  # rough heuristic
    ink_dilate_px  = max(0, (line_w_px - native_ink_px) // 2)
    stroke_scale   = 1.0  # filled SVG — no-op, kept for cache-key stability

    for line_id, edge_y_px in ((3, box_t), (2, box_b)):
        band = _get_curl_band_pil(line_id, orn_w_px,
                                   stroke_scale=stroke_scale,
                                   ink_dilate_px=ink_dilate_px)
        if band is None:
            continue
        # Find the "flat line" row of the curl band — the PIL row with the
        # longest contiguous ink run. That row is the baseline that must be
        # flush with the box edge line.
        flat_row = _find_band_flat_row(band)
        bx = pad_px + (w_px - band.width) // 2
        # For top edge (line 3, edge_y_px=box_t): curl extends OUTWARD (upward
        # off the box), flat_row sits ON the top box line.
        # For bottom edge (line 2, edge_y_px=box_b): curl extends OUTWARD
        # (downward off the box), flat_row sits ON the bottom box line.
        # If the natural flat row is on the wrong side of the band for this
        # edge, flip the band vertically so curls point away from the box.
        if line_id == 3:
            # Want flat_row near band bottom (so curls extend upward above it).
            if flat_row < band.height // 2:
                band = band.transpose(Image.FLIP_TOP_BOTTOM)
                flat_row = band.height - 1 - flat_row
        else:
            # Want flat_row near band top (so curls extend downward below it).
            if flat_row > band.height // 2:
                band = band.transpose(Image.FLIP_TOP_BOTTOM)
                flat_row = band.height - 1 - flat_row
        by = edge_y_px - flat_row

        # Knock out the rounded-rect line under the ornament's horizontal span
        # so the box line doesn't visually cross through the curl territory.
        # Leave a small taper at each end (5% of width) so the flat line
        # smoothly merges into the box line.
        taper = max(2, band.width // 20)
        kx0 = bx + taper
        kx1 = bx + band.width - taper
        ky0 = edge_y_px - line_w_px
        ky1 = edge_y_px + line_w_px
        draw.rectangle((kx0, ky0, kx1, ky1), fill=(255, 255, 255))

        # Paste curl ink on top
        ink_mask = band.point(lambda p: 255 - p)
        canvas.paste((0, 0, 0), (bx, by), mask=ink_mask)

    # Apply halo to all ink in one pass — "outline" style for clean 2nd line.
    gray_band_px = max(4, line_w_px)
    canvas = apply_halo_glow(canvas, near_dil=near_dil, far_blur=far_blur,
                             white_val=white_val, gray_val=gray_val,
