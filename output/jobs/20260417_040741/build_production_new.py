...
def render_mamar_box_image(width_pt, height_pt, dpi=300,
    ...
    # White ring thickness ≈ line thickness (per user) — full thickness on each side.
    # MaxFilter kernel = 2*thickness + 1.
    if near_dil is None:
        white_ring_px = max(2, line_w_px)
        near_dil = 2 * white_ring_px + 1
    if far_blur is None:
        # Modest fade distance — we want a clean gray outline, not feathered.
