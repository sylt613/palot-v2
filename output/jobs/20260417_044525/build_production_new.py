...
def render_mamar_box_image(width_pt, height_pt, dpi=300,
    ...
    if natural is not None:
        # Use the LONGEST-ink-run row as the "main base line" — this picks the
        # ornament's thick main horizontal, not the thin protruding flourish
        # line below it.
        natural_base = _find_band_flat_row(natural)
        ...
        for edge_side, edge_y_px in (('top', box_t), ('bot', box_b)):
            # Natural line 2: "main line" (the ornament's prominent horizontal)
            # is at bottom of band with curls protruding UP.
            #   TOP edge of box  → flipped T⇅B  (curls extend DOWN, into/toward box)
            #   BOT edge of box  → natural      (curls extend UP, into/toward box)
            if edge_side == 'bot':
                band = natural
