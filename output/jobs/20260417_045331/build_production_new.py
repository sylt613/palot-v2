...
def render_mamar_box_image(width_pt, height_pt, dpi=300,
                            ...
                            white_val=252, gray_val=95):
    """Build a PIL image of a closed rounded-rect with curl ornaments centred on
    the top and bottom edges, with a halo glow around all ink. Returns
    (ImageReader, halo_pad_pt). The drawn box occupies a rect of width_pt × height_pt
    centered inside an image padded by halo_pad_pt on all sides.

    All ink (rounded rect line AND curl ornament) shares the SAME visual stroke
    weight (line_w_pt) so they flow into each other seamlessly. Halo wraps the
    ACTUAL paths of every shape, not a bounding rect.
    """
    ...
    r_px        = max(1, round(corner_r_pt * dpi / 72.0))
    # White ring thickness ≈ 0.6 × line thickness (slimmer, per user).
    if near_dil is None:
    ...
    # ── Step 1: box-only canvas + halo (box has its own gray outline) ─────────
    box_canvas = Image.new('RGB', (canvas_w, canvas_h), (255, 255, 255))
    box_draw   = ImageDraw.Draw(box_canvas)
    box_l = pad_px
    box_t = pad_px
    box_r = pad_px + w_px - 1
    box_b = pad_px + h_px - 1
    box_draw.rounded_rectangle((box_l, box_t, box_r, box_b),
                               radius=r_px, outline=(0, 0, 0), width=line_w_px)
    gray_band_px = max(4, line_w_px)
    box_canvas = apply_halo_glow(box_canvas,
                                 near_dil=near_dil, far_blur=far_blur,
                                 white_val=white_val, gray_val=gray_val,
                                 halo_style='outline', gray_band_px=gray_band_px)
    composite = np.array(box_canvas)

    # ── Step 2: each ornament gets its OWN halo, composited on top ────────────
    # Curl SVG is FILLED (no strokes), so ink is thickened via PIL dilation.
    orn_w_px       = max(1, round(width_pt * orn_w_frac * dpi / 72.0))
    native_ink_px  = max(2, round(orn_w_px / 400.0))
    ink_dilate_px  = max(0, (line_w_px - native_ink_px) // 2)
    stroke_scale   = 1.0  # filled SVG — no-op, kept for cache-key stability

    # Single render of line 2 (used natural on bottom, flipped on top)
    natural = _get_curl_band_pil(2, orn_w_px,
                                 stroke_scale=stroke_scale,
                                 ink_dilate_px=ink_dilate_px)
    if natural is not None:
        # Use the BOTTOMMOST long-ink-run row — OUTER edge of the thick main
        # horizontal ink block — so placing this row at the box edge makes
        # the thick line flush with the box edge.
        natural_base = _find_band_base_row_bottom_up(natural)
        # Padding around each orn canvas so its halo isn't clipped
        orn_pad_px = max(16, gray_band_px * 2 + near_dil)
        for edge_side, edge_y_px in (('top', box_t), ('bot', box_b)):
            # Natural line 2: curls at TOP of band, thick main line at BOTTOM.
            # User preference: curls point INWARD (into the box), with thick
            # line flush against the box edge.
            #   TOP edge of box  → flipped T⇅B  (thick at top of band flush with
            #                                     box top edge; curls hang DOWN inside)
            #   BOT edge of box  → natural      (thick at bottom of band flush with
            #                                     box bottom edge; curls rise UP inside)
            if edge_side == 'top':
                band = natural.transpose(Image.FLIP_TOP_BOTTOM)
                base_row = band.height - 1 - natural_base
            else:
                band = natural
                base_row = natural_base

            bx = pad_px + (w_px - band.width) // 2
            by = edge_y_px - base_row   # base row sits ON the box edge line

            # Isolated ornament canvas with padding for its own halo
            orn_w = band.width  + 2 * orn_pad_px
            orn_h = band.height + 2 * orn_pad_px
            orn_canvas = Image.new('RGB', (orn_w, orn_h), (255, 255, 255))
            ink_mask = band.point(lambda p: 255 - p)
            orn_canvas.paste((0, 0, 0), (orn_pad_px, orn_pad_px), mask=ink_mask)
            orn_canvas = apply_halo_glow(orn_canvas,
                                         near_dil=near_dil, far_blur=far_blur,
                                         white_val=white_val, gray_val=gray_val,
                                         halo_style='outline',
                                         gray_band_px=gray_band_px)
            orn_arr = np.array(orn_canvas)

            # Composite onto box canvas — orn pixels that are NOT pure white
            # override the box pixels (so orn has its own gray outline which
            # visually separates it from the box's gray outline).
            paste_x = bx - orn_pad_px
            paste_y = by - orn_pad_px
            H_b, W_b = composite.shape[:2]
            H_o, W_o = orn_arr.shape[:2]
            dx0 = max(0, paste_x); dx1 = min(W_b, paste_x + W_o)
            dy0 = max(0, paste_y); dy1 = min(H_b, paste_y + H_o)
            sx0 = dx0 - paste_x;   sy0 = dy0 - paste_y
            sx1 = sx0 + (dx1 - dx0); sy1 = sy0 + (dy1 - dy0)
            if dx1 > dx0 and dy1 > dy0:
                sub_orn = orn_arr[sy0:sy1, sx0:sx1]
                sub_box = composite[dy0:dy1, dx0:dx1]
                mask = sub_orn[..., 0] < 255
                sub_box[mask] = sub_orn[mask]
                composite[dy0:dy1, dx0:dx1] = sub_box

    canvas = Image.fromarray(composite, 'RGB')
