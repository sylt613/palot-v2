...
def _build_tiled_mamar_box_image(target_w_px, target_h_px):
    ...
    s = min(target_w_px / float(min_w_src), target_h_px / float(min_h_src))
    s = max(s, 0.05)
...
def heading_height(text, kind='section', has_name=True):
    if kind == 'anaf':
        ...
        # Must include frame padding (frame_pad_y=22 top+bottom) to match
        # draw_section_header() which returns frame_bh = text_h + 44.
        frame_pad_y = 22.0
