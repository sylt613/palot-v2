...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, center_frac=0.5):
    ...
    extra_h = max(0, target_h_px - (top_h + bot_h))

    canvas = Image.new('RGB', (target_w_px, target_h_px), (255, 255, 255))

    if extra_h > 0:
        canvas.paste(_tile_v(left_side_slice, left_side_w, extra_h), (0, top_h))
        canvas.paste(_tile_v(right_side_slice, right_side_w, extra_h),
                     (target_w_px - right_side_w, top_h))
