...
_MAMAR_BOX_GPT15_BBOX = (297, 62, 1240, 902)  # inclusive crop bounds from 1536x1024 source


def _mamar_box_min_height_px(target_w_px, center_frac=0.5):
    center_w_src = 733 - 210
    full_h_src = 840
    s = max(0.05, (target_w_px * center_frac) / float(center_w_src))
    return max(1, round(full_h_src * s))


def _build_tiled_mamar_box_image(target_w_px, target_h_px, center_frac=0.5):
    """Assemble the GPT-generated mamar/TOC box by tiling only straight runs.

    The center ornaments keep a locked aspect ratio via a single uniform scale.
    Width is extended by tiling the straight top/bottom runs; height is extended
    by tiling the straight side runs.
    """
    ...
    fw, fh = frame.size  # 943 x 840

    # Source geometry measured from the cropped frame.
    top_h_src = 275
    bot_y_src = 618
    bot_h_src = fh - bot_y_src
    mid_y0_src = top_h_src
    mid_y1_src = bot_y_src
    mid_h_src = mid_y1_src - mid_y0_src
    corner_w_src = 80
    center_x0 = 210
    center_x1 = 733
    left_side_w_src = 61
    ...
    min_h_src = fh
    if min_w_src <= 0 or min_h_src <= 0:
        return None

    # Lock the ornament aspect ratio by choosing scale from the fixed center motif
    # width, then extend the frame with tiled straight runs.
    s = max(0.05, (target_w_px * center_frac) / float(center_x1 - center_x0))
    # If the requested width is very narrow, cap scale so the fixed pieces still fit.
    ...
    mid_left = _resize(frame.crop((0, mid_y0_src, left_side_w_src, mid_y1_src)), left_side_w, mid_h)
    mid_right = _resize(frame.crop((fw - right_side_w_src, mid_y0_src, fw, mid_y1_src)), right_side_w, mid_h)
    ...
    if extra_h > 0:
        ...
        canvas.paste(_tile_v(right_side_slice, right_side_w, extra_h),
                     (target_w_px - right_side_w, mid_y + mid_h))
    ...
    if right_span_w > 0:
        canvas.paste(_tile_h(top_slice, right_span_w, top_h),
                     (corner_w + left_span_w + center_w, 0))
        canvas.paste(_tile_h(bot_slice, right_span_w, bot_h),
                     (corner_w + left_span_w + center_w, target_h_px - bot_h))

    top_center_x = corner_w + left_span_w
    bot_center_x = corner_w + left_span_w

    # Corners and fixed center motifs go last so their transition zones cover the tiles.
    ...
    canvas.paste(bot_center, (bot_center_x, target_h_px - bot_h))
...
def render_mamar_box_image(width_pt, height_pt, dpi=300,
                            line_w_pt=2.4, orn_w_frac=0.5, corner_r_pt=10.0,
                            halo_pad_pt=44.0,
    ...
    Returns (ImageReader, halo_pad_pt). The assembled box occupies width_pt ×
    height_pt centered inside a halo-padded image.
    """
    cache_key = (round(width_pt, 2), round(height_pt, 2), dpi, round(line_w_pt, 2),
                 round(orn_w_frac, 3), round(corner_r_pt, 2),
                 round(halo_pad_pt, 2), near_dil, far_blur, white_val, gray_val)
    cached = _MAMAR_BOX_CACHE.get(cache_key)
    if cached is not None:
        return cached
    pad_px = max(1, round(halo_pad_pt * dpi / 72.0))
    w_px   = max(1, round(width_pt * dpi / 72.0))
    h_px   = max(1, round(height_pt * dpi / 72.0))
    line_w_px = max(1, round(line_w_pt * dpi / 72.0))
    if near_dil is None:
        white_ring_px = max(3, round(line_w_px * 1.4))
        near_dil = 2 * white_ring_px + 1
    if far_blur is None:
        far_blur = max(8, line_w_px * 2)
    assembled = _build_tiled_mamar_box_image(w_px, h_px)
    if assembled is None:
        return None, halo_pad_pt

    canvas = Image.new('RGB', (w_px + 2 * pad_px, h_px + 2 * pad_px), (255, 255, 255))
    canvas.paste(assembled, (pad_px, pad_px))
    reader = ImageReader(canvas)
...
def draw_mamar_box(c, x_left, y_bot, width_pt, height_pt, **kwargs):
    """Draw a haloed rounded-rect + curl ornament box on the PDF canvas at
    (x_left, y_bot) with size (width_pt × height_pt). Box ink is drawn dark gray;
    the haloed image extends slightly outside the box bounds for the glow."""
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    title_frame_h = max(title_frame_h,
                        _mamar_box_min_height_px(max(1, round(title_frame_w * 300 / 72.0))) * 72.0 / 300.0)
    ...
    # Draw mamar box: rounded rect + curl ornaments + halo (rasterized)
    draw_mamar_box(c, title_frame_left, title_frame_bot, title_frame_w, title_frame_h,
                    line_w_pt=1.2, orn_w_frac=0.5, corner_r_pt=10.0)
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        # ── Mamar box: rounded rect + curl ornaments + halo (rasterized) ─────
        draw_mamar_box(c, frame_left, frame_bot, frame_bw, frame_bh,
                        line_w_pt=1.2, orn_w_frac=0.5, corner_r_pt=10.0)
