...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    ...
    mid_left   = _resize(frame.crop((0,              top_h_src,    left_side_w_src, bot_y_src)), left_side_w, mid_h)
    mid_right  = _resize(frame.crop((fw-right_side_w_src, top_h_src, fw,           bot_y_src)), right_side_w, mid_h)
    ...
    base_h      = top_h + mid_h + bot_h
    extra_h     = max(0, target_h_px - base_h)

    canvas = Image.new('RGB', (target_w_px, target_h_px), (255, 255, 255))

    # ── Side strips (placed first so ornament corners cover them) ─────────────
    mid_y = top_h
    canvas.paste(mid_left,  (0,                      mid_y))
    canvas.paste(mid_right, (target_w_px-right_side_w, mid_y))
    if extra_h > 0:
        canvas.paste(_tile_v(left_side_slice,  left_side_w,  extra_h), (0,                       mid_y + mid_h))
        canvas.paste(_tile_v(right_side_slice, right_side_w, extra_h), (target_w_px-right_side_w, mid_y + mid_h))
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    frame_pad_x  = 22.0
    orn_h_pt     = 20.0   # decorative scrollwork height
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        frame_pad_x = 30.0
        orn_h_pt    = 20.0   # decorative scrollwork height (≈1-2 lines)
        frame_pad_y = 10.0   # inner padding top & bottom inside box rectangle
