...
_MB_CORNER_W_SRC = 130   # width of corner cap pieces (left/right) — wider so
                         # the tiled straight run doesn't start right next to
                         # the corner scrollwork
_MB_SIDE_W_SRC   = 60    # width of side (left/right) strips
_MB_TILE_COL     = 172   # source column for 1-px top/bottom tile slice
                         # (midway between corner cap x=130 and center x=215)
_MB_SIDE_ROW     = 421   # source row for 1-px side tile slice — exact vertical
                         # center of the side straight run (top_h=275 … bot_y=567)
...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    """Assemble the GPT-generated mamar/TOC box by tiling only straight runs.

    orn_h_px: desired height of the top (and bottom) ornament in output pixels.
              All other dimensions scale uniformly from this, then straight runs
              are tiled horizontally (width) and vertically (height) to fill target.
    """
    if not os.path.exists(_MAMAR_BOX_GPT15_PATH):
        return None

    src = Image.open(_MAMAR_BOX_GPT15_PATH).convert('RGB')
    l, t, r, b = _MAMAR_BOX_GPT15_BBOX
    frame = src.crop((l, t, r, b))
    fw, fh = frame.size  # 943 × 839

    top_h_src     = _MB_TOP_H_SRC
    bot_y_src     = _MB_BOT_Y_SRC
    bot_h_src     = fh - bot_y_src
    mid_h_src     = bot_y_src - top_h_src
    corner_w_src  = _MB_CORNER_W_SRC
    center_x0     = _MB_CENTER_X0
    center_x1     = _MB_CENTER_X1
    left_side_w_src  = _MB_SIDE_W_SRC
    right_side_w_src = _MB_SIDE_W_SRC
    top_tile_col  = _MB_TILE_COL
    bot_tile_col  = _MB_TILE_COL
    side_tile_row = _MB_SIDE_ROW

    # Minimum source width to fit corners + center (no tiling needed below this)
    min_w_src = corner_w_src * 2 + (center_x1 - center_x0)
    if min_w_src <= 0 or fh <= 0:
        return None

    # Scale uniformly so top ornament is exactly orn_h_px tall
    s = orn_h_px / float(top_h_src)
    # Cap so corners still fit within target width
    s = min(s, target_w_px / float(min_w_src))
    ...
    top_h = max(1, round(top_h_src * s))
    bot_h = max(1, round(bot_h_src * s))
    corner_w = max(1, round(corner_w_src * s))
    center_w = max(1, round((center_x1 - center_x0) * s))
    left_side_w = max(1, round(left_side_w_src * s))
    right_side_w = max(1, round(right_side_w_src * s))

    # Fixed-aspect center motifs + corner caps.
    top_left   = _resize(frame.crop((0,              0,            corner_w_src,    top_h_src)), corner_w, top_h)
    top_center = _resize(frame.crop((center_x0,      0,            center_x1,       top_h_src)), center_w, top_h)
    top_right  = _resize(frame.crop((fw-corner_w_src,0,            fw,              top_h_src)), corner_w, top_h)
    bot_left   = _resize(frame.crop((0,              bot_y_src,    corner_w_src,    fh)),        corner_w, bot_h)
    bot_center = _resize(frame.crop((center_x0,      bot_y_src,    center_x1,       fh)),        center_w, bot_h)
    bot_right  = _resize(frame.crop((fw-corner_w_src,bot_y_src,    fw,              fh)),        corner_w, bot_h)

    # 1-px tile sources. Tops/bottoms: a single COLUMN from the straight run
    # between corner and center ornament. Sides: a single ROW from the straight
    # middle region between the top and bottom ornaments (vertically centered).
    top_slice       = frame.crop((top_tile_col,          0,            top_tile_col+1,         top_h_src))
    bot_slice       = frame.crop((bot_tile_col,          bot_y_src,    bot_tile_col+1,         fh))
    left_side_slice = frame.crop((0,                     side_tile_row,left_side_w_src,         side_tile_row+1))
    right_side_slice= frame.crop((fw-right_side_w_src,   side_tile_row,fw,                     side_tile_row+1))

    extra_w     = max(0, target_w_px - (corner_w * 2 + center_w))
    left_span_w = extra_w // 2
    right_span_w= extra_w - left_span_w
    # Vertical span between top and bottom ornaments — fill entirely by tiling
    # the 1-px side row (no distorted resize of the source middle band).
    side_span_h = max(0, target_h_px - top_h - bot_h)

    canvas = Image.new('RGB', (target_w_px, target_h_px), (255, 255, 255))

    # ── Side strips: tile the 1-px row for the full middle span ──────────────
    if side_span_h > 0:
        canvas.paste(_tile_v(left_side_slice,  left_side_w,  side_span_h), (0,                        top_h))
        canvas.paste(_tile_v(right_side_slice, right_side_w, side_span_h), (target_w_px-right_side_w, top_h))

    # ── Top/bottom tiled straight runs ────────────────────────────────────────
    if left_span_w > 0:
        canvas.paste(_tile_h(top_slice, left_span_w, top_h), (corner_w, 0))
        canvas.paste(_tile_h(bot_slice, left_span_w, bot_h), (corner_w, target_h_px - bot_h))
    if right_span_w > 0:
        canvas.paste(_tile_h(top_slice, right_span_w, top_h), (corner_w + left_span_w + center_w, 0))
        canvas.paste(_tile_h(bot_slice, right_span_w, bot_h), (corner_w + left_span_w + center_w, target_h_px - bot_h))

    top_center_x = corner_w + left_span_w

    # ── Corners and center motifs last (cover tile seams) ─────────────────────
    canvas.paste(top_left,   (0,                  0))
    canvas.paste(top_center, (top_center_x,        0))
    canvas.paste(top_right,  (target_w_px-corner_w,0))
    canvas.paste(bot_left,   (0,                  target_h_px - bot_h))
    canvas.paste(bot_center, (top_center_x,        target_h_px - bot_h))
    canvas.paste(bot_right,  (target_w_px-corner_w,target_h_px - bot_h))

    # ── Flood-fill interior of box rectangle to pure white ────────────────────
    # This makes the box interior exactly 255 so the PDF mask [254-255] makes it
    # fully transparent and text drawn on top is never obscured.
    try:
        from PIL import ImageDraw
        cx = target_w_px // 2
        cy = top_h + side_span_h // 2
        ImageDraw.floodfill(canvas, (cx, cy), (255, 255, 255), thresh=40)
    except Exception:
        pass
