...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    """Assemble the GPT-generated mamar/TOC box using a 9-slice injection scheme.

    The whole source frame is scaled by a single factor s = orn_h_px / top_h_src
    so that ornaments stay at a fixed natural aspect. Extra rows are injected at
    the vertical-centre source row (a 1-px horizontal strip taken from the pure
    straight side-run), pushing the top ornament upward and the bottom ornament
    downward. Extra columns are injected symmetrically at two straight-run
    source columns (_MB_TILE_COL on the left and fw-_MB_TILE_COL on the right),
    pushing the left corner leftward and the right corner rightward while
    leaving the centre ornament untouched. No scaling is applied to the
    ...
    top_h_src = _MB_TOP_H_SRC        # 275
    xL_src    = _MB_TILE_COL         # 172 — left-side straight-run column
    xR_src    = fw - _MB_TILE_COL    # 771 — right-side straight-run column
    yC_src    = _MB_SIDE_ROW         # 421 — vertical-centre straight-run row

    # Scale factor: ornament height drives the whole frame's natural scale.
    s = orn_h_px / float(top_h_src)

    # If requested target is smaller than natural, cap s so natural fits.
    nat_w = max(1, round(fw * s))
    nat_h = max(1, round(fh * s))
    if target_w_px < nat_w:
        s = target_w_px / float(fw)
        nat_w = target_w_px
        nat_h = max(1, round(fh * s))
    if target_h_px < nat_h:
        s = target_h_px / float(fh)
        nat_h = target_h_px
        nat_w = max(1, round(fw * s))

    # Scale the whole frame uniformly.
    scaled = frame.resize((nat_w, nat_h), Image.LANCZOS)

    # Scaled coordinates of injection points.
    xL = max(1, min(nat_w - 2, round(xL_src * s)))
    xR = max(xL + 1, min(nat_w - 1, round(xR_src * s)))
    yC = max(1, min(nat_h - 2, round(yC_src * s)))

    extra_w = max(0, target_w_px - nat_w)
    extra_h = max(0, target_h_px - nat_h)
    pad_L   = extra_w // 2
    pad_R   = extra_w - pad_L
    # Vertical injection goes entirely at yC (top pushed up, bottom pushed down).

    # ── Step 1: vertical injection ──────────────────────────────────────────
    # Build an intermediate image of width=nat_w, height=target_h_px by
    # stacking: top half [0..yC) + extra_h copies of row yC + bottom half [yC..nat_h).
    if extra_h > 0:
        top_part   = scaled.crop((0, 0,  nat_w, yC))
        bot_part   = scaled.crop((0, yC, nat_w, nat_h))
        inject_row = scaled.crop((0, yC, nat_w, yC + 1))
        vert_img = Image.new('RGB', (nat_w, target_h_px), (255, 255, 255))
        vert_img.paste(top_part, (0, 0))
        for i in range(extra_h):
            vert_img.paste(inject_row, (0, yC + i))
        vert_img.paste(bot_part, (0, yC + extra_h))
    else:
        vert_img = scaled

    # ── Step 2: horizontal injection (symmetric at xL and xR) ───────────────
    if extra_w > 0:
        left_part   = vert_img.crop((0,   0, xL,    target_h_px))
        middle_part = vert_img.crop((xL,  0, xR,    target_h_px))
        right_part  = vert_img.crop((xR,  0, nat_w, target_h_px))
        inject_col_L = vert_img.crop((xL, 0, xL + 1, target_h_px))
        inject_col_R = vert_img.crop((xR, 0, xR + 1, target_h_px))
        out = Image.new('RGB', (target_w_px, target_h_px), (255, 255, 255))
        x = 0
        out.paste(left_part, (x, 0));          x += xL
        for i in range(pad_L):
            out.paste(inject_col_L, (x + i, 0))
        x += pad_L
        out.paste(middle_part, (x, 0));        x += (xR - xL)
        for i in range(pad_R):
            out.paste(inject_col_R, (x + i, 0))
        x += pad_R
        out.paste(right_part, (x, 0))
    else:
        out = vert_img

    # ── Flood-fill interior of the box rectangle to pure white ──────────────
    # Box interior is near the horizontal and vertical centre of the output.
    try:
        from PIL import ImageDraw
        cx = target_w_px // 2
        cy = target_h_px // 2
        ImageDraw.floodfill(out, (cx, cy), (255, 255, 255), thresh=40)
    except Exception:
        pass
