...
def _mb_expand_cap_width(cap, target_w, x1, x2, x3, x4, lsx, rsx):
    ...
    lx = max(x1, min(x2 - 1, lsx))
    rx = max(x3, min(x4 - 1, rsx))
...
# x-positions. Matches the reference shar-blatt algorithm.
#   corner 0..0.15 | rail 0.15..0.23 | center 0.23..0.77 | rail 0.77..0.85 | corner 0.85..1
#   rail sample strips at 0.18 / 0.82 (deep inside straight-run rail ink).
_MB_SPLIT_FRACS = (0.15, 0.23, 0.77, 0.85)
_MB_SAMPLE_FRACS = (0.18, 0.82)
# Straight vertical side-run in source rows (pre-scale). Pull in well past
# the corners so tiling never bumps into a curl tip. Sample is midpoint.
_MB_STRAIGHT_Y0 = 330
_MB_STRAIGHT_Y1 = 500
