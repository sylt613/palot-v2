...
# Horizontal 5-region split fractions (of source frame width) and rail-sample
# x-positions. OpenCV analysis of mamar_box_gpt15.png confirmed:
#   - No corner ornaments at all; the frame has only left/right straight rails
#     and a center scroll ornament spanning source x=212..731.
#   - Ornament source cols 212..731 → fracs 0.225..0.775 (fw=943).
#   - x1=0 (no corner), x2=0.220 (just before ornament left foot at 212),
#     x3=0.780 (just after ornament right foot at 731), x4=1.0 (no corner).
#   - Rail sample strips at 0.10 / 0.90 — deep into rail, far from ornament.
_MB_SPLIT_FRACS   = (0.0, 0.220, 0.780, 1.0)
_MB_SAMPLE_FRACS  = (0.10, 0.90)
# Straight vertical side-run in source rows (pre-scale). OpenCV detected
# longest straight run y=280..557. Add pad of 40 on each side.
_MB_STRAIGHT_Y0 = 320
_MB_STRAIGHT_Y1 = 517
...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    ...
    x1 = round(sw * _MB_SPLIT_FRACS[0])
    x2 = round(sw * _MB_SPLIT_FRACS[1])
    x3 = round(sw * _MB_SPLIT_FRACS[2])
    x4 = round(sw * _MB_SPLIT_FRACS[3])
    lsx = round(sw * _MB_SAMPLE_FRACS[0])
    rsx = round(sw * _MB_SAMPLE_FRACS[1])
    ...
    mid_row    = _mb_stretch_middle_width(mid_row, target_w_px, max(1, x1), max(1, sw - x4))
