...
def _mb_tile_horiz(slice_img, target_w):
    """Tile a narrow RGBA slice horizontally to fill target_w."""
...
def _mb_expand_cap_width(cap, target_w, x1, x2, x3, x4, lsx, rsx):
    """Extend a top/bottom cap strip horizontally using a 5-region split:

        [ 0 .. x1 ]   left corner curl     (preserved as-is)
        [ x1 .. x2 ] left smooth rail     (tiled from 1-px strip at lsx)
        [ x2 .. x3 ] center ornament      (preserved as-is)
        [ x3 .. x4 ] right smooth rail    (tiled from 1-px strip at rsx)
        [ x4 .. W  ] right corner curl    (preserved as-is)
    """
    ...
    lx = max(x1, min(max(x1, x2 - 1), lsx))
    rx = max(x3, min(min(x4, cap.width) - 1, rsx))
    left_mid  = cap.crop((lx, 0, lx + 1, cap.height))
    right_mid = cap.crop((rx, 0, rx + 1, cap.height))
    ...
    right_w = grow - left_w
    left_mid_t  = _mb_tile_horiz(left_mid,  max(1, left_w))
    right_mid_t = _mb_tile_horiz(right_mid, max(1, right_w))

    out = Image.new('RGBA', (target_w, cap.height), (0, 0, 0, 0))
    x = 0
    out.alpha_composite(left,        (x, 0)); x += left.width
    out.alpha_composite(left_mid_t,  (x, 0)); x += left_mid_t.width
    out.alpha_composite(center,      (x, 0)); x += center.width
    out.alpha_composite(right_mid_t, (x, 0)); x += right_mid_t.width
    out.alpha_composite(right, (target_w - right.width, 0))
    return out


def _mb_stretch_middle_width(row_img, target_w, left_fixed, right_fixed):
    """Extend a 1-px-tall middle row to target_w, preserving left/right corner cols."""
    if row_img.width >= target_w:
        return row_img.resize((target_w, row_img.height), Image.LANCZOS)
    left   = row_img.crop((0, 0, left_fixed, row_img.height))
    right  = row_img.crop((row_img.width - right_fixed, 0, row_img.width, row_img.height))
    sample_x = max(left_fixed, min(row_img.width - right_fixed - 1, row_img.width // 2))
    mid    = row_img.crop((sample_x, 0, sample_x + 1, row_img.height))
    mid_w  = max(1, target_w - left.width - right.width)
    mid_t  = _mb_tile_horiz(mid, mid_w)
    out = Image.new('RGBA', (target_w, row_img.height), (0, 0, 0, 0))
    out.alpha_composite(left,  (0, 0))
    out.alpha_composite(mid_t, (left.width, 0))
    out.alpha_composite(right, (target_w - right.width, 0))
    return out


# Exact source-pixel cap split boundaries measured from
# `output/mamar_box_gpt15_cropped.png` (943x840):
#   [0..54]    left rounded corner
#   [54..210]  left straight rail
#   [210..733] center ornament
#   [733..889] right straight rail
#   [889..943] right rounded corner
# Rail sample strips sit deep in the straight runs, away from both the corner
# turn and the ornament feet.
_MB_CAP_X1_SRC = 60
# Preserve almost all of the cap between the rounded side corners and the
# center flourish so we don't shave off the outer curl shoulders. This keeps a
# narrow but clean straight-rail stretch zone on each side.
_MB_CAP_X2_SRC = 94
_MB_CAP_X3_SRC = 849
_MB_CAP_X4_SRC = 883
_MB_CAP_SAMPLE_L_SRC = 77
_MB_CAP_SAMPLE_R_SRC = 866
# On the pure straight side-run row (source y=421), the full left and right
# side walls occupy 60px each: left x=1..60, right x=883..942. Preserve these
# full widths when stretching the middle row so both vertical double-lines stay
# intact.
_MB_SIDE_FIXED_W_SRC = 60
# Straight vertical side-run in source rows (pre-scale). OpenCV detected
# longest straight run y=280..557. Add pad so vertical tiling stays clear of
# the top/bottom ornament turns.
_MB_STRAIGHT_Y0 = 320
_MB_STRAIGHT_Y1 = 517


def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    """Assemble the GPT-generated mamar/TOC box using aspect-locked 5-region
    stretching (ported from the shar-blatt system-2 reference).

    Steps:
      1.  Aspect-lock scale the whole frame so it fits inside target
          (scale = min(target_w/fw, target_h/fh)). Ornaments never distort.
      2.  Split the scaled frame into:
            - top cap  : rows 0..y0  (full ornament band + corner region)
            - mid row  : single 1-px row at sample_y (pure straight rail)
            - bot cap  : rows y1..H  (full ornament band + corner region)
      3.  For each cap, extend horizontally using a 5-region split:
          corners and center ornament are preserved as-is; the two rail
          sub-regions between them are each tiled from a single 1-px vertical
          strip taken deep inside the rail ink.
      4.  Tile the 1-px mid row vertically to fill between top/bot caps.
      5.  Flood-fill the interior to white so text shows through transparently.

    `orn_h_px` is kept as a parameter for API compatibility but only acts as
    a *cap* on scale (ornaments never grow larger than orn_h_px/top_h_src of
    the source). The reference's aspect-lock rule dominates otherwise.
    """
    ...
    scale = max(scale, 0.02)  # safety floor
    ...
    local_y0 = max(1, round(_MB_STRAIGHT_Y0 * scale))
    local_y1 = min(sh - 2, round(_MB_STRAIGHT_Y1 * scale))
    if local_y1 <= local_y0 + 1:
        local_y1 = min(sh - 2, local_y0 + 2)
    local_sample = (local_y0 + local_y1) // 2

    top_cap    = scaled.crop((0, 0, sw, local_y0))
    bottom_cap = scaled.crop((0, local_y1, sw, sh))
    mid_row    = scaled.crop((0, local_sample, sw, local_sample + 1))

    x1 = max(1, min(sw - 4, round(_MB_CAP_X1_SRC * scale)))
    x2 = max(x1 + 1, min(sw - 3, round(_MB_CAP_X2_SRC * scale)))
    x3 = max(x2 + 1, min(sw - 2, round(_MB_CAP_X3_SRC * scale)))
    x4 = max(x3 + 1, min(sw - 1, round(_MB_CAP_X4_SRC * scale)))
    lsx = max(x1, min(x2 - 1, round(_MB_CAP_SAMPLE_L_SRC * scale)))
    rsx = max(x3, min(x4 - 1, round(_MB_CAP_SAMPLE_R_SRC * scale)))
    ...
    side_fixed_w = max(1, round(_MB_SIDE_FIXED_W_SRC * scale))
    mid_row = _mb_stretch_middle_width(mid_row, target_w_px, side_fixed_w, side_fixed_w)
    ...
    if top_h + bot_h >= target_h_px:
        ...
        resized = frame.resize((target_w_px, target_h_px), Image.LANCZOS)
        out_rgb = resized.convert('RGB')
    else:
        out_rgba = Image.new('RGBA', (target_w_px, target_h_px), (0, 0, 0, 0))
        out_rgba.alpha_composite(top_cap, (0, 0))
        y = top_h
        while y < target_h_px - bot_h:
            out_rgba.alpha_composite(mid_row, (0, y))
            y += 1
        out_rgba.alpha_composite(bottom_cap, (0, target_h_px - bot_h))
        # Composite onto white so flood-fill has a clean canvas.
