...
def _mb_expand_cap_width(cap, target_w, x1, x2, x3, x4):
    """Extend a top/bottom cap strip horizontally using a 5-region split:

        [ 0 .. x1 ]   left corner curl     (preserved as-is)
        [ x1 .. x2 ] left smooth rail     (stretched from full rail strip)
        [ x2 .. x3 ] center ornament      (preserved as-is)
        [ x3 .. x4 ] right smooth rail    (stretched from full rail strip)
    ...
    left_rail = cap.crop((x1, 0, x2, cap.height))
    center = cap.crop((x2, 0, x3, cap.height))
    right_rail = cap.crop((x3, 0, x4, cap.height))
    ...
    right_w = grow - left_w
    left_rail_t  = _mb_resize_horiz(left_rail,  max(1, left_w))
    right_rail_t = _mb_resize_horiz(right_rail, max(1, right_w))

    out = Image.new('RGBA', (target_w, cap.height), (0, 0, 0, 0))
    x = 0
    out.alpha_composite(left,        (x, 0)); x += left.width
    out.alpha_composite(left_rail_t, (x, 0)); x += left_rail_t.width
    out.alpha_composite(center,      (x, 0)); x += center.width
    out.alpha_composite(right_rail_t, (x, 0)); x += right_rail_t.width
...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    ...
    top_cap    = _mb_expand_cap_width(top_cap,    target_w_px, x1, x2, x3, x4)
    bottom_cap = _mb_expand_cap_width(bottom_cap, target_w_px, x1, x2, x3, x4)
