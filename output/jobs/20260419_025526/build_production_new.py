...
def _mb_expand_middle_band(band_img, target_w, left_fixed, right_fixed):
    """Extend the straight middle band to target_w, preserving the full side walls."""
    ...
    mid = band_img.crop((left_fixed, 0, band_img.width - right_fixed, band_img.height))
    mid_w = max(1, target_w - left.width - right.width)
    mid_r = _mb_resize_horiz(mid, mid_w)
    out = Image.new('RGBA', (target_w, band_img.height), (0, 0, 0, 0))
    out.alpha_composite(left,  (0, 0))
    out.alpha_composite(mid_r, (left.width, 0))
    out.alpha_composite(right, (target_w - right.width, 0))
    return out
...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    ...
    Steps:
      ...
      2.  Split the scaled frame into:
            ...
            - mid band : full straight side-run band
      ...
      3.  For each cap, extend horizontally using a 5-region split:
          ...
          sub-regions between them are stretched from the full straight rail.
      4.  Stretch the full straight middle band vertically to fill between top/bot caps.
    ...
    middle_band = scaled.crop((0, local_y0, sw, local_y1))
    ...
    middle_band = _mb_expand_middle_band(middle_band, target_w_px, side_fixed_w, side_fixed_w)
    ...
    if top_h + bot_h >= target_h_px:
        ...
        mid_fill = middle_band.resize((target_w_px, mid_h), Image.LANCZOS)
        out_rgba.alpha_composite(mid_fill, (0, top_h))
