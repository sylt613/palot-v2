...
def _mb_detect_cap_split(frame_rgba, top_frac=0.28, ink_thresh=40, pad_px=8):
    ...
    # Corner width: use outermost ink edge
    all_cols = _np.where(strong)[0]
    left_edge  = int(all_cols.min())  if len(all_cols) else 0
    right_edge = int(all_cols.max()) if len(all_cols) else w - 1

    x1 = min(left_edge + max(4, (center_l - left_edge) // 2), center_l - 2)
    x4 = max(right_edge - max(4, (right_edge - center_r) // 2), center_r + 2)
    x1 = max(1, x1)
    x4 = min(w - 2, x4)
