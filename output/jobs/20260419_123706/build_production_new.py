...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    """Draw the post-anaf section divider: gray ink, white ring, soft gray gradient.

    Like even-page header title (_draw_title_with_glow): white outline between
    ink and gray, gray fades softly outward.  Canvas padded vertically so the
    gradient is never cropped at the image edge.  y_top is the top edge of the
    visible ornament band."""
    ...
    if cached is None:
        ...
        try:
            ...
            # Padded canvas — pure white (→ masked transparent in PDF)
            canvas = Image.new('RGB', (needed_w_px, canvas_h_px), (255, 255, 255))

            # RGBA → gray ink core
            arr        = np.array(src, dtype=np.float32)
            alpha      = arr[:, :, 3] / 255.0
            ink_gray   = 255 - alpha * (255 - 72)   # 72 = dark core value
            ink_img    = Image.fromarray(ink_gray.clip(0, 255).astype(np.uint8), 'L')
            rgb_orn    = Image.merge('RGB', (ink_img, ink_img, ink_img))
            alpha_mask = Image.fromarray((alpha * 255).clip(0, 255).astype(np.uint8), 'L')

            # Paste centered horizontally; at pad_v_px from top of padded canvas
            paste_x = max(0, (needed_w_px - new_w) // 2)
            canvas.paste(rgb_orn, (paste_x, pad_v_px), mask=alpha_mask)

            # Apply two-ring effect
            canvas = _apply_white_gray_rings(canvas, near_dil=11, far_blur=22,
                                             white_val=252, gray_val=95)
            cached = ImageReader(canvas)
    ...
    c.saveState()
    c.drawImage(cached, x_center - width / 2, draw_y_bot,
                width=width, height=total_h_pt, mask=[254, 255, 254, 255, 254, 255])
    c.restoreState()
    return render_h
...
def _mb_detect_cap_split(frame_rgba, top_frac=0.28, ink_thresh=40, pad_px=8,
                         left_fixed_frac=(60.0 / 943.0),
                         right_fixed_frac=(883.0 / 943.0)):
...
_MB_CAP_SPLIT_CACHE   = None   # (x1f, x2f, x3f, x4f) fractions
_MB_STRAIGHT_Y_CACHE  = None   # (y0, y1, sample_y) in source-image coords


def _mb_get_geometry(frame_rgba, stroke_mask_arr, bbox):
    """Return cached (cap_split_fracs, straight_band) for the source frame."""
    global _MB_CAP_SPLIT_CACHE, _MB_STRAIGHT_Y_CACHE
    if _MB_CAP_SPLIT_CACHE is None:
        _MB_CAP_SPLIT_CACHE  = _mb_detect_cap_split(frame_rgba)
        _MB_STRAIGHT_Y_CACHE = _mb_detect_straight_band(stroke_mask_arr, bbox)
    return _MB_CAP_SPLIT_CACHE, _MB_STRAIGHT_Y_CACHE


def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    ...
    Steps
    -----
    1.  Load source, auto-detect straight side-run band and cap split.
    2.  Aspect-lock scale so ornaments are never distorted; cap size by
        orn_h_px if desired.
    3.  Split scaled frame → top cap / 1-px mid row / bottom cap.
    4.  Expand each cap horizontally: corners + center ornament preserved
        as-is; rails filled by tiling a single pixel column.
    5.  Expand mid row: side walls preserved; interior filled by 1-px tile.
    6.  Stack: top cap / (mid row × needed height) / bottom cap.
    7.  Flood-fill interior white for PDF transparency mask.
    """
    if not os.path.exists(_MAMAR_BOX_GPT15_PATH):
        return None

    import numpy as _np

    src = Image.open(_MAMAR_BOX_GPT15_PATH).convert('RGBA')
    l, t, r, b = _MAMAR_BOX_GPT15_BBOX
    frame = src.crop((l, t, r, b))
    fw, fh = frame.size  # 943 × 839
    ...
    split_fracs, (sy0, sy1, ssamp) = _mb_get_geometry(frame, stroke_mask, bbox_local)

    # Use the natural fit scale for the full frame body so the vertical side
    # walls keep enough thickness. Then, if the requested ornament height is
    # smaller, vertically compress only the top/bottom cap bands.
    scale = min(target_w_px / fw, target_h_px / fh)
    scale = max(scale, 0.02)
    scale_cap = min(scale, orn_h_px / float(_MB_TOP_H_SRC))
    cap_v_scale = scale_cap / scale if scale > 0 else 1.0
    ...
    if cap_v_scale < 0.999:
        top_cap_h = max(1, round(top_cap.height * cap_v_scale))
        bot_cap_h = max(1, round(bottom_cap.height * cap_v_scale))
        top_cap = top_cap.resize((top_cap.width, top_cap_h), Image.LANCZOS)
        bottom_cap = bottom_cap.resize((bottom_cap.width, bot_cap_h), Image.LANCZOS)
    ...
    if top_h + bot_h >= target_h_px:
        # Box shorter than natural caps — fall back to uniform resize.
        out_rgb = frame.resize((target_w_px, target_h_px), Image.LANCZOS).convert('RGB')
    else:
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # ornament heights together ~ 2*orn_h_pt; box rect ~ text + 2*frame_pad_y
