...
_MB_ROOT_SCALE        = 0.85   # uniform pre-shrink before tiling; keeps one aspect-locked source scale
...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    ...
    # separate cap/body scaling; that breaks the frame's geometry.  A modest
    # root_scale shrinks the source uniformly so the straight runs do more of
    # the fitting work, matching the reference approach.
    fit_scale = min(target_w_px / fw, target_h_px / fh)
    scale = max(0.02, fit_scale * _MB_ROOT_SCALE)
