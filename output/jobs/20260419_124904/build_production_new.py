...
_MB_ROOT_SCALE_MAX    = 1.0    # uniform pre-shrink before tiling; keeps one aspect-locked source scale
...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83):
    ...
    # Single aspect-locked source scale, then tiling only.  Do not apply any
    # separate cap/body scaling; that breaks the frame's geometry.  Derive the
    # uniform root-scale from orn_h_px so the frame keeps the old visual size,
    # but stays a single locked-aspect source before tiling.
    fit_scale = min(target_w_px / fw, target_h_px / fh)
    root_scale = min(_MB_ROOT_SCALE_MAX, orn_h_px / max(1.0, (_MB_TOP_H_SRC * fit_scale)))
