...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        try:
            ...
            white_mask = alpha_mask.filter(ImageFilter.MaxFilter(5))
            # Larger gray dilation: forms the outer glow field.
            gray_mask  = alpha_mask.filter(ImageFilter.MaxFilter(11))
            ...
            gray_ring = gray_ring.filter(ImageFilter.GaussianBlur(3.8))

            rgba = Image.new('RGBA', (needed_w_px, canvas_h_px), (0, 0, 0, 0))
            paste_x = max(0, (needed_w_px - new_w) // 2)

            # Outer faded gray outline.
            gray_fill = Image.new('RGBA', (new_w, band_h_px), (110, 110, 110, 255))
