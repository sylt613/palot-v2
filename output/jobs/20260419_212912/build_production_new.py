...
_MB_ROOT_SCALE_MAX    = 1.15   # uniform pre-shrink before tiling; keeps one aspect-locked source scale
_MB_ROOT_SCALE_BOOST  = 1.15   # prior approved visual-size boost (do not touch)
# Frame-rectangle outer size multiplier.  This is what actually makes the
# on-page box visibly bigger: it scales the caller-computed frame width / height
# (by scaling orn_h_pt + paddings at the call sites).  1.0 = current visual.
_MAMAR_BOX_SIZE_MULT  = 1.30
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    # Base geometry fits the text tightly; then the ENTIRE rectangle is
    # multiplied by _MAMAR_BOX_SIZE_MULT so the frame visibly grows around
    # the un-scaled text.
    tw           = Wid(title_text, HEADING_BOLD_FONT, TOC_TITLE_FS)
    _base_pad_x  = 22.0
    _base_orn_h  = 13.0
    _base_pad_y  = 10.0
    _base_w      = tw + _base_pad_x * 2
    _base_h      = 2 * _base_orn_h + TOC_TITLE_FS + _base_pad_y * 2
    title_frame_w = _base_w * _MAMAR_BOX_SIZE_MULT
    title_frame_h = _base_h * _MAMAR_BOX_SIZE_MULT
    orn_h_pt     = _base_orn_h * _MAMAR_BOX_SIZE_MULT
    frame_pad_y  = _base_pad_y * _MAMAR_BOX_SIZE_MULT
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        _base_orn_h = 13.0
        _base_pad_y = 10.0
        _base_bw    = max_lw + _base_pad_x * 2
        _base_box_rect_h = total_h + _base_pad_y * 2
        _base_bh    = 2 * _base_orn_h + _base_box_rect_h

        frame_bw    = _base_bw * _MAMAR_BOX_SIZE_MULT
        frame_bh    = _base_bh * _MAMAR_BOX_SIZE_MULT
        orn_h_pt    = _base_orn_h * _MAMAR_BOX_SIZE_MULT
        frame_pad_y = _base_pad_y * _MAMAR_BOX_SIZE_MULT
        box_rect_h  = _base_box_rect_h * _MAMAR_BOX_SIZE_MULT
