...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    frame_pad_x  = 22.0
    orn_h_pt     = 13.0   # decorative scrollwork height
...
def draw_section_header(c, text, y, kind='section'):
    ...
    if kind == 'sefer_title':
        ...
        frame_pad_x = 30.0
        orn_h_pt    = 13.0   # decorative scrollwork height (≈1 line)
        frame_pad_y = 16.0   # inner padding top & bottom inside box rectangle
...
def heading_height(text, kind='section', has_name=True):
    if kind == 'anaf':
        ...
        max_lw = max(Wid(vis(' '.join(lw)), font, fs) for lw in lines)
        frame_bw = max_lw + frame_pad_x * 2
        frame_bh = text_h + frame_pad_y * 2
        frame_bh = max(frame_bh,
                       _mamar_box_min_height_px(max(1, round(frame_bw * 300 / 72.0))) * 72.0 / 300.0)
