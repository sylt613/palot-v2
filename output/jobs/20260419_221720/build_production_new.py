...
def draw_post_anaf_divider_band(c, x_center, y_top, width):
    ...
    if cached is None:
        ...
        try:
            ...
            gray_ring = gray_ring.point(lambda p: int(p * 0.45))
            ...
            gray_fill = Image.new('RGBA', gray_ring.size, (145, 145, 145, 255))
...
def _mb_variant_source(variant):
    ...
    if (variant or 'default').lower() == 'alt':
        return _MAMAR_BOX_ALT_PATH, _MAMAR_BOX_ALT_BBOX, None
...
_MB_ROOT_SCALE_MAX    = 0.55
_MB_ROOT_SCALE_BOOST  = 0.55
...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83, variant=None):
    ...
    if variant == 'default':
        # Preserve the approved default visual: scale driven by ornament height
        # vs. the pinned top_h_src hint.
        root_scale = min(_MB_ROOT_SCALE_MAX,
                         (orn_h_px / max(1.0, (top_h_src * fit_scale))) * _MB_ROOT_SCALE_BOOST)
    else:
        # Alt variant: BOOST is a direct multiplier on fit_scale so ornaments
        # and border lines render at BOOST× their fit-only pixel size.
        root_scale = min(_MB_ROOT_SCALE_MAX, _MB_ROOT_SCALE_BOOST)
...
def draw_toc_page(c, anaf_entries, page_offset):
    ...
    orn_h_pt     = 32.0 if _MAMAR_BOX_VARIANT == 'alt' else 13.0
...
def draw_anaf_heading(c, label, name, y):
    ...
    # Curl faces the label (inner side), line/tile extends outward
