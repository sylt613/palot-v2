...
# Optional frame-asset variant toggle.  Default = original GPT-15 frame.
# Set env MAMAR_BOX_VARIANT=alt to render with the new user-supplied frame
# (mamar_box_alt_asset.png) and write to palaot_altframe.pdf instead.
_MAMAR_BOX_VARIANT = os.environ.get('MAMAR_BOX_VARIANT', 'default').strip().lower() or 'default'
...
def _mamar_orn_h_pt(variant=None):
    """Nominal ornament height (pt) per variant. Centralised so all callers agree."""
    v = (variant or _MAMAR_BOX_VARIANT or 'default').lower()
    return {'alt': 32.0, 'newbox': 22.0}.get(v, 13.0)
...
def _mb_variant_source(variant):
    ...
    if v == 'alt':
        return _MAMAR_BOX_ALT_PATH, _MAMAR_BOX_ALT_BBOX, 408
    if v == 'newbox':
        # top_h_src=None → auto-detect straight band (we don't know the exact
        # top-ornament height for this new asset; auto-detect is reliable).
        return _MAMAR_BOX_NEWBOX_PATH, _MAMAR_BOX_NEWBOX_BBOX, None
    return _MAMAR_BOX_GPT15_PATH, _MAMAR_BOX_GPT15_BBOX, 275
