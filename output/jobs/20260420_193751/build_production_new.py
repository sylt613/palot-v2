...
_MAMAR_BOX_ORN_H_PT = {'default': 13.0, 'alt': 32.0, 'newbox': 22.0}
...
def _mb_get_geometry(frame_rgba, stroke_mask_arr, bbox, variant='default'):
    ...
    if cached is None:
        cached = (_mb_detect_cap_split(frame_rgba),
...
def _apply_mamar_box_halo(box_rgb):
    ...
    Pipeline (mirrors draw_post_anaf_divider_band):
      1. Extract ink mask (outline + flourishes) from the assembled RGB image.
      2. Small dilation → white mask (slightly thickens outline body).
      3. Larger dilation → gray mask (outer halo field).
      4. gray_ring = gray_mask - white_mask (ring only).
      5. Gaussian-blur the ring for soft faded edges.
      6. Composite onto pure white: gray ring first, then white-fill over ink+near.

    Tunable via settings (near_dil, far_blur, white_val, gray_val, fade_strength).
    """
    import numpy as _np
    from PIL import ImageFilter

    near_dil      = int(getattr(S, 'MAMAR_BOX_HALO_NEAR_DIL', 4))
    far_blur      = float(getattr(S, 'MAMAR_BOX_HALO_FAR_BLUR', 6))
    white_val     = int(getattr(S, 'MAMAR_BOX_HALO_WHITE_VAL', 252))
    gray_val      = int(getattr(S, 'MAMAR_BOX_HALO_GRAY_VAL', 130))
    fade_strength = float(getattr(S, 'MAMAR_BOX_HALO_FADE_STRENGTH', 0.55))

    # Dilation sizes must be odd ≥ 3. Convert pixel radius → MaxFilter kernel.
    def _odd(n):
        n = max(3, int(n))
        return n if n % 2 == 1 else n + 1
    white_k = _odd(near_dil * 2 + 1)
    gray_k  = _odd(int(near_dil * 2 + far_blur * 2) + 1)

    arr = _np.array(box_rgb.convert('RGB'))
    gray = arr.mean(axis=2)
    ink = (gray < 245).astype(_np.uint8) * 255
    ...
    w_arr = _np.array(white_mask, dtype=_np.int16)
    g_arr = _np.array(gray_mask,  dtype=_np.int16)
    ring_arr = _np.clip(g_arr - w_arr, 0, 255).astype(_np.uint8)
    ring = Image.fromarray(ring_arr, 'L')
    if far_blur > 0:
        ring = ring.filter(ImageFilter.GaussianBlur(far_blur))
    # Scale ring opacity by fade_strength.
    if fade_strength != 1.0:
        ring = ring.point(lambda p: int(p * fade_strength))

    # Build final RGB: start from ORIGINAL box (preserves interior fill choice),
    # then overlay the soft gray ring underneath by darkening only the ring region.
    out = box_rgb.convert('RGB').copy()
    # Ring darken: only where original is still near-white (don't muddy existing ink).
    orig_arr = _np.array(out, dtype=_np.int16)
    ring_arr = _np.array(ring, dtype=_np.int16)
    # Target gray value at ring peak = gray_val; fade linearly with ring alpha.
    # blend = orig * (1 - a) + gray_val * a, where a = ring/255 * fade_strength already baked.
    a = ring_arr.astype(_np.float32) / 255.0
    a3 = a[..., None]
    gray_rgb = _np.array([gray_val, gray_val, gray_val], dtype=_np.float32)
    blended = orig_arr.astype(_np.float32) * (1.0 - a3) + gray_rgb * a3
    # Keep existing ink (don't lighten outlines).
    orig_lum = orig_arr.mean(axis=2)
    ink_mask = orig_lum < 200
    blended[ink_mask] = orig_arr[ink_mask]
    out_arr = _np.clip(blended, 0, 255).astype(_np.uint8)
