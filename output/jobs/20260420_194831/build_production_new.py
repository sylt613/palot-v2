...
def _build_tiled_mamar_box_image(target_w_px, target_h_px, orn_h_px=83, variant=None):
...
def _apply_mamar_box_halo(box_rgb):
    ...
    Layered compose (outer → inner):
      1. Start from PURE WHITE canvas.
      2. Far gray ring  — ink dilated by (near + far), blurred, painted as
         faded light gray. This is the soft outer halo.
      3. Thick white band — ink dilated by `near` only, painted pure white.
         This SITS ON TOP of the gray ring, erasing the ring's inner portion
         and leaving a clean thick white cushion around the outline.
      4. Original ink — the box outline + flourishes pasted back with their
         original (gray) pixel values, so ornaments are never eaten.
    """
    import numpy as _np
    from PIL import ImageFilter

    near_dil      = int(getattr(S, 'MAMAR_BOX_HALO_NEAR_DIL', 14))
    far_blur      = float(getattr(S, 'MAMAR_BOX_HALO_FAR_BLUR', 10))
    white_val     = int(getattr(S, 'MAMAR_BOX_HALO_WHITE_VAL', 255))
    gray_val      = int(getattr(S, 'MAMAR_BOX_HALO_GRAY_VAL', 180))
    fade_strength = float(getattr(S, 'MAMAR_BOX_HALO_FADE_STRENGTH', 0.45))

    def _odd(n):
        n = max(3, int(n))
        return n if n % 2 == 1 else n + 1
    # near dilation grows ink → white band thickness
    white_k = _odd(near_dil * 2 + 1)
    # far dilation = near band + extra halo reach (then blurred)
    gray_k  = _odd(near_dil * 2 + int(far_blur * 2) + 1)

    arr = _np.array(box_rgb.convert('RGB'))
    lum = arr.mean(axis=2)

    # Optional ink darkening: remap gray source strokes to near-black BEFORE
    # painting halo, so the final outline reads as a single crisp line rather
    # than a muddy gray band.  Settings-driven; None disables.
    ...
    if dk_thr is not None and dk_tgt is not None:
        ...
        # Linearly remap [0..thr] → [0..tgt] for ink-side pixels.
        dark_mask = lum < thr
        if dark_mask.any():
    ...
    # Blur the gray mask edge so its outer fringe fades out smoothly.
    if far_blur > 0:
    ...
    # 1. Paint faded gray ring using the (blurred) gray_mask.
    ...
    # 2. Paint pure white band over inner region (erases ring near outline).
    ...
    # 3. Restore original ink pixels verbatim (preserves outline + flourishes).
...
def render_mamar_box_image(width_pt, height_pt, dpi=600,
    ...
    cache_key = (round(width_pt, 2), round(height_pt, 2), dpi,
                 round(orn_h_pt, 2), near_dil, far_blur, white_val, gray_val, variant)
    ...
    assembled = _build_tiled_mamar_box_image(w_px, h_px, orn_h_px=orn_h_px, variant=variant)
...
def draw_mamar_box(c, x_left, y_bot, width_pt, height_pt, **kwargs):
    ...
    reader, _pad = render_mamar_box_image(width_pt, height_pt, **kwargs)
    c.saveState()
    c.drawImage(reader,
                x_left, y_bot,
