...
def _apply_mamar_box_halo(box_rgb):
    """Apply the post-anaf-style halo to the assembled mamar box.

    Final look (no visible dark ink — pure white frame against gray halo):
      • Canvas is EXPANDED by `pad` px on each side so the gray halo has room
        to fade out without being clipped at the image edge.
      • Gray halo (far mask, blurred, faded) painted first.
      • Pure-white band (near mask) painted on top — this IS the visible
        "white line" of the frame (the user does not want any black).
      • Original source ink is NOT re-pasted (ink_preserve=False by default).
    """
    import numpy as _np
    from PIL import ImageFilter
    ...
    gray_k  = _odd(near_dil * 2 + int(far_blur * 2) + 1)
    ...
    if far_blur > 0:
        gray_mask = gray_mask.filter(ImageFilter.GaussianBlur(far_blur))

    H, W = arr.shape[:2]
    out = _np.full((H, W, 3), white_val, dtype=_np.float32)

    # 1. Gray halo ring.
    g_alpha = (_np.array(gray_mask, dtype=_np.float32) / 255.0) * fade_strength
...
def render_mamar_box_image(width_pt, height_pt, dpi=600,
    ...
    # Compute halo padding (in pt) based on settings so the gray glow isn't
    ...
    if getattr(S, 'MAMAR_BOX_HALO', False):
        ...
        halo_pad_pt = max(halo_pad_pt, (_near + _far + 8) * 72.0 / dpi)
...
DC_FONT = S.DROPCAP_FONT_NAME     # first word uses dropcap font (pninaregular)
