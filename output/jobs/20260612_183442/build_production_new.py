...
def _base_chars(text):
    """Non-combining base glyphs in a (logical) string — the letter-spacing slots."""
    return [ch for ch in text if not unicodedata.combining(ch)]
...
def _just_gap_and_charspace(tot, gaps, col_width, font, size, letter_gaps):
    ...
    sp_t = _just_target_space(font, size)
    cap = sp_t * float(getattr(S, 'JUST_WORD_GAP_MAX_MULT', 1.5))
    if gw_nat <= cap:
        return gw_nat, 0.0
    residual = (gw_nat - cap) * gaps
    char_extra = 0.0
    ...
    # back into the gap, but reduced from its uncapped value.
    gap_w = cap + (residual / gaps if residual > 0.01 else 0.0)
...
def _draw_word_tracked(c, x_left, y, vis_text, font, size, char_extra):
    ...
    between base glyphs, via a text object. char_extra==0 falls back to plain."""
    ...
    t.setFont(font, size)
    t.setCharSpace(char_extra)   # uniform extra advance after every glyph
    t.textOut(vis_text)
    c.drawText(t)

def _word_tracked_w(vis_text, font, size, char_extra):
    """Drawn width of a word including letter-spacing (so positions stay exact)."""
    base = Wid(vis_text, font, size)
    if char_extra <= 1e-6:
        return base
    # setCharSpace adds char_extra after EVERY glyph drawn (n slots for n glyphs).
    return base + char_extra * len(vis_text)
...
def draw_line_with_fn(c, words, font, size, x_right, y, col_width, last, fn_counter_base):
    ...
    if not has_any_fn and not mixed:
        ...
        if last:
            ...
            # change). letter_gaps = number of intra-word base-glyph slots.
            vis_words = [livorna_fix_quotes(vis(w)) for w in words]
            letter_gaps = sum(len(vw) for vw in vis_words)
    ...
    if last:
        ...
        plain_letter_gaps = sum(
            len(livorna_fix_quotes(vis(tw_str(w))))
            for w in words
