...
def _draw_anaf_flanker(c, x_left, x_right, y_center, side='right'):
    ...
    # TEMP degraded for before-render
    render_h = 13.6
    ppi = 300
...
_JUST_STATS = []  # diagnostic: (gap_w, char_extra, sp_nat, size, letter_gaps) per justified line
def _just_stats_record(gap_w, char_extra, sp_nat, size, letter_gaps):
    if os.environ.get('JUST_STATS'):
        _JUST_STATS.append((gap_w, char_extra, sp_nat, size, letter_gaps))
...
def _just_gap_and_charspace(tot, gaps, col_width, font, size, letter_gaps):
    ...
    if gw_nat <= cap:
        _just_stats_record(gw_nat, 0.0, sp_nat, size, letter_gaps)
        return gw_nat, 0.0
    ...
    _just_stats_record(gap_w, char_extra, sp_nat, size, letter_gaps)
...
if __name__ == "__main__":
    main()
    if os.environ.get('JUST_STATS') and _JUST_STATS:
        import statistics as _st
        gaps = [g for (g, ce, sp, sz, lg) in _JUST_STATS]
        # gap as multiple of natural space, and char_extra as em fraction
        gmult = [g / sp for (g, ce, sp, sz, lg) in _JUST_STATS if sp > 0]
        ce_em = [ce / sz for (g, ce, sp, sz, lg) in _JUST_STATS if sz > 0]
        active_ce = [x for x in ce_em if x > 1e-6]
        print("=== JUST_STATS ===")
        print(f"lines={len(_JUST_STATS)}")
        print(f"gap_w pt: mean={_st.mean(gaps):.3f} median={_st.median(gaps):.3f} max={max(gaps):.3f}")
        print(f"gap/natural-space mult: mean={_st.mean(gmult):.3f} median={_st.median(gmult):.3f} max={max(gmult):.3f}")
        print(f"char_extra em: mean={_st.mean(ce_em):.4f} max={max(ce_em):.4f}; nonzero lines={len(active_ce)} ({100*len(active_ce)/len(ce_em):.0f}%)")
        if active_ce:
            print(f"char_extra em (nonzero only): mean={_st.mean(active_ce):.4f} median={_st.median(active_ce):.4f} max={max(active_ce):.4f}")
