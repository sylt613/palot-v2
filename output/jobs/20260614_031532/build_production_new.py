...
class PageLayout:
    ...
    def _render_page(self, elements, el_idx, line_off, fn_off, carry_fn_lds):
        ...
        # ── Section-tail redistribution pullback ──
        # When this (donor) page is full but the next page is a SHORT section
        # tail ending just before an anaf heading, that tail page renders
        # catastrophically short (the owner's "horrible" half-empty page — e.g.
        # a lone subhead + a few lines + the closing ornament).  Pull trailing
        # paragraph group(s) from this page back to the tail so BOTH pages end
        # reasonably full.  Triggered only for a true section tail; thresholds
        # are fractions of the available body height (settings-relative).
        ...
        def _section_tail_pull():
            ...
            if not getattr(S, 'SECTION_TAIL_PULL_ENABLE', True):
                return False
            if is_final_flush or not acc_paras or not acc_origins:
                return False
            if len(acc_paras) != len(acc_origins):
                return False
            # Next content must be a section tail: zero or more paras then an anaf.
            _ei = cur_el_idx
            _tail_paras = []
            ...
            _committed = False
            _groups = 0
            # Footnote-bearing paragraphs are declined (the rebuilt tail run does
            # not re-derive their footnote line-dicts); bail before corrupting.
            while acc_paras and _groups < _max_groups:
                _last_o = acc_origins[-1]
                _grp = _pop_group()
                if not _grp:
                    break
                _groups += 1
                _moved = _grp + _moved
                # Donor fill after this pull (recompute its column height).
                _donor_h = _run_height(list(acc_paras)) if acc_paras else 0.0
                # Tail run = pulled paras (in document order) + original tail.
        ...
        _did_tail_pull = _section_tail_pull()
