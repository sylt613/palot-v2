...
def _draw_anaf_flanker(c, x_left, x_right, y_center, side='right'):
    ...
    """
    render_h = float(getattr(S, 'ANAF_FLANKER_H', 9.0))   # small; settings-driven
    slot = x_right - x_left
    ...
    if info is None:
        try:
            ...
            if bbox:
                img = img.crop(bbox)
            # side='right' (right of label): curl must be on the OUTER (right)
            # end with the line fading LEFT toward the label → mirror the PNG
            # (PNG is curl-left/line-right). side='left': use as-is (curl-left).
            if side == 'right':
    ...
    if draw_w > slot:                    # never exceed the reserved slot
        draw_w = slot
    # Anchor at the OUTER end of the slot (curl outward), line points inward.
    x = (x_right - draw_w) if side == 'right' else x_left
    c.saveState()
