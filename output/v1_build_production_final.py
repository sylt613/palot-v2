...
SUB_AFTER_FRAG_GAP = 0.0
MIN_SUB_AFTER_GAP = 0.0
MAX_SUB_AFTER_GAP = 0.0
...
def draw_contact_page(c):
    ...
    lines = [
        ...
        ("(845) 825 - 1850", HEADING_FONT, 12.0, 0.12),
    ]
...
def heading_height(text, kind='section', has_name=True):
    if kind == 'anaf':
        ...
        return SEFER_TITLE_FS + HEADING_AFTER_GAP + 18
    elif kind == 'anaf_sub':
        fs = SEC_FS
        font = HEADING_BOLD_FONT
...
def _stretchable_gaps(cd, para_run):
    ...
    Gaps after subhead-ending fragments are always clamped to 0 (dead),
    ...
    for ci in range(len(cd) - 1):
        ...
        if _is_subhead_ending_frag(para_run, cur_kind, cur_pi, cur_k):
            # This gap is clamped to [MIN_SUB_AFTER_GAP, MAX_SUB_AFTER_GAP]
            # which are both 0 — it's a dead gap, skip it
            continue
        count += 1
    return count
...
def _compute_top_pads(h1_final, h2_final, c1d=None, c2d=None, para_run=None):
    """After gap-stretching, equalize column heights by adding top-padding
    to the shorter column so both columns end at exactly the same Y position.
    Returns (e_top1, e_top2).

    Uses RAW rendered heights (not visual discount) — the goal is pixel-perfect
    bottom alignment regardless of subhead content."""
    diff = h1_final - h2_final
    if abs(diff) <= 0.05:
        return 0.0, 0.0

    if diff > 0:
        # col1 is taller → pad col2
        return 0.0, min(diff, MAX_TOP_PAD)
    else:
        # col2 is taller → pad col1
        return min(-diff, MAX_TOP_PAD), 0.0
...
def _col_layout(para_run, allow_trailing_subhead=False, max_height=None):
    ...
    **BUG FIX**: Enhanced subhead keep-with-next logic.
    ...
    def _compute_padding(c1d, c2d, h1, h2):
        ...
        def _max_shrink(cd):
            ...
            for ci in range(len(cd) - 1):
                cur_kind, cur_pi, cur_k = cd[ci]
                if _is_subhead_ending_frag(para_run, cur_kind, cur_pi, cur_k):
                    continue  # dead gap — already at 0, can't shrink
        ...
        # Use visually-effective heights for balancing target:
        # a column with a subhead has extra whitespace that doesn't contain text,
        # so we discount it to align the last visible text lines.
        d1 = _svd(c1d)
        d2 = _svd(c2d)
        vh1 = ah1 - d1
        vh2 = ah2 - d2
        diff = vh1 - vh2  # positive = col1 visually taller
    ...
    def _effective_residual(c1d, c2d, h1, h2):
        ...
        Returns a tuple: (hard_residual, per_gap_penalty).
        hard_residual = height diff that can't be absorbed at all.
        per_gap_penalty = max per-gap stretch needed (lower = better spread).
        """
        MAX_E_PER_GAP = LH * 1.5   # reasonable max per gap (~26pt)
        diff = abs(h1 - h2)
        ...
        def _shrink_cap(cd):
            ...
            return total
        if h1 > h2:
            grow_cap = gaps2 * MAX_E_PER_GAP if gaps2 > 0 else 0.0
            shrink_cap = _shrink_cap(c1d)
            # Only credit top-pad capacity if col2 starts with a subhead
            top_pad_cap = MAX_TOP_PAD if _starts_with_subhead_frag(c2d) else 0.0
        else:
            grow_cap = gaps1 * MAX_E_PER_GAP if gaps1 > 0 else 0.0
            shrink_cap = _shrink_cap(c2d)
            # Only credit top-pad capacity if col1 starts with a subhead
...
class PageLayout:
    ...
    def build(self):
        ...
        writer = PdfWriter()
        # Page 1: shaar blatt (title page) from covers
        shaar_reader = PdfReader(SHAAR_BLATT_PDF)
    ...
    def _render_page(self, elements, el_idx, line_off, fn_off, carry_fn_lds):
        ...
        while cur_el_idx < len(elements):
            ...
            if used_h + trial_h + sep + _orn_h_after(cur_el_idx + 1) <= body_avail(len(trial_fns)) + 0.01:
                acc_origins.append({
